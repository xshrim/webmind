...
- `src/sidepanel/App.tsx`：另负责待办页的加载、筛选、搜索、编辑、完成/恢复、删除、清理、回答转待办、智能拆分和来源跳转；待办内容不自动进入普通聊天消息、历史或 MCP 请求，智能拆分时会作为专用输入发送给默认模型。
...
- 划词浮层的“侧边栏提问”按钮永远是第一项；设置页可配置复制、搜索、待办和二维码。复制、搜索、待办和二维码均已实现。
...
- `onStartup` 必须调用完整的 `setupExtension()`，不能只设置侧边栏行为：Chrome 可能保留已经显示的菜单，但 Service Worker 内存中的 `menuActions` 会在重启后丢失，必须同步重建动作映射。删除自定义工具时，所有 `enabledToolIds` 表面都要移除该 ID，包括 `context-menu`。
...
- 2026-08-13：修复扩展重启后右键菜单动作映射未恢复的问题；删除自定义工具时同步清理右键菜单工具配置，并补充配置归一化回归测试。
- 2026-08-13：完成第一阶段待办：新增独立待办模型、本地存储、后台 CRUD 路由、划词浮层和右键菜单创建入口，以及侧边栏搜索/筛选/编辑/完成/删除和来源跳转；操作日志只记录动作，不记录待办正文。
- 2026-08-14：回答操作栏新增“加入待办”；待办项新增智能拆分，使用默认模型返回 2 到 20 个 JSON 待办，并通过后台原子替换保证模型失败时原待办不丢失；标题上限收紧到 80 个字符，右侧折叠、拆分、编辑、删除按钮改为紧凑布局。
...
- 右键菜单工具依赖 `selection` 或 `editable` 上下文；图片、视频、链接等其他 Chrome 菜单上下文目前不会单独生成专用工具菜单项。收藏和分享按钮目前只有视觉入口；划词浮层、回答操作栏和右键菜单的待办入口已经接入本地存储，待办智能拆分需要已配置可用的默认模型。
