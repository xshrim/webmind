...
- 侧边栏当前正文预览支持质量评分、段落高亮定位、手动框选正文、逐段删除、智能剔除和还原正文。删除或智能剔除属于当前正文编辑状态，会同步影响自动翻译、沉浸翻译和沉浸阅读；还原正文会清除编辑状态，但不会改变预览当前的展开/折叠状态。
...
- 尚未为自动补注入、受限页面失败提示、正文预览段落删除和智能剔除补充完整的 Chrome 自动化测试。
...
1. 在真实 Chrome 中回归当前正文、预览评分 tag、手动框选/逐段删除/智能剔除/还原、划词浮层打开侧边栏和选区消失后的上下文切换。
...
测试结果：`10` 个测试文件、`80` 个测试通过；构建成功生成未打包扩展目录 `dist/`。以上是代码级验证，尚未替代真实 Chrome 手工验证。
...
- 检查当前正文预览中“智能剔除”是否位于段数/字数/DOM 等 tag 之前，逐段删除图标是否为 `X`，以及编辑后评分和质量 tag 是否刷新。
