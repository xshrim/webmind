import type { AppLanguage } from "./types";
import { deUiText } from "./locales/de";
import { esUiText } from "./locales/es";
import { frUiText } from "./locales/fr";
import { itUiText } from "./locales/it";

export type ResolvedLanguage = Exclude<AppLanguage, "auto">;
export type PromptLanguage = Exclude<ResolvedLanguage, "es" | "fr" | "de" | "it">;

export const LANGUAGE_OPTIONS: Array<{ id: AppLanguage; label: string }> = [
  { id: "auto", label: "自动" },
  { id: "zh-CN", label: "简体中文" },
  { id: "zh-TW", label: "繁體中文" },
  { id: "en", label: "English" },
  { id: "ja", label: "日本語" },
  { id: "ko", label: "한국어" },
  { id: "es", label: "Español" },
  { id: "fr", label: "Français" },
  { id: "de", label: "Deutsch" },
  { id: "it", label: "Italiano" }
];

export const LANGUAGE_LABELS: Record<ResolvedLanguage, string> = {
  "zh-CN": "简体中文",
  "zh-TW": "繁體中文",
  en: "English",
  ja: "日本語",
  ko: "한국어",
  es: "Spanish",
  fr: "French",
  de: "German",
  it: "Italian"
};

export function resolveLanguage(
  language: AppLanguage | undefined,
  browserLanguage = typeof navigator !== "undefined" ? navigator.language : "zh-CN"
): ResolvedLanguage {
  if (language && language !== "auto") return language;
  const normalized = browserLanguage.toLowerCase();
  if (normalized.startsWith("zh-tw") || normalized.startsWith("zh-hk")) {
    return "zh-TW";
  }
  if (normalized.startsWith("en")) return "en";
  if (normalized.startsWith("ja")) return "ja";
  if (normalized.startsWith("ko")) return "ko";
  if (normalized.startsWith("es")) return "es";
  if (normalized.startsWith("fr")) return "fr";
  if (normalized.startsWith("de")) return "de";
  if (normalized.startsWith("it")) return "it";
  return "zh-CN";
}

export function resolvePromptLanguage(
  language: AppLanguage | undefined,
  browserLanguage?: string
): PromptLanguage {
  const resolved = resolveLanguage(language, browserLanguage);
  return resolved === "es" ||
    resolved === "fr" ||
    resolved === "de" ||
    resolved === "it"
    ? "en"
    : resolved;
}

const CORE_UI_TEXT = {
  "zh-CN": {
    askSelectionTitle: "在侧边栏提问",
    askSelectionDescription: "把当前内容交给侧边栏继续提问",
    copy: "复制",
    more: "更多",
    webmindAnswer: "WebMind 回答",
    showTools: "显示 WebMind 工具",
    copySelection: "复制所选文本",
    searchSelection: "搜索所选文本",
    openSidebar: "打开侧边栏",
    immersiveTranslateApplied: "已在页面中应用沉浸翻译。",
    searchAnswerSystem:
      "你是 WebMind 搜索回答助手。用户正在搜索引擎结果页。请基于 DuckDuckGo 返回的网页搜索结果回答，优先引用结果中的信息；信息不足时说明不足，不要假装访问了未提供的网页。回答要简洁、结构清楚。",
    searchQuery: "搜索词",
    duckResults: "DuckDuckGo 网页搜索结果",
    duckNoResults: "DuckDuckGo 网页搜索结果：没有返回可用结果。",
    searchAnswerRequest: "请直接回答这个搜索问题，并在必要时列出可继续确认的方向。",
    sourceCitationInstruction:
      "每个基于搜索结果的段落末尾必须用 [搜索 N] 标注来源编号，N 对应上方结果序号。不要在末尾列来源列表，不要输出来源 URL。",
    browserAssistantSystem:
      "你是浏览器中的写作与阅读助手。忠实处理用户提供的文本，不虚构上下文。",
    modelToolSystem: "你是 WebMind 工具执行器。直接完成工具指令，只返回用户需要的结果。",
    unsupportedQuickAction: "不支持的快捷操作",
    currentContext: "当前上下文",
    assistantSystem:
      "你是 WebMind，一个在浏览器侧边栏中工作的研究、阅读与写作助手。默认使用与用户相同的语言回答。",
    assistantGuard:
      "严格区分上下文中明确给出的事实和你的推断。上下文不足时直接说明。不要声称浏览、点击或读取了没有提供给你的内容。",
    selectionContextIntro:
      "以下是用户主动选择的文本。本轮操作只针对这段选中内容，不要扩展到页面中未提供的部分：",
    pageContextIntro: "以下是用户主动附带的整个页面上下文：",
    articleContextIntro: "以下是从当前页面提取的正文内容：",
    translationInputIntro: "以下内容是本次必须翻译的输入正文：",
    title: "标题",
    url: "地址",
    description: "说明",
    body: "正文：",
    selectionOnly: "回答时应把“所选文本”作为唯一页面资料。",
    pdfCitation: "回答 PDF 问题时尽量引用“第 N 页”。",
    youtubeCitation: "回答视频问题时尽量引用字幕时间戳。",
    pageCitation: "引用当前页面时可使用“当前页面”并点明相关段落。",
    searchSummaryIntro:
      "以下是网页搜索结果摘要。回答时用 [搜索 1]、[搜索 2] 标注依据，并在末尾列出实际使用的来源：",
    noContextImage: "请分析这张图片。",
    noContextAttachment: "请根据附件内容回答。",
    attachmentIntro: "以下是用户添加的附件内容：",
    languageSetting: "界面语言",
    languageSettingHelp: "影响界面文字、内置工具名称和默认提示词。",
    translationLanguageSetting: "译文语言",
    translationLanguageSettingHelp:
      "基于上下文和界面语言自动判断或手动指定。",
    defaultContextScopeSetting: "默认上下文",
    defaultContextScopeHelp:
      "对话/工具/沉浸翻译和阅读默认携带的上下文。",
    immersiveTranslationParagraphShortcut: "当前段落沉浸翻译快捷键",
    immersiveTranslationPageShortcut: "当前上下文沉浸翻译快捷键",
    immersiveTranslationModeToggleShortcut: "译文显示模式切换快捷键",
    immersiveTranslationShortcutHelp:
      "当前段落用于选区或段落内容，当前上下文会跟随侧边栏当前选中的上下文。",
    shortcutCtrl: "Ctrl",
    shortcutAlt: "Alt",
    shortcutShift: "Shift",
    shortcutHoldPrefix: "按住:",
    navChat: "对话",
    navMcp: "MCP",
    mcpServers: "MCP Server",
    mcpServersHelp: "连接 SSE 或 Streamable HTTP Server，并查看可用工具。",
    addMcpServer: "添加 MCP Server",
    editMcpServer: "编辑 MCP Server",
    noMcpServers: "还没有 MCP Server",
    mcpServerName: "名称",
    mcpServerUrl: "Server URL",
    mcpTransport: "传输方式",
    mcpHeaders: "自定义请求头（JSON）",
    saveAndConnect: "保存并连接",
    refreshMcpTools: "刷新工具",
    noMcpTools: "该 Server 没有返回工具",
    mcpNoDescription: "没有描述",
    mcpServerSaved: "MCP Server 已保存",
    mcpPermissionDenied: "未授予 MCP Server 网络权限",
    mcpHttpOnly: "MCP Server 只支持 HTTP 或 HTTPS 地址",
    enableMcp: "选择 MCP 工具",
    mcpSelectionSummary: "{servers} 个 MCP 的 {tools} 个工具",
    approveMcpTool: "允许执行 MCP 工具？",
    mcpDestructiveWarning: "此工具声明可能修改或删除外部数据。",
    deny: "拒绝",
    allowOnce: "本次允许",
    allowRound: "本轮允许",
    allowSession: "会话允许",
    mcpToolApprovalMode: "MCP工具授权模式",
    mcpToolApprovalModeHelp:
      "控制已启用 MCP 工具的全局执行授权；始终询问时可在对话中按本次、本轮或会话授权。",
    mcpToolApprovalDeny: "始终拒绝",
    mcpToolApprovalAsk: "始终询问",
    mcpToolApprovalAllow: "始终允许",
    cookieViewer: "查看 Cookie",
    cookieFormat: "格式",
    cookiePreview: "预览",
    cookiePreviewCount: "（{count} 个 Cookie）",
    mcpToolUsed: "已使用 MCP 工具",
    mcpToolNotCalled: "未调用 MCP 工具",
    mcpToolFailed: "MCP 工具调用失败",
    mcpToolBlockedGlobalDeny: "已按全局设置拒绝",
    mcpToolBlockedUserDeny: "用户拒绝授权",
    mcpToolBlockedApprovalTimeout: "等待授权超时",
    navTools: "工具",
    navTodos: "待办",
    navHistory: "历史",
    navLogs: "日志",
    logsPageTitle: "日志",
    logsPageHeading: "操作日志",
    historyPageEyebrow: "历史",
    localSessionsTitle: "历史会话",
    todoPageEyebrow: "待办",
    localTodosTitle: "待办事项",
    todosTitle: "待办",
    todoItemsTitle: "待办事项",
    todoAdd: "添加待办",
    todoEdit: "编辑待办",
    todoContent: "正文",
    todoSplit: "智能拆分",
    todoSplitRunning: "正在智能拆分待办",
    todoSplitInvalid: "大模型没有返回至少两个有效待办",
    todoSplitSuccess: "待办已拆分",
    todoSplitSystemPrompt:
      "请将用户提供的一个待办拆分为多个相互独立、可执行的待办。只返回 JSON 字符串数组，不要返回 Markdown 或其他说明；每个字符串只包含一个待办的正文，至少返回 2 项，最多返回 20 项。不要添加原文没有依据的新任务。",
    todoSearchPlaceholder: "搜索待办",
    todoFilter: "待办筛选",
    todoAll: "全部",
    todoOpen: "未完成",
    todoCompleted: "已完成",
    todoMarkComplete: "标记完成",
    todoMarkOpen: "恢复未完成",
    todoMarkInProgress: "标记为进行中",
    todoMarkNotInProgress: "取消进行中标记",
    todoInProgressDisabled: "已完成事项不可标记为进行中",
    todoExpand: "展开待办",
    todoCollapse: "折叠待办",
    todoContentExpand: "展开内容",
    todoContentCollapse: "折叠内容",
    todoDelete: "删除待办",
    todoClearCompleted: "清理已完成",
    todoClearAll: "清除全部待办",
    todoEmpty: "还没有待办",
    todoEmptyHelp: "可以从划词浮层、右键菜单或这里创建待办。",
    todoNoSearchResults: "没有匹配的待办",
    todoCreatedFromSelection: "来自选中文本",
    todoCreatedManually: "手动创建",
    todoSaved: "已加入待办",
    todoTitleRequired: "请填写待办正文",
    logTodoSplit: "已智能拆分待办",
    messageAddToTodo: "将回答加入待办",
    logTodoCreated: "已创建待办",
    logTodoUpdated: "已更新待办",
    logTodoCompleted: "已完成待办",
    logTodoReopened: "已恢复待办",
    logTodoDeleted: "已删除待办",
    logTodoCleared: "已清理完成待办",
    logTodoAllCleared: "已清除全部待办",
    addSelectionToTodo: "加入待办",
    operationLogs: "操作日志",
    operationLogsHelp: "产品执行的关键操作会实时显示在这里。",
    clearLogs: "清空日志",
    noOperationLogs: "暂无日志",
    logLevelDebug: "调试",
    logLevelInfo: "信息",
    logLevelSuccess: "成功",
    logLevelWarning: "提醒",
    logLevelError: "错误",
    logSidepanelReady: "侧边栏已准备就绪",
    logSettingsUpdated: "设置已更新",
    logToolsUpdated: "工具配置已更新",
    logPendingAction: "收到页面操作",
    logChatStart: "开始对话",
    logChatDone: "回答完成",
    logChatCancelled: "回答已取消",
    logChatStop: "用户停止回答",
    logChatRegenerate: "重新回答",
    logToolRun: "执行工具",
    logToolSelected: "选择工具",
    logAskSelectionReady: "已切换到侧边栏提问",
    logNewChat: "新建对话",
    logAttachmentAdded: "添加附件",
    logConversationLoaded: "载入历史对话",
    logMcpApproval: "MCP 工具授权：{decision} - {server} / {tool}",
    logMcpToolCalled: "MCP 工具调用成功：{server} / {tool}",
    logMcpToolFailed: "MCP 工具调用失败：{server} / {tool}",
    logMcpToolBlocked: "MCP 工具未执行：{server} / {tool}",
    logMcpServerSaved: "MCP Server 已保存：{server}（{count} 个工具）",
    logMcpToolsRefreshed: "MCP 工具已刷新：{server}（{count} 个工具）",
    logMcpServerDeleted: "MCP Server 已删除：{server}",
    logEnabled: "已启用",
    logDisabled: "已关闭",
    logRuntimeRequest: "运行请求",
    displayLogLevel: "显示日志级别",
    displayLogLevelHelp:
      "日志面板只显示所选级别及以上的日志；调试级别会显示每次大模型请求的详细记录。",
    cancel: "取消",
    close: "关闭",
    save: "保存",
    saving: "保存中",
    add: "添加",
    edit: "编辑",
    delete: "删除",
    test: "测试",
    testing: "测试中",
    current: "当前",
    modelRoles: "角色",
    defaultModelRole: "默认",
    translationModelRole: "翻译",
    visionModelRole: "视觉",
    setDefaultModelRole: "设为默认模型",
    setTranslationModelRole: "设为翻译模型",
    clearTranslationModelRole: "取消翻译模型",
    setVisionModelRole: "设为视觉模型",
    clearVisionModelRole: "取消视觉模型",
    visionModelRoleUnavailable: "该引擎未启用图片识别，不能设为视觉模型",
    settings: "设置",
    loading: "加载中",
    newChat: "新对话",
    send: "发送",
    stop: "停止",
    restorePage: "还原页面",
    chooseModel: "选择模型",
    currentModelEngine: "当前模型引擎",
    selectTool: "选择工具",
    selectMoreTools: "选择更多工具",
    moreTools: "更多工具",
    copyContent: "复制内容",
    clearComposer: "清空输入",
    copyCurrentBody: "复制正文",
    copyUrl: "复制网页 URL",
    copied: "已复制",
    regenerate: "重新回答",
    continueExecution: "继续执行",
    replace: "替换",
    closeNotice: "关闭提示",
    removeAttachment: "移除附件",
    openTools: "打开工具",
    currentPage: "当前页面",
    currentBody: "当前正文",
    currentSelection: "当前选中",
    currentBodyPreview: "上下文预览",
    nonePreview: "预览",
    pagePreview: "页面预览",
    bodyPreview: "正文预览",
    selectionPreview: "选中预览",
    expand: "展开",
    collapse: "收起",
    currentBodyBlocks: "{count} 段",
    currentBodyChars: "{count} 字",
    currentBodyScore: "正文评分：{score}",
    articleMetricLength: "长度",
    articleMetricStructure: "结构",
    articleMetricHeading: "标题",
    articleMetricSemantics: "语义容器",
    articleMetricDensity: "文本密度",
    articleMetricLinkPurity: "链接纯度",
    articleMetricFocus: "聚焦度",
    articleMetricCleanliness: "干净度",
    currentBodySourceDom: "DOM",
    currentBodySourceRule: "规则",
    currentBodySourceManual: "手动选择",
    currentBodySourceEdited: "已编辑",
    selectCurrentBody: "框选正文",
    restoreCurrentBody: "还原正文",
    highlightCurrentBodyBlock: "在页面中高亮这一段",
    removeCurrentBodyBlock: "从当前正文中删除这一段",
    smartPruneCurrentBody: "自动剔除",
    modelPruneCurrentBody: "智能剔除",
    modelPruneCurrentBodyRunning: "正在让大模型判断正文 block",
    modelPruneInvalidResult: "大模型返回的正文剔除结果无法解析",
    modelPruneNoChanges: "大模型判断没有需要剔除的正文 block",
    modelPruneAllRejected: "为避免误删，不能剔除全部正文 block",
    articleRecognition: "正文识别",
    recognitionRules: "识别规则",
    articleExtractionRules: "正文提取规则",
    articleExtractionRulesHelp:
      "每条规则包含适配页面地址和正文 selector；地址支持域名、通配符和 URL 片段。",
    articleExtractionUrlPattern: "适配页面地址",
    articleExtractionSelector: "正文 selector",
    articleExtractionUrlPatternPlaceholder:
      "页面地址，例如 example.com、*.example.com、https://example.com/*",
    articleExtractionSelectorPlaceholder:
      "CSS selector，例如 article、main、#content、.post-body",
    addArticleExtractionRule: "添加正文规则",
    deleteArticleExtractionRule: "删除正文规则",
    noArticleExtractionRules: "还没有正文提取规则",
    articleExtractionRuleSaved: "正文提取规则已保存",
    articleExtractionRuleInvalid: "请填写页面地址和 selector",
    selectingBodyRange: "正在选择正文范围",
    manualBodySelectionHint: "滚轮/方向键调整范围，点击确认，ESC取消",
    manualBodySelectionCancelled: "已取消正文框选",
    noneContext: "无上下文",
    webSearch: "网页搜索",
    composerWebSearch: "联网搜索",
    addAttachment: "添加图片或文档",
    captureScreenshot: "截图",
    composerAddScreenshot: "添加截图",
    screenshotSelectArea: "请在页面上拖拽框选截图区域，按 Esc 取消。",
    addUrl: "添加 URL",
    you: "你",
    ordinaryConversation: "普通对话",
    usedTool: "使用工具",
    questionContext: "提问上下文",
    imageChat: "图片对话",
    openAnyPage: "打开任意网页，然后从这里开始。",
    noEnabledTools: "这个位置还没有启用工具",
    toolsPageShowsAll: "工具页会显示全部可用工具。",
    noSavedConversations: "还没有保存的对话",
    conversationsAutoSave: "完成一次问答后会自动保存在本地。",
    languageOptionAuto: "自动",
    languageOptionZhCN: "简体中文",
    languageOptionZhTW: "繁體中文",
    languageOptionEn: "英语",
    languageOptionJa: "日语",
    languageOptionKo: "韩语",
    appSubtitle: "本地优先的浏览器模型工作台",
    modelEngines: "模型引擎",
    modelEnginesDescription:
      "不需要账号或订阅。默认、翻译和视觉标记分别只能绑定一个模型，同一个模型可以同时拥有多个标记。翻译和视觉任务未指定专项模型时会使用默认模型。引擎配置与密钥保存在扩展存储中。",
    addEngine: "添加引擎",
    noModelEngines: "还没有模型引擎",
    noModelEnginesHelp: "添加常见云端模型、OpenAI 兼容接口或本地 Ollama。",
    pageFeatures: "页面功能",
    pageFeaturesHelp: "每个模块的设置独立收纳，便于确认影响范围。",
    selectionOverlay: "划词浮层",
    selectionOverlayHelp: "控制选择文字后的悬浮工具栏。",
    selectionOverlayMode: "划词浮层",
    selectionOverlayShortcut: "划词浮层快捷键",
    selectionOverlayShortcutHelp:
      "启用后，只有按住所选快捷键选择文字或查看已有选区时才显示划词浮层。",
    selectionOverlayMinChars: "最少触发字符数",
    selectionOverlayMinCharsHelp:
      "选中文字达到该字符数时才触发划词浮层和同文高亮，最低为 1。",
    selectionMatchHighlight: "同文高亮",
    selectionMatchHighlightHelp:
      "选中页面文字后，高亮所有相同内容，并在右侧滚动位置显示可点击的定位标记。",
    selectionMatchHighlightOff: "不启用",
    selectionMatchHighlightIgnoreCase: "忽略大小写",
    selectionMatchHighlightCaseSensitive: "大小写敏感",
    linkTextSelection: "允许选择页面内链接文字",
    linkTextSelectionHelp:
      "横向拖拽链接文字时按普通文本选择；纵向拖拽保持链接原有行为。普通单击仍会打开链接。",
    linkSelection: "链接选择",
    selectionOverlayOff: "不启用",
    selectionOverlayOffHelp: "选择文字后不显示任何快捷入口。",
    selectionOverlayAlways: "直接显示",
    selectionOverlayAlwaysHelp: "选择文字后立即显示可用工具。",
    selectionOverlayHover: "先显示浮点",
    selectionOverlayHoverHelp: "选择文字后显示小圆点，悬停后展开工具栏。",
    urlBlacklist: "网址黑名单",
    selectionOverlayBlacklistHelp: "每行一条规则；支持域名、通配符和 URL 片段。",
    edgeQuickTools: "快捷工具",
    edgeQuickToolsHelp: "统一控制链接选择、贴边菜单、图文提取和自动回复。",
    edgeDockMenu: "贴边菜单",
    edgeQuickToolsEnable: "启用贴边菜单",
    edgeQuickToolsEnableHelp: "关闭后网页右侧不会显示贴边菜单。",
    edgeQuickToolsBlacklistHelp: "命中后页面右侧快捷工具不会显示。",
    quickToolsBlacklistHelp: "命中后链接选择、贴边菜单、图文提取和自动回复都不会在页面中生效。",
    githubCloneUrl: "GitHub 克隆地址",
    githubSshCloneUrlRewrite: "GitHub克隆SSH使用443端口",
    githubSshCloneUrlRewriteHelp:
      "在 GitHub 仓库的 Code 菜单中，将 SSH 克隆地址改为 ssh.github.com:443 URI。",
    githubSshCloneUrlRewriteOff: "不启用",
    githubSshCloneUrlRewriteTextOnly: "仅文本框启用",
    githubSshCloneUrlRewriteCopyOnly: "仅复制按钮启用",
    githubSshCloneUrlRewriteAll: "完全启用",
    immersiveTranslation: "沉浸翻译",
    immersiveTranslationHelp: "控制译文写回页面后的呈现方式。",
    immersiveTranslationAutoWhitelist: "自动沉浸翻译白名单",
    immersiveTranslationAutoWhitelistHelp:
      "每行一条规则；支持域名、通配符和 URL 片段。页面打开并命中后会自动进行沉浸翻译。",
    immersiveReading: "沉浸阅读",
    immersiveReadingHelp: "把页面中符合难度的词语替换为学习语言，形成母语与非母语混合阅读。",
    immersiveReadingStrategy: "执行方案",
    immersiveReadingStrategyLocalFirst: "本地优先",
    immersiveReadingStrategyLocalFirstHelp:
      "优先使用本地词表筛选和离线词典翻译，速度更快、消耗更低；大模型仅做补充。",
    immersiveReadingStrategyModelPage: "模型优先",
    immersiveReadingStrategyModelPageHelp:
      "将页面内容交给大模型结合上下文筛选和翻译，效果更灵活，但速度和费用取决于模型。",
    immersiveReadingAutoWhitelist: "自动沉浸阅读白名单",
    immersiveReadingAutoWhitelistHelp:
      "每行一条规则；支持域名、通配符和 URL 片段。页面打开并命中后会自动进行沉浸阅读。",
    immersiveReadingDifficulty: "词语难度",
    immersiveReadingDifficultyHelp: "难度越高，模型越倾向于替换更少、更难的词语。",
    immersiveReadingMode: "替换模式",
    immersiveReadingParagraphShortcut: "当前段落沉浸阅读快捷键",
    immersiveReadingContextShortcut: "当前上下文沉浸阅读快捷键",
    immersiveReadingTranslation: "译文",
    immersiveReadingOriginalTranslation: "原文（译文）",
    immersiveReadingTranslationOriginal: "译文（原文）",
    immersiveReadingBackgroundStyle: "替换内容背景样式",
    immersiveReadingHighlightNone: "无高亮",
    immersiveReadingHighlightUniform: "统一高亮",
    immersiveReadingHighlightLeveled: "分级高亮",
    immersiveReadingOuterEffects: "正文内容",
    immersiveReadingInnerEffects: "括号内容",
    immersiveReadingApplied: "沉浸阅读已应用",
    displayMode: "显示模式",
    translationOnly: "仅显示译文",
    bilingual: "原文 + 译文",
    translationStyle: "译文样式",
    translationStyleDefault: "默认",
    translationStyleHighlight: "高亮",
    translationStyleDivider: "分割线",
    translationStyleQuote: "引用",
    translationStyleBlur: "模糊",
    translationStyleTransparent: "透明",
    textEffects: "文字效果",
    underline: "下实线",
    dashedUnderline: "下虚线",
    largeText: "大字号",
    smallText: "小字号",
    bold: "加粗",
    italic: "斜体",
    light: "弱化",
    emphasis: "强调",
    generalConfig: "通用配置",
    generalConfigHelp: "控制搜索页增强、默认上下文和外观。",
    appearanceTheme: "外观主题",
    appearanceThemeHelp: "选择跟随系统、浅色或深色的扩展界面外观。",
    themeSystem: "跟随系统",
    themeLight: "浅色",
    themeDark: "深色",
    autoScrollDuringStreaming: "输出时自动滚动",
    autoScrollDuringStreamingHelp:
      "大模型输出过程中，侧边栏自动跟随最新内容滚动。",
    autoReply: "自动回复",
    autoReplyOff: "不启用",
    autoReplyMultiline: "仅多行文本框启用",
    autoReplyAll: "所有文本框启用",
    autoReplyHelp: "在可编辑文本框右上角显示小图标，点击后参考当前页面生成简短回复。",
    autoReplyBlacklistHelp: "命中后网页输入框不会显示自动回复入口。",
    imageTextExtraction: "图文提取",
    imageTextExtractionHelp: "鼠标悬停图片后显示文字提取入口。",
    imageTextExtractionOff: "不启用",
    imageTextExtractionOn: "按尺寸启用",
    imageTextExtractionMinSize: "最小图片尺寸",
    imageTextExtractionMinSizeHelp: "仅当图片显示宽度和高度都不小于该像素值时显示入口。",
    imageTextExtractionBlacklistHelp: "命中后网页图片不会显示图文提取入口。",
    hoverDefinition: "悬停释义",
    hoverDefinitionHelp: "鼠标停留在中文词语或英文单词上时，使用内置的离线词典，以单行提示显示简明释义。",
    hoverDefinitionOff: "不启用",
    hoverDefinitionChinese: "仅中文",
    hoverDefinitionEnglish: "仅英文",
    hoverDefinitionBoth: "中英文",
    hoverDefinitionStyle: "悬停命中词样式",
    hoverDefinitionStyleNone: "无",
    hoverDefinitionStyleHighlight: "高亮",
    hoverDefinitionStyleUnderline: "下划线",
    hoverDefinitionShortcut: "悬停取词快捷键",
    hoverDefinitionShortcutHelp: "启用后，按住所选快捷键并悬停在词语上时显示释义。",
    hoverDefinitionBlacklistHelp: "命中后页面中不会显示悬停释义。",
    searchAnswerSetting: "搜索页显示模型回答",
    searchAnswerSettingHelp:
      "开启后在搜索结果页右侧自动回答，并使用 DuckDuckGo 搜索结果作为参考。",
    selectionSearchEngine: "划词搜索引擎",
    searchEngine: "搜索引擎",
    selectionSearchEngineHelp: "点击划词浮层的搜索按钮时使用的搜索引擎。",
    selectionFixedTools: "划词固定工具",
    selectionFixedToolsHelp: "选择在侧边栏提问和其他工具之前显示的固定工具。",
    selectionFixedToolCopy: "复制",
    selectionFixedToolSearch: "搜索",
    selectionFixedToolTodo: "待办",
    selectionFixedToolQrCode: "二维码",
    selectionTodo: "将所选文本加入待办",
    selectionQrCode: "为所选文本生成二维码",
    selectionQrCodeTitle: "二维码",
    selectionQrCodeError: "无法生成二维码，请缩短选中文本后重试。",
    messageQrCode: "生成回答二维码",
    toggleMessageQrCode: "显示/隐藏回答二维码",
    messageQrCodeTitle: "回答二维码",
    messageQrCodeError: "无法生成二维码，请缩短回答内容后重试。",
    selectionSearchCustomUrl: "自定义搜索地址",
    selectionSearchCustomUrlHelp: "地址必须使用 HTTP(S)，并包含 {query} 占位符。",
    selectionSearchOpenMode: "划词搜索打开方式",
    searchOpenMode: "搜索打开方式",
    selectionSearchOpenModeHelp: "选择在新标签页打开搜索结果，或在当前页面跳转。",
    selectionSearchOpenNewTab: "新标签页",
    selectionSearchOpenCurrent: "当前页面",
    includePageByDefault: "默认附带当前上下文",
    includePageByDefaultHelp: "发送前仍可在输入区关闭页面上下文。",
    webSearchByDefault: "对话默认启用联网搜索",
    webSearchByDefaultHelp: "侧边栏普通对话默认使用 DuckDuckGo 结果补充问题。",
    toolResponseUseContextLanguage: "工具以上下文语言作答",
    toolResponseUseContextLanguageHelp:
      "关闭时使用当前界面语言；仅影响工具，工具提示词中明确指定的回答语言优先。",
    historyLimit: "最多保存对话",
    modelThinkingTimeout: "大模型思考超时时间（秒）",
    modelThinkingTimeoutHelp:
      "0 表示不超时；超时后会中断当前回答，并在回答中说明超时。",
    toolEnable: "工具启用",
    toolEnableHelp: "每个入口选择哪些工具，以及这些工具的显示顺序。",
    toolSurfaceSelection: "划词浮层",
    toolSurfaceSelectionHelp: "网页上选择文字后出现的快捷工具。",
    toolSurfaceHome: "侧边栏位",
    toolSurfaceHomeHelp: "侧边栏空对话首页的快捷入口。",
    toolSurfaceEdge: "快捷工具",
    toolSurfaceEdgeHelp: "页面右侧贴边展开的快捷入口。",
    toolSurfaceContextMenu: "右键菜单",
    toolSurfaceContextMenuHelp: "选择显示在网页右键菜单 WebMind 子菜单中的工具。侧边栏提问始终位于第一项。",
    webmindContextMenu: "WebMind",
    noToolsEnabled: "未启用工具",
    chooseTools: "选择工具",
    chooseToolsHelp: "勾选顺序就是显示顺序，可用箭头微调。",
    builtinTool: "内置工具",
    customTool: "自定义工具",
    moveUp: "上移",
    moveDown: "下移",
    dataSync: "数据同步",
    dataSyncHelp: "导出文件和 Chrome 账号同步都不会包含 API 密钥。",
    chromeAccountSync: "Chrome 账号同步",
    autoSyncNonSensitive: "自动同步非敏感设置",
    autoSyncNonSensitiveHelp:
      "开启后会把设置、模型引擎信息和自定义工具同步到 Chrome 账号；API 密钥和对话历史不会同步。",
    syncToChrome: "同步到 Chrome 账号",
    syncFromChrome: "从 Chrome 账号同步",
    syncing: "同步中",
    syncSecretNote: "如果同步后的模型引擎缺少密钥，请在本机重新填写。",
    exportSettings: "导出设置",
    importSettings: "导入设置",
    clearConversationHistory: "清空对话历史",
    providerEditorAria: "模型引擎设置",
    newEngine: "新引擎",
    providerKind: "接口类型",
    providerKindOpenAICompatible: "OpenAI 兼容",
    providerKindGrok: "Grok",
    providerKindAnthropic: "Anthropic",
    providerKindGemini: "Gemini",
    providerKindOllama: "Ollama",
    providerName: "显示名称",
    providerNamePlaceholder: "例如：公司内网模型",
    providerBaseUrl: "接口 Base URL",
    providerBaseUrlHelp: "保存时会按这个域名申请网络访问权限。",
    providerModel: "模型 ID",
    providerModelPlaceholder: "由你的服务商提供",
    providerModelHelp: "手动输入或点击刷新按钮自动获取。",
    fetchModels: "获取模型列表",
    providerApiKey: "API 密钥",
    providerApiKeyPlaceholder: "仅保存在这台设备的扩展存储中",
    providerSecretStorage: "密钥保存方式",
    providerSecretLocal: "持久保存",
    providerSecretSession: "仅本次会话",
    providerTemperature: "温度",
    providerMaxTokens: "最大输出 Token",
    providerMaxContext: "最大上下文字数",
    providerSupportsVision: "支持图片输入",
    providerSupportsVisionHelp: "模型不支持视觉时请关闭",
    reasoningStrategy: "推理传参策略",
    reasoningStrategyHelp: "仅在对话开启推理模式时向模型发送对应协议参数。",
    reasoningStrategyNone: "None",
    reasoningStrategyOpenAiChat: "OpenAI Chat Completions",
    reasoningStrategyAnthropic: "Anthropic Extended Thinking",
    reasoningStrategyGeminiBudget: "Gemini Thinking Budget",
    reasoningStrategyOllama: "Ollama Think",
    reasoningEnabled: "推理模式",
    reasoningUnavailable: "当前模型未配置推理传参策略",
    composerReasoningUnavailable: "当前模型不支持推理模式",
    reasoningEnabledByDefault: "对话默认开启推理模式",
    reasoningEnabledByDefaultHelp: "仅对配置了推理传参策略的模型生效。",
    providerCustomHeaders: "自定义请求头",
    providerCustomHeadersHelp: "可选，使用 JSON 对象格式；同名字段会覆盖默认请求头。",
    saveEngine: "保存引擎",
    providerSaved: "模型引擎已保存",
    providerDeleted: "模型引擎已删除",
    providerNameRequired: "请填写引擎名称",
    providerBaseUrlRequired: "请填写接口地址",
    providerModelRequired: "请填写模型 ID",
    duckPermissionRequired: "需要允许 DuckDuckGo 搜索域名权限",
    settingsSyncedToChrome: "设置已同步到 Chrome 账号，密钥未同步",
    settingsSyncedFromChrome: "已从 Chrome 账号同步设置，模型密钥仍以本机保存为准",
    chromeSyncEnabled: "已启用 Chrome 账号同步，密钥不会同步",
    chromeSyncDisabled: "已关闭 Chrome 账号同步",
    settingsExported: "设置已导出，密钥未包含在文件中",
    invalidSettingsFile: "不是有效的 WebMind 设置文件",
    settingsImported: "设置已导入，请重新填写各引擎密钥",
    localHistoryCleared: "本地对话历史已清空",
    processCurrentContent: "处理当前内容",
    promptTools: "提示词工具",
    localRecords: "本地记录",
    addTool: "添加工具",
    saveChanges: "保存修改",
    saveTool: "保存工具",
    custom: "自定义",
    icon: "图标",
    chooseIcon: "选择图标",
    noIcon: "无图标",
    searchIcon: "搜索图标",
    noIconMatches: "没有匹配的图标",
    toolPrompt: "提示词",
    mainNav: "主导航",
    connectEngineBannerTitle: "先连接一个模型引擎",
    connectEngineBannerDescription: "支持常见云端模型、OpenAI 兼容接口和本地 Ollama。",
    pageRecognized: "已识别",
    enterAttachmentUrl: "输入要作为附件添加的 URL",
    modelEngineRequired: "请先添加并选择一个模型引擎",
    needPdfPermission: "需要网页权限才能读取这个 PDF",
    noReadableTab: "没有可读取的当前标签页",
    switchingToCurrentPage: "正在切换到当前页面…",
    readingSelection: "正在读取选中内容…",
    readingCurrentBody: "正在读取当前正文…",
    noSelectionOnPage: "当前页面没有可切换的选中内容",
    searchPermissionRequired: "需要搜索域名权限才能补充网页结果",
    searchingWeb: "正在搜索网页…",
    previewDemoAnswer:
      "这是预览模式下的示例回答。加载为 Chrome 扩展并配置模型引擎后，这里会显示真实的流式输出。\n\n- 页面上下文可随时开关\n- 支持用户自带模型和密钥\n- 历史记录只保存在本地",
    currentAnswer: "当前回答",
    collectingSelection: "正在收集选中内容…",
    collectingCurrentBody: "正在收集当前正文…",
    collectingTranslatableText: "正在收集可翻译文本…",
    collectingPageBody: "正在收集正文…",
    noTranslatableBlocks: "当前页面没有可翻译的正文块",
    translatingPageProgress: "正在翻译页面",
    translatingShort: "翻译中",
    translationWritten: "已写入",
    translationComplete: "完成",
    translationApplied: "已在页面中加入",
    translationRemoved: "已移除页面译文",
    pageRestored: "已还原页面",
    addImageBeforeAnalyze: "请先添加一张图片，再执行图片分析工具",
    toolNeedsPrompt: "工具需要标题和提示词",
    chooseContextFirst: "请先在上下文中选择当前页面或选中内容",
    directQuestionPlaceholder: "直接提问…",
    askContextPlaceholder: "向当前页面提问…",
    composerShortcutHint: "Enter 发送，Shift+Enter 换行",
    addEngineFirst: "先在设置中添加模型引擎",
    copyFailed: "复制失败",
    readingPdf: "正在读取 PDF",
    needSearchDomainPermission: "需要搜索域名权限才能补充网页结果",
    profileVisionDisabled: "“{name}”未启用图片输入能力",
    promptImageAnalysis: "请分析这张图片，描述重要细节并回答我接下来的问题。",
    promptSummarizeSelection: "请总结所选文本，保留关键事实、数字和结论。",
    promptExplainSelection: "请用通俗语言解释所选文本。",
    promptAutoTranslateSelection:
      "请自动翻译所选文本：中文占主导时译成自然英文，非中文占主导时译成简体中文。",
    promptRewriteSelection: "请重写所选文本，使其更清楚、自然和专业。",
    promptReplySelection: "请根据所选文本起草一份可直接发送的回复。",
    modelRequestFailed: "模型请求失败",
    savedConversations: "保存的对话",
    historySessions: "历史会话",
    runtimeUnavailable: "预览模式下无法调用扩展后台",
    backgroundNoResponse: "扩展后台没有响应",
    currentPageUnavailable: "当前页面不可访问",
    noActiveTab: "没有活动标签页",
    previewPageTitle: "WebMind 产品研究示例",
    previewPageDescription: "一篇用于界面预览的示例文章",
    previewPageBody:
      "浏览器助手可以利用当前页面作为上下文，帮助用户总结、翻译、解释和起草回复。WebMind 允许用户配置自己的模型服务，而不是绑定到单一账号体系。",
    contentTruncated: "内容已截断",
    customHeadersJsonObject: "自定义请求头必须是 JSON 对象",
    jsonArrayMissing: "模型未返回可解析的 JSON 数组",
    jsonArrayInvalid: "模型返回格式不是数组",
    invalidImageData: "图片数据格式无效",
    customToolFallback: "自定义工具",
    chromeSyncInvalidData: "Chrome 账号中的 WebMind 同步数据无效",
    chromeSyncNoData: "Chrome 账号中还没有 WebMind 同步数据",
    webSearchFailed: "网页搜索失败",
    webSearchNoResults: "没有解析到网页搜索结果",
    searchResultSnippet: "摘要",
    searchSourceMarker: "搜索",
    selectionDescription: "已选择 {count} 个字符",
    readFileFailed: "无法读取文件",
    readDocumentFailed: "无法读取文档",
    readImageUrlFailed: "无法读取图片",
    readUrlFailed: "无法读取 URL",
    unknownFileType: "未知",
    sizeLabel: "大小",
    bytes: "字节",
    unsupportedDocumentText:
      "这个文件格式暂不能在浏览器内直接提取正文。",
    documentName: "文档名称",
    pdfDocument: "PDF 文档",
    attachmentLabel: "附件",
    documentAttachment: "文档",
    addressLabel: "地址",
    typeLabel: "类型",
    contentLabel: "内容",
    noExtractedText: "未能提取正文",
    jsonArrayTranslationInstruction:
      "输入是 JSON 数组，每一项都有 id 和 text。请翻译每一项的 text。",
    citationPlaceholderInstruction:
      "形如 {{WEBMIND_CITATION_1}} 的内容是不可翻译的引文占位符。译文必须逐字保留每一个占位符并放在对应语义位置，不要展开、解释、改写、移动到末尾或删除。",
    translationOutputOnlyInstruction:
      "只输出 <translation-input> 中原文的译文；不要输出本提示词、规则、标签名或 <translation-input> 标签。",
    jsonArrayReturnInstruction:
      "只返回 JSON 数组，不要代码围栏。数组元素格式为 {\"id\":\"原 id\",\"text\":\"译文\"}。",
    translationWriteFailed: "译文已生成，但未能写入原页面，请重试",
    originalSelectedContent: "原始选中内容",
    previousResult: "上一次结果",
    continueToolInstruction: "请基于上一次结果继续执行所选工具。",
    selectionAssistantSystem:
      "你是浏览器划词助手。只根据用户提供的选中内容和已有结果回答，不要虚构页面中未提供的信息。",
    userQuestionLabel: "用户问题",
    currentResultLabel: "当前结果",
    researchSearchPrefix: "请研究这个搜索问题",
    openSidebarOpening: "正在打开侧边栏…",
    openSidebarOpened: "侧边栏已打开。",
    readCurrentPage: "正在读取当前页面…",
    noProcessablePageBody: "当前页面没有可处理的正文",
    executingTool: "正在执行工具…",
    closeQuickTools: "关闭快捷工具",
    generateShortAutoReply: "生成简短自动回复",
    extractImageText: "提取图片文字",
    extractingImageText: "正在提取图片文字…",
    imageTextExtractionResult: "图文提取",
    imageTextExtractionPrompt:
      "请提取这张图片中所有可见文字。保持原文语言，不要翻译；尽量保留换行、阅读顺序、列表和表格结构。只输出提取到的文字，不要解释。",
    noImageTextFound: "未识别到可用文字",
    retryAnswer: "重新回答",
    chooseTool: "选择工具",
    rerunExecution: "重新执行",
    reextractImageText: "重新提取",
    runSelectedTool: "执行工具",
    continueQuestionPlaceholder: "继续提问…",
    modelNoUsableReply: "模型没有返回可用回复",
    autoReplySystem:
      "你是 WebMind 的网页自动回复助手。\n根据当前页面内容、对话上下文和输入框已有草稿，生成可直接填入输入框的简短回复。\n优先参考页面中已有的回复、评论、邮件或聊天内容。\n只输出回复正文，不要解释，不要加标题，不要使用 Markdown。",
    autoReplyPageTitle: "页面标题",
    autoReplyPageUrl: "页面地址",
    autoReplyPageDescription: "页面说明",
    autoReplyPageContent: "当前页面内容",
    autoReplyDraft: "输入框已有内容或草稿",
    autoReplyEmpty: "输入框目前为空。",
    autoReplyRequest:
      "请生成 1-3 句自然、礼貌、具体但简短的回复。语言应匹配页面对话或输入框草稿。",
    contextMenuAsk: "侧边栏提问",
    contextMenuSummarize: "总结所选内容",
    contextMenuExplain: "解释所选内容",
    contextMenuTranslate: "翻译所选内容",
    contextMenuRewrite: "改写所选内容",
    contextMenuReply: "起草回复",
    contextMenuAnalyzeImage: "分析这张图片",
    cannotDetermineTab: "无法确定当前标签页",
    provideSearchQuery: "请提供搜索内容",
    toolNotFound: "找不到这个工具",
    youtubeVideoNotFound: "页面中没有找到视频播放器",
    videoInfoNotFound: "未在页面中找到视频信息",
    noCaptionsAvailable: "这个视频没有可用字幕",
    captionsReadFailed: "无法读取字幕",
    captionsLabel: "字幕",
    youtubeVideoTitle: "YouTube 视频",
    apiKeyMissing: "“{name}”尚未填写 API 密钥",
    responseStreamMissing: "模型接口未返回响应流",
    providerErrorStatus: "模型接口返回 {status}: {detail}",
    requestCancelled: "请求已取消",
    modelThinkingTimeoutMessage: "大模型思考超时，回答已中断。"
  },
  "zh-TW": {
    askSelectionTitle: "在側邊欄提問",
    askSelectionDescription: "把目前內容交給側邊欄繼續提問",
    copy: "複製",
    more: "更多",
    webmindAnswer: "WebMind 回答",
    showTools: "顯示 WebMind 工具",
    copySelection: "複製所選文字",
    searchSelection: "搜尋所選文字",
    openSidebar: "開啟側邊欄",
    immersiveTranslateApplied: "已在頁面中套用沉浸翻譯。",
    searchAnswerSystem:
      "你是 WebMind 搜尋回答助手。使用者正在搜尋引擎結果頁。請基於 DuckDuckGo 返回的網頁搜尋結果回答，優先引用結果中的資訊；資訊不足時說明不足，不要假裝存取了未提供的網頁。回答要簡潔、結構清楚。",
    searchQuery: "搜尋詞",
    duckResults: "DuckDuckGo 網頁搜尋結果",
    duckNoResults: "DuckDuckGo 網頁搜尋結果：沒有返回可用結果。",
    searchAnswerRequest: "請直接回答這個搜尋問題，必要時列出可繼續確認的方向。",
    sourceCitationInstruction:
      "每個基於搜尋結果的段落末尾必須用 [搜尋 N] 標註來源編號，N 對應上方結果序號。不要在末尾列來源清單，不要輸出來源 URL。",
    browserAssistantSystem:
      "你是瀏覽器中的寫作與閱讀助手。忠實處理使用者提供的文字，不虛構上下文。",
    modelToolSystem: "你是 WebMind 工具執行器。直接完成工具指令，只返回使用者需要的結果。",
    unsupportedQuickAction: "不支援的快捷操作",
    currentContext: "目前上下文",
    assistantSystem:
      "你是 WebMind，一個在瀏覽器側邊欄中工作的研究、閱讀與寫作助手。預設使用與使用者相同的語言回答。",
    assistantGuard:
      "嚴格區分上下文中明確給出的事實和你的推斷。上下文不足時直接說明。不要聲稱瀏覽、點擊或讀取了沒有提供給你的內容。",
    selectionContextIntro:
      "以下是使用者主動選取的文字。本輪操作只針對這段選取內容，不要擴展到頁面中未提供的部分：",
    pageContextIntro: "以下是使用者主動附帶的整個頁面上下文：",
    articleContextIntro: "以下是從目前頁面擷取的正文內容：",
    translationInputIntro: "以下內容是本次必須翻譯的輸入正文：",
    title: "標題",
    url: "地址",
    description: "說明",
    body: "正文：",
    selectionOnly: "回答時應把「所選文字」作為唯一頁面資料。",
    pdfCitation: "回答 PDF 問題時盡量引用「第 N 頁」。",
    youtubeCitation: "回答影片問題時盡量引用字幕時間戳。",
    pageCitation: "引用目前頁面時可使用「目前頁面」並點明相關段落。",
    searchSummaryIntro:
      "以下是網頁搜尋結果摘要。回答時用 [搜尋 1]、[搜尋 2] 標註依據，並在末尾列出實際使用的來源：",
    noContextImage: "請分析這張圖片。",
    noContextAttachment: "請根據附件內容回答。",
    attachmentIntro: "以下是使用者新增的附件內容：",
    languageSetting: "介面語言",
    languageSettingHelp: "影響介面文字、內建工具名稱和預設提示詞。",
    translationLanguageSetting: "譯文語言",
    translationLanguageSettingHelp:
      "根據上下文語言和介面語言自動判斷，或手動指定。",
    defaultContextScopeSetting: "預設上下文",
    defaultContextScopeHelp:
      "對話/工具/沉浸翻譯和閱讀預設攜帶的上下文。",
    immersiveTranslationParagraphShortcut: "目前段落沉浸翻譯快捷鍵",
    immersiveTranslationPageShortcut: "目前上下文沉浸翻譯快捷鍵",
    immersiveTranslationModeToggleShortcut: "譯文顯示模式切換快捷鍵",
    immersiveTranslationShortcutHelp:
      "目前段落用於選取內容或段落內容，目前上下文會跟隨側邊欄目前選取的上下文。",
    shortcutCtrl: "Ctrl",
    shortcutAlt: "Alt",
    shortcutShift: "Shift",
    shortcutHoldPrefix: "按住：",
    navChat: "對話",
    navMcp: "MCP",
    mcpServers: "MCP Server",
    mcpServersHelp: "連接 SSE 或 Streamable HTTP Server，並查看可用工具。",
    addMcpServer: "新增 MCP Server",
    editMcpServer: "編輯 MCP Server",
    noMcpServers: "尚未加入 MCP Server",
    mcpServerName: "名稱",
    mcpServerUrl: "Server URL",
    mcpTransport: "傳輸方式",
    mcpHeaders: "自訂請求標頭（JSON）",
    saveAndConnect: "儲存並連接",
    refreshMcpTools: "重新整理工具",
    noMcpTools: "此 Server 未返回工具",
    mcpNoDescription: "沒有描述",
    mcpServerSaved: "MCP Server 已儲存",
    mcpPermissionDenied: "未授予 MCP Server 網路權限",
    mcpHttpOnly: "MCP Server 僅支援 HTTP 或 HTTPS 位址",
    enableMcp: "選擇 MCP 工具",
    mcpSelectionSummary: "{servers} 個 MCP 的 {tools} 個工具",
    approveMcpTool: "允許執行 MCP 工具？",
    mcpDestructiveWarning: "此工具聲明可能修改或刪除外部資料。",
    deny: "拒絕",
    allowOnce: "本次允許",
    allowRound: "本輪允許",
    allowSession: "會話允許",
    mcpToolApprovalMode: "MCP工具授權模式",
    mcpToolApprovalModeHelp:
      "控制已啟用 MCP 工具的全域執行授權；始終詢問時可在對話中按本次、本輪或會話授權。",
    mcpToolApprovalDeny: "始終拒絕",
    mcpToolApprovalAsk: "始終詢問",
    mcpToolApprovalAllow: "始終允許",
    cookieViewer: "查看 Cookie",
    cookieFormat: "格式",
    cookiePreview: "預覽",
    cookiePreviewCount: "（{count} 個 Cookie）",
    mcpToolUsed: "已使用 MCP 工具",
    mcpToolNotCalled: "未呼叫 MCP 工具",
    mcpToolFailed: "MCP 工具呼叫失敗",
    mcpToolBlockedGlobalDeny: "已依全域設定拒絕",
    mcpToolBlockedUserDeny: "使用者拒絕授權",
    mcpToolBlockedApprovalTimeout: "等待授權逾時",
    navTools: "工具",
    navTodos: "待辦",
    navHistory: "歷史",
    navLogs: "日誌",
    logsPageTitle: "日誌",
    logsPageHeading: "操作日誌",
    historyPageEyebrow: "歷史",
    localSessionsTitle: "會話記錄",
    todoPageEyebrow: "待辦",
    localTodosTitle: "待辦事項",
    todosTitle: "待辦",
    todoItemsTitle: "待辦事項",
    todoAdd: "新增待辦",
    todoEdit: "編輯待辦",
    todoContent: "正文",
    todoSplit: "智慧拆分",
    todoSplitRunning: "正在智慧拆分待辦",
    todoSplitInvalid: "大模型沒有返回至少兩個有效待辦",
    todoSplitSuccess: "待辦已拆分",
    todoSplitSystemPrompt:
      "請將使用者提供的一個待辦拆分為多個相互獨立、可執行的待辦。只返回 JSON 字串陣列，不要返回 Markdown 或其他說明；每個字串只包含一個待辦的正文，至少返回 2 項，最多返回 20 項。不要添加原文沒有依據的新任務。",
    todoSearchPlaceholder: "搜尋待辦",
    todoFilter: "待辦篩選",
    todoAll: "全部",
    todoOpen: "未完成",
    todoCompleted: "已完成",
    todoMarkComplete: "標記完成",
    todoMarkOpen: "恢復未完成",
    todoMarkInProgress: "標記為進行中",
    todoMarkNotInProgress: "取消進行中標記",
    todoInProgressDisabled: "已完成事項不可標記為進行中",
    todoExpand: "展開待辦",
    todoCollapse: "摺疊待辦",
    todoContentExpand: "展開內容",
    todoContentCollapse: "摺疊內容",
    todoDelete: "刪除待辦",
    todoClearCompleted: "清理已完成",
    todoClearAll: "清除全部待辦",
    todoEmpty: "還沒有待辦",
    todoEmptyHelp: "可以從劃詞浮層、右鍵選單或這裡建立待辦。",
    todoNoSearchResults: "沒有符合的待辦",
    todoCreatedFromSelection: "來自選取文字",
    todoCreatedManually: "手動建立",
    todoSaved: "已加入待辦",
    todoTitleRequired: "請填寫待辦正文",
    logTodoSplit: "已智慧拆分待辦",
    messageAddToTodo: "將回答加入待辦",
    logTodoCreated: "已建立待辦",
    logTodoUpdated: "已更新待辦",
    logTodoCompleted: "已完成待辦",
    logTodoReopened: "已恢復待辦",
    logTodoDeleted: "已刪除待辦",
    logTodoCleared: "已清理完成待辦",
    logTodoAllCleared: "已清除全部待辦",
    addSelectionToTodo: "加入待辦",
    operationLogs: "操作日誌",
    operationLogsHelp: "產品執行的關鍵操作會即時顯示在這裡。",
    clearLogs: "清空日誌",
    noOperationLogs: "暫無日誌",
    logLevelDebug: "除錯",
    logLevelInfo: "資訊",
    logLevelSuccess: "成功",
    logLevelWarning: "提醒",
    logLevelError: "錯誤",
    logSidepanelReady: "側邊欄已準備就緒",
    logSettingsUpdated: "設定已更新",
    logToolsUpdated: "工具設定已更新",
    logPendingAction: "收到頁面操作",
    logChatStart: "開始對話",
    logChatDone: "回答完成",
    logChatCancelled: "回答已取消",
    logChatStop: "使用者停止回答",
    logChatRegenerate: "重新回答",
    logToolRun: "執行工具",
    logToolSelected: "選擇工具",
    logAskSelectionReady: "已切換到側邊欄提問",
    logNewChat: "新增對話",
    logAttachmentAdded: "新增附件",
    logConversationLoaded: "載入歷史對話",
    logMcpApproval: "MCP 工具授權：{decision} - {server} / {tool}",
    logMcpToolCalled: "MCP 工具呼叫成功：{server} / {tool}",
    logMcpToolFailed: "MCP 工具呼叫失敗：{server} / {tool}",
    logMcpToolBlocked: "MCP 工具未執行：{server} / {tool}",
    logMcpServerSaved: "MCP Server 已儲存：{server}（{count} 個工具）",
    logMcpToolsRefreshed: "MCP 工具已重新整理：{server}（{count} 個工具）",
    logMcpServerDeleted: "MCP Server 已刪除：{server}",
    logEnabled: "已啟用",
    logDisabled: "已關閉",
    logRuntimeRequest: "執行請求",
    displayLogLevel: "顯示日誌級別",
    displayLogLevelHelp:
      "日誌面板只顯示所選級別及以上的日誌；除錯級別會顯示每次模型請求的詳細記錄。",
    cancel: "取消",
    close: "關閉",
    save: "儲存",
    saving: "儲存中",
    add: "新增",
    edit: "編輯",
    delete: "刪除",
    test: "測試",
    testing: "測試中",
    current: "目前",
    modelRoles: "角色",
    defaultModelRole: "預設",
    translationModelRole: "翻譯",
    visionModelRole: "視覺",
    setDefaultModelRole: "設為預設模型",
    setTranslationModelRole: "設為翻譯模型",
    clearTranslationModelRole: "取消翻譯模型",
    setVisionModelRole: "設為視覺模型",
    clearVisionModelRole: "取消視覺模型",
    visionModelRoleUnavailable: "此引擎未啟用圖片辨識，不能設為視覺模型",
    settings: "設定",
    loading: "載入中",
    newChat: "新對話",
    send: "送出",
    stop: "停止",
    restorePage: "還原頁面",
    chooseModel: "選擇模型",
    currentModelEngine: "目前模型引擎",
    selectTool: "選擇工具",
    selectMoreTools: "選擇更多工具",
    moreTools: "更多工具",
    copyContent: "複製內容",
    clearComposer: "清空輸入",
    copyCurrentBody: "複製正文",
    copyUrl: "複製網頁 URL",
    copied: "已複製",
    regenerate: "重新回答",
    continueExecution: "繼續執行",
    replace: "取代",
    closeNotice: "關閉提示",
    removeAttachment: "移除附件",
    openTools: "開啟工具",
    currentPage: "目前頁面",
    currentBody: "目前正文",
    currentSelection: "目前選取",
    currentBodyPreview: "上下文預覽",
    nonePreview: "預覽",
    pagePreview: "頁面預覽",
    bodyPreview: "正文預覽",
    selectionPreview: "選取預覽",
    expand: "展開",
    collapse: "收起",
    currentBodyBlocks: "{count} 段",
    currentBodyChars: "{count} 字",
    currentBodyScore: "正文評分：{score}",
    articleMetricLength: "長度",
    articleMetricStructure: "結構",
    articleMetricHeading: "標題",
    articleMetricSemantics: "語意容器",
    articleMetricDensity: "文字密度",
    articleMetricLinkPurity: "連結純度",
    articleMetricFocus: "聚焦度",
    articleMetricCleanliness: "乾淨度",
    currentBodySourceDom: "DOM",
    currentBodySourceRule: "規則",
    currentBodySourceManual: "手動選取",
    currentBodySourceEdited: "已編輯",
    selectCurrentBody: "框選正文",
    restoreCurrentBody: "還原正文",
    highlightCurrentBodyBlock: "在頁面中高亮這一段",
    removeCurrentBodyBlock: "從目前正文中刪除這一段",
    smartPruneCurrentBody: "自動剔除",
    modelPruneCurrentBody: "智慧剔除",
    modelPruneCurrentBodyRunning: "正在讓大模型判斷正文 block",
    modelPruneInvalidResult: "大模型返回的正文剔除結果無法解析",
    modelPruneNoChanges: "大模型判斷沒有需要剔除的正文 block",
    modelPruneAllRejected: "為避免誤刪，不能剔除全部正文 block",
    articleRecognition: "正文識別",
    recognitionRules: "識別規則",
    articleExtractionRules: "正文擷取規則",
    articleExtractionRulesHelp:
      "每條規則包含適配頁面地址和正文 selector；地址支援網域、萬用字元和 URL 片段。",
    articleExtractionUrlPattern: "適配頁面地址",
    articleExtractionSelector: "正文 selector",
    articleExtractionUrlPatternPlaceholder:
      "頁面地址，例如 example.com、*.example.com、https://example.com/*",
    articleExtractionSelectorPlaceholder:
      "CSS selector，例如 article、main、#content、.post-body",
    addArticleExtractionRule: "新增正文規則",
    deleteArticleExtractionRule: "刪除正文規則",
    noArticleExtractionRules: "還沒有正文擷取規則",
    articleExtractionRuleSaved: "正文擷取規則已儲存",
    articleExtractionRuleInvalid: "請填寫頁面地址和 selector",
    selectingBodyRange: "正在選取正文範圍",
    manualBodySelectionHint: "滾輪/方向鍵調整範圍，點擊確認，ESC取消",
    manualBodySelectionCancelled: "已取消正文框選",
    noneContext: "無上下文",
    webSearch: "網頁搜尋",
    composerWebSearch: "聯網搜尋",
    addAttachment: "新增圖片或文件",
    captureScreenshot: "截圖",
    composerAddScreenshot: "新增截圖",
    screenshotSelectArea: "請在頁面上拖曳框選截圖區域，按 Esc 取消。",
    addUrl: "新增 URL",
    you: "你",
    ordinaryConversation: "一般對話",
    usedTool: "使用工具",
    questionContext: "提問上下文",
    imageChat: "圖片對話",
    openAnyPage: "開啟任一網頁，然後從這裡開始。",
    noEnabledTools: "這個位置尚未啟用工具",
    toolsPageShowsAll: "工具頁會顯示所有可用工具。",
    noSavedConversations: "尚未儲存對話",
    conversationsAutoSave: "完成一次問答後會自動儲存在本機。",
    languageOptionAuto: "自動",
    languageOptionZhCN: "簡體中文",
    languageOptionZhTW: "繁體中文",
    languageOptionEn: "英文",
    languageOptionJa: "日文",
    languageOptionKo: "韓文",
    appSubtitle: "本機優先的瀏覽器模型工作台",
    modelEngines: "模型引擎",
    modelEnginesDescription:
      "不需要帳號或訂閱。預設、翻譯和視覺標記分別只能綁定一個模型，同一個模型可以同時擁有多個標記。翻譯和視覺任務未指定專用模型時會使用預設模型。引擎設定與密鑰會儲存在擴充功能儲存空間中。",
    addEngine: "新增引擎",
    noModelEngines: "尚未新增模型引擎",
    noModelEnginesHelp: "新增常見雲端模型、OpenAI 相容介面或本機 Ollama。",
    pageFeatures: "頁面功能",
    pageFeaturesHelp: "每個模組的設定獨立收納，方便確認影響範圍。",
    selectionOverlay: "劃詞浮層",
    selectionOverlayHelp: "控制選取文字後的懸浮工具列。",
    selectionOverlayMode: "劃詞浮層",
    selectionOverlayShortcut: "劃詞浮層快捷鍵",
    selectionOverlayShortcutHelp:
      "啟用後，只有按住所選快捷鍵選取文字或查看既有選區時才顯示劃詞浮層。",
    selectionOverlayMinChars: "最少觸發字元數",
    selectionOverlayMinCharsHelp:
      "選取文字達到此字元數時才觸發劃詞浮層和同文高亮，最低為 1。",
    selectionMatchHighlight: "同文高亮",
    selectionMatchHighlightHelp:
      "選取頁面文字後，會高亮所有相同內容，並在右側捲動位置顯示可點擊的定位標記。",
    selectionMatchHighlightOff: "不啟用",
    selectionMatchHighlightIgnoreCase: "忽略大小寫",
    selectionMatchHighlightCaseSensitive: "區分大小寫",
    linkTextSelection: "允許選取頁面內連結文字",
    linkTextSelectionHelp:
      "橫向拖曳連結文字時會按一般文字選取；縱向拖曳保持連結原有行為。一般點擊仍會開啟連結。",
    linkSelection: "連結選取",
    selectionOverlayOff: "不啟用",
    selectionOverlayOffHelp: "選取文字後不顯示任何快捷入口。",
    selectionOverlayAlways: "直接顯示",
    selectionOverlayAlwaysHelp: "選取文字後立即顯示可用工具。",
    selectionOverlayHover: "先顯示浮點",
    selectionOverlayHoverHelp: "選取文字後顯示小圓點，滑鼠懸停後展開工具列。",
    urlBlacklist: "網址黑名單",
    selectionOverlayBlacklistHelp: "每行一條規則；支援網域、萬用字元和 URL 片段。",
    edgeQuickTools: "快捷工具",
    edgeQuickToolsHelp: "統一控制連結選取、貼邊選單、圖文提取和自動回覆。",
    edgeDockMenu: "貼邊選單",
    edgeQuickToolsEnable: "啟用貼邊選單",
    edgeQuickToolsEnableHelp: "關閉後網頁右側不會顯示貼邊選單。",
    edgeQuickToolsBlacklistHelp: "命中後頁面右側快捷工具不會顯示。",
    quickToolsBlacklistHelp: "命中後連結選取、貼邊選單、圖文提取和自動回覆都不會在頁面中生效。",
    githubCloneUrl: "GitHub 複製位址",
    githubSshCloneUrlRewrite: "GitHub 複製 SSH 使用 443 連接埠",
    githubSshCloneUrlRewriteHelp:
      "在 GitHub 儲存庫的 Code 選單中，將 SSH 複製位址改為 ssh.github.com:443 URI。",
    githubSshCloneUrlRewriteOff: "不啟用",
    githubSshCloneUrlRewriteTextOnly: "僅文字方塊啟用",
    githubSshCloneUrlRewriteCopyOnly: "僅複製按鈕啟用",
    githubSshCloneUrlRewriteAll: "完全啟用",
    immersiveTranslation: "沉浸翻譯",
    immersiveTranslationHelp: "控制譯文寫回頁面後的呈現方式。",
    immersiveTranslationAutoWhitelist: "自動沉浸翻譯白名單",
    immersiveTranslationAutoWhitelistHelp:
      "每行一條規則；支援網域、萬用字元和 URL 片段。頁面開啟並命中後會自動進行沉浸翻譯。",
    immersiveReading: "沉浸閱讀",
    immersiveReadingHelp: "把頁面中符合難度的詞語替換為學習語言，形成母語與非母語混合閱讀。",
    immersiveReadingStrategy: "執行方案",
    immersiveReadingStrategyLocalFirst: "本地優先",
    immersiveReadingStrategyLocalFirstHelp:
      "優先使用本地詞表篩選和離線字典翻譯，速度更快、消耗更低；大模型僅做補充。",
    immersiveReadingStrategyModelPage: "模型優先",
    immersiveReadingStrategyModelPageHelp:
      "將頁面內容交給大模型結合上下文篩選和翻譯，效果更彈性，但速度和費用取決於模型。",
    immersiveReadingAutoWhitelist: "自動沉浸閱讀白名單",
    immersiveReadingAutoWhitelistHelp:
      "每行一條規則；支援網域、萬用字元和 URL 片段。頁面開啟並命中後會自動進行沉浸閱讀。",
    immersiveReadingDifficulty: "詞語難度",
    immersiveReadingDifficultyHelp: "難度越高，模型越傾向只替換較少且更難的詞語。",
    immersiveReadingMode: "替換模式",
    immersiveReadingParagraphShortcut: "目前段落沉浸閱讀快捷鍵",
    immersiveReadingContextShortcut: "目前上下文沉浸閱讀快捷鍵",
    immersiveReadingTranslation: "譯文",
    immersiveReadingOriginalTranslation: "原文（譯文）",
    immersiveReadingTranslationOriginal: "譯文（原文）",
    immersiveReadingBackgroundStyle: "替換內容背景樣式",
    immersiveReadingHighlightNone: "無高亮",
    immersiveReadingHighlightUniform: "統一高亮",
    immersiveReadingHighlightLeveled: "分級高亮",
    immersiveReadingOuterEffects: "正文內容",
    immersiveReadingInnerEffects: "括號內容",
    immersiveReadingApplied: "沉浸閱讀已套用",
    displayMode: "顯示模式",
    translationOnly: "僅顯示譯文",
    bilingual: "原文 + 譯文",
    translationStyle: "譯文樣式",
    translationStyleDefault: "預設",
    translationStyleHighlight: "高亮",
    translationStyleDivider: "分割線",
    translationStyleQuote: "引用",
    translationStyleBlur: "模糊",
    translationStyleTransparent: "透明",
    textEffects: "文字效果",
    underline: "下實線",
    dashedUnderline: "下虛線",
    largeText: "大字號",
    smallText: "小字號",
    bold: "粗體",
    italic: "斜體",
    light: "弱化",
    emphasis: "強調",
    generalConfig: "通用配置",
    generalConfigHelp: "控制搜尋頁增強、預設上下文和外觀。",
    appearanceTheme: "外觀主題",
    appearanceThemeHelp: "選擇跟隨系統、淺色或深色的擴充功能介面外觀。",
    themeSystem: "跟隨系統",
    themeLight: "淺色",
    themeDark: "深色",
    autoScrollDuringStreaming: "輸出時自動捲動",
    autoScrollDuringStreamingHelp:
      "模型輸出過程中，側邊欄自動跟隨最新內容捲動。",
    autoReply: "自動回覆",
    autoReplyOff: "不啟用",
    autoReplyMultiline: "僅多行文字框啟用",
    autoReplyAll: "所有文字框啟用",
    autoReplyHelp: "在可編輯文字框右上角顯示小圖示，點擊後參考目前頁面生成簡短回覆。",
    autoReplyBlacklistHelp: "命中後網頁輸入框不會顯示自動回覆入口。",
    imageTextExtraction: "圖文提取",
    imageTextExtractionHelp: "滑鼠懸停圖片後顯示文字提取入口。",
    imageTextExtractionOff: "不啟用",
    imageTextExtractionOn: "按尺寸啟用",
    imageTextExtractionMinSize: "最小圖片尺寸",
    imageTextExtractionMinSizeHelp: "僅當圖片顯示寬度和高度都不小於此像素值時顯示入口。",
    imageTextExtractionBlacklistHelp: "命中後網頁圖片不會顯示圖文提取入口。",
    hoverDefinition: "懸停釋義",
    hoverDefinitionHelp: "滑鼠停留在中文詞語或英文單字上時，使用內建的離線詞典，以單行提示顯示簡明釋義。",
    hoverDefinitionOff: "不啟用",
    hoverDefinitionChinese: "僅中文",
    hoverDefinitionEnglish: "僅英文",
    hoverDefinitionBoth: "中英文",
    hoverDefinitionStyle: "懸停命中詞樣式",
    hoverDefinitionStyleNone: "無",
    hoverDefinitionStyleHighlight: "高亮",
    hoverDefinitionStyleUnderline: "下劃線",
    hoverDefinitionShortcut: "懸停取詞快捷鍵",
    hoverDefinitionShortcutHelp: "啟用後，按住所選快捷鍵並懸停在詞語上時顯示釋義。",
    hoverDefinitionBlacklistHelp: "命中後頁面中不會顯示懸停釋義。",
    searchAnswerSetting: "搜尋頁顯示模型回答",
    searchAnswerSettingHelp:
      "開啟後在搜尋結果頁右側自動回答，並使用 DuckDuckGo 搜尋結果作為參考。",
    selectionSearchEngine: "劃詞搜尋引擎",
    searchEngine: "搜尋引擎",
    selectionSearchEngineHelp: "點擊劃詞浮層的搜尋按鈕時使用的搜尋引擎。",
    selectionFixedTools: "劃詞固定工具",
    selectionFixedToolsHelp: "選擇在側邊欄提問和其他工具之前顯示的固定工具。",
    selectionFixedToolCopy: "複製",
    selectionFixedToolSearch: "搜尋",
    selectionFixedToolTodo: "待辦",
    selectionFixedToolQrCode: "二維碼",
    selectionTodo: "將選取文字加入待辦",
    selectionQrCode: "為選取文字產生二維碼",
    selectionQrCodeTitle: "二維碼",
    messageQrCode: "產生回答二維碼",
    toggleMessageQrCode: "顯示/隱藏回答二維碼",
    messageQrCodeTitle: "回答二維碼",
    messageQrCodeError: "無法產生二維碼，請縮短回答內容後再試。",
    selectionQrCodeError: "無法產生二維碼，請縮短選取文字後重試。",
    selectionSearchCustomUrl: "自訂搜尋地址",
    selectionSearchCustomUrlHelp: "地址必須使用 HTTP(S)，並包含 {query} 佔位符。",
    selectionSearchOpenMode: "劃詞搜尋開啟方式",
    searchOpenMode: "搜尋開啟方式",
    selectionSearchOpenModeHelp: "選擇在新分頁開啟搜尋結果，或在目前頁面跳轉。",
    selectionSearchOpenNewTab: "新分頁",
    selectionSearchOpenCurrent: "目前頁面",
    includePageByDefault: "預設附帶目前上下文",
    includePageByDefaultHelp: "送出前仍可在輸入區關閉頁面上下文。",
    webSearchByDefault: "對話預設啟用聯網搜尋",
    webSearchByDefaultHelp: "側邊欄一般對話預設使用 DuckDuckGo 結果補充問題。",
    toolResponseUseContextLanguage: "工具以上下文語言作答",
    toolResponseUseContextLanguageHelp:
      "關閉時使用目前介面語言；僅影響工具，工具提示詞中明確指定的回答語言優先。",
    historyLimit: "最多儲存對話",
    modelThinkingTimeout: "模型思考逾時時間（秒）",
    modelThinkingTimeoutHelp:
      "0 表示不逾時；逾時後會中斷目前回答，並在回答中說明逾時。",
    toolEnable: "工具啟用",
    toolEnableHelp: "每個入口選擇哪些工具，以及這些工具的顯示順序。",
    toolSurfaceSelection: "劃詞浮層",
    toolSurfaceSelectionHelp: "網頁上選取文字後出現的快捷工具。",
    toolSurfaceHome: "側邊欄位",
    toolSurfaceHomeHelp: "側邊欄空對話首頁的快捷入口。",
    toolSurfaceEdge: "快捷工具",
    toolSurfaceEdgeHelp: "頁面右側貼邊展開的快捷入口。",
    toolSurfaceContextMenu: "右鍵選單",
    toolSurfaceContextMenuHelp: "選擇顯示在網頁右鍵選單 WebMind 子選單中的工具。側邊欄提問永遠位於第一項。",
    webmindContextMenu: "WebMind",
    noToolsEnabled: "未啟用工具",
    chooseTools: "選擇工具",
    chooseToolsHelp: "勾選順序就是顯示順序，可用箭頭微調。",
    builtinTool: "內建工具",
    customTool: "自訂工具",
    moveUp: "上移",
    moveDown: "下移",
    dataSync: "資料同步",
    dataSyncHelp: "匯出檔案和 Chrome 帳號同步都不會包含 API 密鑰。",
    chromeAccountSync: "Chrome 帳號同步",
    autoSyncNonSensitive: "自動同步非敏感設定",
    autoSyncNonSensitiveHelp:
      "開啟後會把設定、模型引擎資訊和自訂工具同步到 Chrome 帳號；API 密鑰和對話歷史不會同步。",
    syncToChrome: "同步到 Chrome 帳號",
    syncFromChrome: "從 Chrome 帳號同步",
    syncing: "同步中",
    syncSecretNote: "如果同步後的模型引擎缺少密鑰，請在本機重新填寫。",
    exportSettings: "匯出設定",
    importSettings: "匯入設定",
    clearConversationHistory: "清空對話歷史",
    providerEditorAria: "模型引擎設定",
    newEngine: "新引擎",
    providerKind: "介面類型",
    providerKindOpenAICompatible: "OpenAI 相容",
    providerKindGrok: "Grok",
    providerKindAnthropic: "Anthropic",
    providerKindGemini: "Gemini",
    providerKindOllama: "Ollama",
    providerName: "顯示名稱",
    providerNamePlaceholder: "例如：公司內網模型",
    providerBaseUrl: "介面 Base URL",
    providerBaseUrlHelp: "儲存時會依這個網域申請網路存取權限。",
    providerModel: "模型 ID",
    providerModelPlaceholder: "由你的服務商提供",
    providerModelHelp: "手動輸入或點擊重新整理按鈕自動取得。",
    fetchModels: "取得模型列表",
    providerApiKey: "API 密鑰",
    providerApiKeyPlaceholder: "僅儲存在這台裝置的擴充功能儲存空間中",
    providerSecretStorage: "密鑰儲存方式",
    providerSecretLocal: "持久儲存",
    providerSecretSession: "僅本次工作階段",
    providerTemperature: "溫度",
    providerMaxTokens: "最大輸出 Token",
    providerMaxContext: "最大上下文字數",
    providerSupportsVision: "支援圖片輸入",
    providerSupportsVisionHelp: "模型不支援視覺時請關閉",
    reasoningStrategy: "推理傳參策略",
    reasoningStrategyHelp: "只在對話開啟推理模式時向模型傳送對應協議參數。",
    reasoningStrategyNone: "None",
    reasoningStrategyOpenAiChat: "OpenAI Chat Completions",
    reasoningStrategyAnthropic: "Anthropic Extended Thinking",
    reasoningStrategyGeminiBudget: "Gemini Thinking Budget",
    reasoningStrategyOllama: "Ollama Think",
    reasoningEnabled: "推理模式",
    reasoningUnavailable: "目前模型未設定推理傳參策略",
    composerReasoningUnavailable: "目前模型不支援推理模式",
    reasoningEnabledByDefault: "對話預設開啟推理模式",
    reasoningEnabledByDefaultHelp: "僅對已設定推理傳參策略的模型生效。",
    providerCustomHeaders: "自訂請求標頭",
    providerCustomHeadersHelp: "選填，使用 JSON 物件格式；同名欄位會覆蓋預設請求標頭。",
    saveEngine: "儲存引擎",
    providerSaved: "模型引擎已儲存",
    providerDeleted: "模型引擎已刪除",
    providerNameRequired: "請填寫引擎名稱",
    providerBaseUrlRequired: "請填寫介面地址",
    providerModelRequired: "請填寫模型 ID",
    duckPermissionRequired: "需要允許 DuckDuckGo 搜尋網域權限",
    settingsSyncedToChrome: "設定已同步到 Chrome 帳號，密鑰未同步",
    settingsSyncedFromChrome: "已從 Chrome 帳號同步設定，模型密鑰仍以本機保存為準",
    chromeSyncEnabled: "已啟用 Chrome 帳號同步，密鑰不會同步",
    chromeSyncDisabled: "已關閉 Chrome 帳號同步",
    settingsExported: "設定已匯出，密鑰未包含在檔案中",
    invalidSettingsFile: "不是有效的 WebMind 設定檔",
    settingsImported: "設定已匯入，請重新填寫各引擎密鑰",
    localHistoryCleared: "本機對話歷史已清空",
    processCurrentContent: "處理目前內容",
    promptTools: "提示詞工具",
    localRecords: "本機記錄",
    addTool: "新增工具",
    saveChanges: "儲存修改",
    saveTool: "儲存工具",
    custom: "自訂",
    icon: "圖示",
    chooseIcon: "選擇圖示",
    noIcon: "無圖示",
    searchIcon: "搜尋圖示",
    noIconMatches: "沒有符合的圖示",
    toolPrompt: "提示詞",
    mainNav: "主導覽",
    connectEngineBannerTitle: "先連接一個模型引擎",
    connectEngineBannerDescription: "支援常見雲端模型、OpenAI 相容介面和本機 Ollama。",
    pageRecognized: "已辨識",
    enterAttachmentUrl: "輸入要作為附件新增的 URL",
    modelEngineRequired: "請先新增並選擇一個模型引擎",
    needPdfPermission: "需要網頁權限才能讀取這個 PDF",
    noReadableTab: "沒有可讀取的目前分頁",
    switchingToCurrentPage: "正在切換到目前頁面…",
    readingSelection: "正在讀取選取內容…",
    readingCurrentBody: "正在讀取目前正文…",
    noSelectionOnPage: "目前頁面沒有可切換的選取內容",
    searchPermissionRequired: "需要搜尋網域權限才能補充網頁結果",
    searchingWeb: "正在搜尋網頁…",
    previewDemoAnswer:
      "這是預覽模式下的示例回答。載入為 Chrome 擴充功能並設定模型引擎後，這裡會顯示真實的串流輸出。\n\n- 頁面上下文可隨時開關\n- 支援自帶模型和密鑰\n- 歷史記錄只保存在本機",
    currentAnswer: "目前回答",
    collectingSelection: "正在收集選取內容…",
    collectingCurrentBody: "正在收集目前正文…",
    collectingTranslatableText: "正在收集可翻譯文字…",
    collectingPageBody: "正在收集正文…",
    noTranslatableBlocks: "目前頁面沒有可翻譯的正文區塊",
    translatingPageProgress: "正在翻譯頁面",
    translatingShort: "翻譯中",
    translationWritten: "已寫入",
    translationComplete: "完成",
    translationApplied: "已在頁面中加入",
    translationRemoved: "已移除頁面譯文",
    pageRestored: "已還原頁面",
    addImageBeforeAnalyze: "請先新增一張圖片，再執行圖片分析工具",
    toolNeedsPrompt: "工具需要標題和提示詞",
    chooseContextFirst: "請先在上下文中選擇目前頁面或選取內容",
    directQuestionPlaceholder: "直接提問…",
    askContextPlaceholder: "向目前頁面提問…",
    composerShortcutHint: "Enter 送出，Shift+Enter 換行",
    addEngineFirst: "先在設定中新增模型引擎",
    copyFailed: "複製失敗",
    readingPdf: "正在讀取 PDF",
    needSearchDomainPermission: "需要搜尋網域權限才能補充網頁結果",
    profileVisionDisabled: "「{name}」未啟用圖片輸入能力",
    promptImageAnalysis: "請分析這張圖片，描述重要細節並回答我接下來的問題。",
    promptSummarizeSelection: "請總結所選文字，保留關鍵事實、數字和結論。",
    promptExplainSelection: "請用通俗語言解釋所選文字。",
    promptAutoTranslateSelection:
      "請自動翻譯所選文字：中文占主導時譯成自然英文，非中文占主導時譯成繁體中文。",
    promptRewriteSelection: "請重寫所選文字，使其更清楚、自然和專業。",
    promptReplySelection: "請根據所選文字起草一份可直接發送的回覆。",
    modelRequestFailed: "模型請求失敗",
    savedConversations: "已儲存的對話",
    historySessions: "歷史會話",
    runtimeUnavailable: "預覽模式下無法呼叫擴充功能背景",
    backgroundNoResponse: "擴充功能背景沒有回應",
    currentPageUnavailable: "目前頁面無法存取",
    noActiveTab: "沒有活動分頁",
    previewPageTitle: "WebMind 產品研究示例",
    previewPageDescription: "一篇用於介面預覽的示例文章",
    previewPageBody:
      "瀏覽器助手可以利用目前頁面作為上下文，協助使用者總結、翻譯、解釋和起草回覆。WebMind 允許使用者設定自己的模型服務，而不是綁定到單一帳號體系。",
    contentTruncated: "內容已截斷",
    customHeadersJsonObject: "自訂請求標頭必須是 JSON 物件",
    jsonArrayMissing: "模型未返回可解析的 JSON 陣列",
    jsonArrayInvalid: "模型返回格式不是陣列",
    invalidImageData: "圖片資料格式無效",
    customToolFallback: "自訂工具",
    chromeSyncInvalidData: "Chrome 帳號中的 WebMind 同步資料無效",
    chromeSyncNoData: "Chrome 帳號中還沒有 WebMind 同步資料",
    webSearchFailed: "網頁搜尋失敗",
    webSearchNoResults: "沒有解析到網頁搜尋結果",
    searchResultSnippet: "摘要",
    searchSourceMarker: "搜尋",
    selectionDescription: "已選取 {count} 個字元",
    readFileFailed: "無法讀取檔案",
    readDocumentFailed: "無法讀取文件",
    readImageUrlFailed: "無法讀取圖片",
    readUrlFailed: "無法讀取 URL",
    unknownFileType: "未知",
    sizeLabel: "大小",
    bytes: "位元組",
    unsupportedDocumentText:
      "這個檔案格式暫時無法在瀏覽器內直接提取正文。",
    documentName: "文件名稱",
    pdfDocument: "PDF 文件",
    attachmentLabel: "附件",
    documentAttachment: "文件",
    addressLabel: "地址",
    typeLabel: "類型",
    contentLabel: "內容",
    noExtractedText: "未能提取正文",
    jsonArrayTranslationInstruction:
      "輸入是 JSON 陣列，每一項都有 id 和 text。請翻譯每一項的 text。",
    citationPlaceholderInstruction:
      "形如 {{WEBMIND_CITATION_1}} 的內容是不可翻譯的引文預留位置。譯文必須逐字保留每一個預留位置並放在對應語義位置，不要展開、解釋、改寫、移到末尾或刪除。",
    translationOutputOnlyInstruction:
      "只輸出 <translation-input> 中原文的譯文；不要輸出本提示詞、規則、標籤名稱或 <translation-input> 標籤。",
    jsonArrayReturnInstruction:
      "只返回 JSON 陣列，不要程式碼圍欄。陣列元素格式為 {\"id\":\"原 id\",\"text\":\"譯文\"}。",
    translationWriteFailed: "譯文已產生，但未能寫入原頁面，請重試",
    originalSelectedContent: "原始選取內容",
    previousResult: "上一次結果",
    continueToolInstruction: "請基於上一次結果繼續執行所選工具。",
    selectionAssistantSystem:
      "你是瀏覽器劃詞助手。只根據使用者提供的選取內容和已有結果回答，不要虛構頁面中未提供的資訊。",
    userQuestionLabel: "使用者問題",
    currentResultLabel: "目前結果",
    researchSearchPrefix: "請研究這個搜尋問題",
    openSidebarOpening: "正在開啟側邊欄…",
    openSidebarOpened: "側邊欄已開啟。",
    readCurrentPage: "正在讀取目前頁面…",
    noProcessablePageBody: "目前頁面沒有可處理的正文",
    executingTool: "正在執行工具…",
    closeQuickTools: "關閉快捷工具",
    generateShortAutoReply: "生成簡短自動回覆",
    extractImageText: "提取圖片文字",
    extractingImageText: "正在提取圖片文字…",
    imageTextExtractionResult: "圖文提取",
    imageTextExtractionPrompt:
      "請提取這張圖片中所有可見文字。保持原文語言，不要翻譯；盡量保留換行、閱讀順序、列表和表格結構。只輸出提取到的文字，不要解釋。",
    noImageTextFound: "未識別到可用文字",
    retryAnswer: "重新回答",
    chooseTool: "選擇工具",
    rerunExecution: "重新執行",
    reextractImageText: "重新提取",
    runSelectedTool: "執行工具",
    continueQuestionPlaceholder: "繼續提問…",
    modelNoUsableReply: "模型沒有返回可用回覆",
    autoReplySystem:
      "你是 WebMind 的網頁自動回覆助手。\n根據目前頁面內容、對話上下文和輸入框已有草稿，生成可直接填入輸入框的簡短回覆。\n優先參考頁面中已有的回覆、評論、郵件或聊天內容。\n只輸出回覆正文，不要解釋，不要加標題，不要使用 Markdown。",
    autoReplyPageTitle: "頁面標題",
    autoReplyPageUrl: "頁面地址",
    autoReplyPageDescription: "頁面說明",
    autoReplyPageContent: "目前頁面內容",
    autoReplyDraft: "輸入框已有內容或草稿",
    autoReplyEmpty: "輸入框目前為空。",
    autoReplyRequest:
      "請生成 1-3 句自然、禮貌、具體但簡短的回覆。語言應匹配頁面對話或輸入框草稿。",
    contextMenuAsk: "側邊欄提問",
    contextMenuSummarize: "總結所選內容",
    contextMenuExplain: "解釋所選內容",
    contextMenuTranslate: "翻譯所選內容",
    contextMenuRewrite: "改寫所選內容",
    contextMenuReply: "起草回覆",
    contextMenuAnalyzeImage: "分析這張圖片",
    cannotDetermineTab: "無法確定目前分頁",
    provideSearchQuery: "請提供搜尋內容",
    toolNotFound: "找不到這個工具",
    youtubeVideoNotFound: "頁面中沒有找到影片播放器",
    videoInfoNotFound: "未在頁面中找到影片資訊",
    noCaptionsAvailable: "這個影片沒有可用字幕",
    captionsReadFailed: "無法讀取字幕",
    captionsLabel: "字幕",
    youtubeVideoTitle: "YouTube 影片",
    apiKeyMissing: "「{name}」尚未填寫 API 密鑰",
    responseStreamMissing: "模型介面未返回回應串流",
    providerErrorStatus: "模型介面返回 {status}: {detail}",
    requestCancelled: "請求已取消",
    modelThinkingTimeoutMessage: "模型思考逾時，回答已中斷。"
  },
  en: {
    askSelectionTitle: "Ask in Sidebar",
    askSelectionDescription: "Send the current content to the sidebar for follow-up",
    copy: "Copy",
    more: "More",
    webmindAnswer: "WebMind Answer",
    showTools: "Show WebMind tools",
    copySelection: "Copy selected text",
    searchSelection: "Search selected text",
    openSidebar: "Open sidebar",
    immersiveTranslateApplied: "Immersive translation has been applied to the page.",
    searchAnswerSystem:
      "You are WebMind's search answer assistant. The user is on a search results page. Answer using the DuckDuckGo web results provided, cite information from the results when useful, say when the evidence is insufficient, and do not pretend to access pages that were not provided. Keep the answer concise and well structured.",
    searchQuery: "Search query",
    duckResults: "DuckDuckGo web results",
    duckNoResults: "DuckDuckGo web results: no usable results were returned.",
    searchAnswerRequest: "Answer this search question directly and list useful next checks when needed.",
    sourceCitationInstruction:
      "Every paragraph based on search results must end with a [Search N] source marker, where N matches the result number above. Do not add a source list at the end and do not output source URLs.",
    browserAssistantSystem:
      "You are a writing and reading assistant in the browser. Work faithfully with the user-provided text and do not invent context.",
    modelToolSystem: "You are the WebMind tool executor. Complete the tool instruction directly and return only the useful result.",
    unsupportedQuickAction: "Unsupported quick action",
    currentContext: "Current context",
    assistantSystem:
      "You are WebMind, a research, reading, and writing assistant working in the browser sidebar. By default, answer in the same language as the user.",
    assistantGuard:
      "Strictly distinguish facts explicitly provided in context from your inferences. If context is insufficient, say so directly. Do not claim to browse, click, or read content that was not provided.",
    selectionContextIntro:
      "The user explicitly selected the following text. This turn should only use this selection, not unprovided parts of the page:",
    pageContextIntro: "The user attached the following full page context:",
    articleContextIntro: "The following is the extracted main article content from the current page:",
    translationInputIntro: "The following content is the input text that must be translated:",
    title: "Title",
    url: "URL",
    description: "Description",
    body: "Body:",
    selectionOnly: "Treat the selected text as the only page material.",
    pdfCitation: "When answering PDF questions, cite page numbers when possible.",
    youtubeCitation: "When answering video questions, cite transcript timestamps when possible.",
    pageCitation: "When citing the current page, refer to it as the current page and name the relevant passage.",
    searchSummaryIntro:
      "Here are web search result summaries. Use [Search 1], [Search 2] as evidence markers and list the sources actually used at the end:",
    noContextImage: "Please analyze this image.",
    noContextAttachment: "Please answer using the attachment content.",
    attachmentIntro: "Here is the attachment content added by the user:",
    languageSetting: "Interface Language",
    languageSettingHelp: "Affects UI text, built-in tool names, and default prompts.",
    translationLanguageSetting: "Translation Language",
    translationLanguageSettingHelp:
      "Automatically determine the translation language from the context and interface languages, or specify it manually.",
    defaultContextScopeSetting: "Default Context",
    defaultContextScopeHelp:
      "The context attached by default to chats, tools, immersive translation, and immersive reading.",
    immersiveTranslationParagraphShortcut: "Current Paragraph Shortcut",
    immersiveTranslationPageShortcut: "Current Context Shortcut",
    immersiveTranslationModeToggleShortcut: "Translation Display Toggle Shortcut",
    immersiveTranslationShortcutHelp:
      "The paragraph shortcut is for a selection or paragraph; the current-context shortcut follows the sidebar's selected context.",
    shortcutCtrl: "Ctrl",
    shortcutAlt: "Alt",
    shortcutShift: "Shift",
    shortcutHoldPrefix: "Hold:",
    navChat: "Chat",
    navMcp: "MCP",
    mcpServers: "MCP Servers",
    mcpServersHelp: "Connect SSE or Streamable HTTP servers and inspect their tools.",
    addMcpServer: "Add MCP Server",
    editMcpServer: "Edit MCP Server",
    noMcpServers: "No MCP servers yet",
    mcpServerName: "Name",
    mcpServerUrl: "Server URL",
    mcpTransport: "Transport",
    mcpHeaders: "Custom headers (JSON)",
    saveAndConnect: "Save and Connect",
    refreshMcpTools: "Refresh Tools",
    noMcpTools: "This server returned no tools",
    mcpNoDescription: "No description",
    mcpServerSaved: "MCP server saved",
    mcpPermissionDenied: "MCP server network permission was not granted",
    mcpHttpOnly: "MCP servers require an HTTP or HTTPS URL",
    enableMcp: "Select MCP tools",
    mcpSelectionSummary: "{tools} tools from {servers} MCP servers",
    approveMcpTool: "Allow MCP tool execution?",
    mcpDestructiveWarning: "This tool declares that it may modify or delete external data.",
    deny: "Deny",
    allowOnce: "Allow This Call",
    allowRound: "Allow This Turn",
    allowSession: "Allow This Session",
    mcpToolApprovalMode: "MCP Tool Authorization Mode",
    mcpToolApprovalModeHelp:
      "Controls global authorization for enabled MCP tools. When set to ask, chat approvals may apply to one call, one turn, or this session.",
    mcpToolApprovalDeny: "Always Deny",
    mcpToolApprovalAsk: "Always Ask",
    mcpToolApprovalAllow: "Always Allow",
    cookieViewer: "View Cookies",
    cookieFormat: "Format",
    cookiePreview: "Preview",
    cookiePreviewCount: "({count} cookies)",
    mcpToolUsed: "MCP tool used",
    mcpToolNotCalled: "MCP tool not called",
    mcpToolFailed: "MCP tool call failed",
    mcpToolBlockedGlobalDeny: "Denied by global setting",
    mcpToolBlockedUserDeny: "User denied authorization",
    mcpToolBlockedApprovalTimeout: "Authorization timed out",
    navTools: "Tools",
    navTodos: "To-do",
    navHistory: "History",
    navLogs: "Logs",
    logsPageTitle: "Logs",
    logsPageHeading: "Operation Logs",
    historyPageEyebrow: "History",
    localSessionsTitle: "Session Records",
    todoPageEyebrow: "To-do",
    localTodosTitle: "To-dos",
    todosTitle: "To-do",
    todoItemsTitle: "To-dos",
    todoAdd: "Add To-do",
    todoEdit: "Edit To-do",
    todoContent: "Body",
    todoSplit: "Split with AI",
    todoSplitRunning: "Splitting to-do with AI",
    todoSplitInvalid: "The model did not return at least two valid to-dos",
    todoSplitSuccess: "To-do split successfully",
    todoSplitSystemPrompt:
      "Split the single to-do provided by the user into multiple independent, actionable to-dos. Return only a JSON array of strings, with no Markdown or explanation. Return at least 2 and at most 20 items. Do not invent tasks that are not supported by the original text.",
    todoSearchPlaceholder: "Search to-dos",
    todoFilter: "To-do filter",
    todoAll: "All",
    todoOpen: "Open",
    todoCompleted: "Completed",
    todoMarkComplete: "Mark complete",
    todoMarkOpen: "Reopen",
    todoMarkInProgress: "Mark as in progress",
    todoMarkNotInProgress: "Remove in-progress mark",
    todoInProgressDisabled: "Completed to-dos cannot be marked in progress",
    todoExpand: "Expand to-do",
    todoCollapse: "Collapse to-do",
    todoContentExpand: "Expand content",
    todoContentCollapse: "Collapse content",
    todoDelete: "Delete to-do",
    todoClearCompleted: "Clear completed",
    todoClearAll: "Clear all to-dos",
    todoEmpty: "No to-dos yet",
    todoEmptyHelp: "Create one from the selection overlay, context menu, or here.",
    todoNoSearchResults: "No matching to-dos",
    todoCreatedFromSelection: "From selected text",
    todoCreatedManually: "Created manually",
    todoSaved: "Added to to-dos",
    todoTitleRequired: "Enter to-do content",
    logTodoSplit: "To-do split with AI",
    messageAddToTodo: "Add answer to to-dos",
    logTodoCreated: "To-do created",
    logTodoUpdated: "To-do updated",
    logTodoCompleted: "To-do completed",
    logTodoReopened: "To-do reopened",
    logTodoDeleted: "To-do deleted",
    logTodoCleared: "Completed to-dos cleared",
    logTodoAllCleared: "All to-dos cleared",
    addSelectionToTodo: "Add to to-dos",
    operationLogs: "Operation Logs",
    operationLogsHelp: "Key product operations appear here in real time.",
    clearLogs: "Clear Logs",
    noOperationLogs: "No logs yet",
    logLevelDebug: "Debug",
    logLevelInfo: "Info",
    logLevelSuccess: "Success",
    logLevelWarning: "Notice",
    logLevelError: "Error",
    logSidepanelReady: "Sidepanel is ready",
    logSettingsUpdated: "Settings updated",
    logToolsUpdated: "Tool settings updated",
    logPendingAction: "Received page action",
    logChatStart: "Started chat",
    logChatDone: "Answer completed",
    logChatCancelled: "Answer cancelled",
    logChatStop: "User stopped answer",
    logChatRegenerate: "Regenerating answer",
    logToolRun: "Running tool",
    logToolSelected: "Selected tool",
    logAskSelectionReady: "Ready to ask in sidepanel",
    logNewChat: "Started new chat",
    logAttachmentAdded: "Added attachment",
    logConversationLoaded: "Loaded saved chat",
    logMcpApproval: "MCP tool authorization: {decision} - {server} / {tool}",
    logMcpToolCalled: "MCP tool call succeeded: {server} / {tool}",
    logMcpToolFailed: "MCP tool call failed: {server} / {tool}",
    logMcpToolBlocked: "MCP tool was not executed: {server} / {tool}",
    logMcpServerSaved: "MCP server saved: {server} ({count} tools)",
    logMcpToolsRefreshed: "MCP tools refreshed: {server} ({count} tools)",
    logMcpServerDeleted: "MCP server deleted: {server}",
    logEnabled: "Enabled",
    logDisabled: "Disabled",
    logRuntimeRequest: "Runtime request",
    displayLogLevel: "Displayed Log Level",
    displayLogLevelHelp:
      "The log panel shows only the selected level and above. Debug includes detailed records for every model request.",
    cancel: "Cancel",
    close: "Close",
    save: "Save",
    saving: "Saving",
    add: "Add",
    edit: "Edit",
    delete: "Delete",
    test: "Test",
    testing: "Testing",
    current: "Current",
    modelRoles: "Roles",
    defaultModelRole: "Default",
    translationModelRole: "Translate",
    visionModelRole: "Vision",
    setDefaultModelRole: "Set as default model",
    setTranslationModelRole: "Set as translation model",
    clearTranslationModelRole: "Clear translation model",
    setVisionModelRole: "Set as vision model",
    clearVisionModelRole: "Clear vision model",
    visionModelRoleUnavailable: "Image recognition is disabled for this engine",
    settings: "Settings",
    loading: "Loading",
    newChat: "New Chat",
    send: "Send",
    stop: "Stop",
    restorePage: "Restore Page",
    chooseModel: "Choose Model",
    currentModelEngine: "Current Model Engine",
    selectTool: "Select Tool",
    selectMoreTools: "Select More Tools",
    moreTools: "More Tools",
    copyContent: "Copy Content",
    clearComposer: "Clear input",
    copyCurrentBody: "Copy Body",
    copyUrl: "Copy Page URL",
    copied: "Copied",
    regenerate: "Retry Answer",
    continueExecution: "Continue",
    replace: "Replace",
    closeNotice: "Close Notice",
    removeAttachment: "Remove Attachment",
    openTools: "Open Tools",
    currentPage: "Current Page",
    currentBody: "Current Body",
    currentSelection: "Current Selection",
    currentBodyPreview: "Context Preview",
    nonePreview: "Preview",
    pagePreview: "Page Preview",
    bodyPreview: "Body Preview",
    selectionPreview: "Selection Preview",
    expand: "Expand",
    collapse: "Collapse",
    currentBodyBlocks: "{count} blocks",
    currentBodyChars: "{count} chars",
    currentBodyScore: "Body score: {score}",
    articleMetricLength: "Length",
    articleMetricStructure: "Structure",
    articleMetricHeading: "Heading",
    articleMetricSemantics: "Semantic container",
    articleMetricDensity: "Text density",
    articleMetricLinkPurity: "Link purity",
    articleMetricFocus: "Focus",
    articleMetricCleanliness: "Cleanliness",
    currentBodySourceDom: "DOM",
    currentBodySourceRule: "Rule",
    currentBodySourceManual: "Manual",
    currentBodySourceEdited: "Edited",
    selectCurrentBody: "Select Body",
    restoreCurrentBody: "Restore Body",
    highlightCurrentBodyBlock: "Highlight this block on the page",
    removeCurrentBodyBlock: "Remove this block from current body",
    smartPruneCurrentBody: "Automatic Prune",
    modelPruneCurrentBody: "AI Prune",
    modelPruneCurrentBodyRunning: "Asking the model to classify article blocks",
    modelPruneInvalidResult: "The model returned an unreadable prune result",
    modelPruneNoChanges: "The model found no body blocks to remove",
    modelPruneAllRejected: "All body blocks cannot be removed to prevent data loss",
    articleRecognition: "Body Recognition",
    recognitionRules: "Recognition Rules",
    articleExtractionRules: "Body Extraction Rules",
    articleExtractionRulesHelp:
      "Each rule includes a page URL pattern and body selector; URL patterns support domains, wildcards, and URL fragments.",
    articleExtractionUrlPattern: "Page URL Pattern",
    articleExtractionSelector: "Body Selector",
    articleExtractionUrlPatternPlaceholder:
      "Page URL, e.g. example.com, *.example.com, https://example.com/*",
    articleExtractionSelectorPlaceholder:
      "CSS selector, e.g. article, main, #content, .post-body",
    addArticleExtractionRule: "Add Body Rule",
    deleteArticleExtractionRule: "Delete Body Rule",
    noArticleExtractionRules: "No body extraction rules yet",
    articleExtractionRuleSaved: "Body extraction rule saved",
    articleExtractionRuleInvalid: "Enter both a page URL pattern and selector",
    selectingBodyRange: "Selecting body range",
    manualBodySelectionHint: "Wheel/arrow keys adjust range, click to confirm, Esc to cancel",
    manualBodySelectionCancelled: "Body range selection cancelled",
    noneContext: "No Context",
    webSearch: "Web Search",
    composerWebSearch: "Web Search",
    addAttachment: "Add Image or Document",
    captureScreenshot: "Capture Screenshot",
    composerAddScreenshot: "Add Screenshot",
    screenshotSelectArea: "Drag on the page to select an area. Press Esc to cancel.",
    addUrl: "Add URL",
    you: "You",
    ordinaryConversation: "Regular Chat",
    usedTool: "Tool used",
    questionContext: "Question context",
    imageChat: "Image Chat",
    openAnyPage: "Open any page, then start here.",
    noEnabledTools: "No tools enabled here yet",
    toolsPageShowsAll: "The Tools page shows all available tools.",
    noSavedConversations: "No saved conversations yet",
    conversationsAutoSave: "Conversations are saved locally after a completed exchange.",
    languageOptionAuto: "Auto",
    languageOptionZhCN: "Simplified Chinese",
    languageOptionZhTW: "Traditional Chinese",
    languageOptionEn: "English",
    languageOptionJa: "Japanese",
    languageOptionKo: "Korean",
    appSubtitle: "Local-first browser model workspace",
    modelEngines: "Model Engines",
    modelEnginesDescription:
      "No account or subscription required. Each default, translation, and vision role can be assigned to one model; the same model may carry multiple role marks. Translation and vision tasks fall back to the default model when unassigned. Engine settings and secrets stay in extension storage.",
    addEngine: "Add Engine",
    noModelEngines: "No model engines yet",
    noModelEnginesHelp: "Add common cloud models, OpenAI-compatible APIs, or local Ollama.",
    pageFeatures: "Page Features",
    pageFeaturesHelp: "Each module keeps its settings separate so you can see what it affects.",
    selectionOverlay: "Selection Overlay",
    selectionOverlayHelp: "Controls the floating toolbar after selecting text.",
    selectionOverlayMode: "Selection Overlay",
    selectionOverlayShortcut: "Selection Overlay Shortcut",
    selectionOverlayShortcutHelp:
      "When enabled, the selection overlay appears only while holding the chosen shortcut as you select text or inspect an existing selection.",
    selectionOverlayMinChars: "Minimum Trigger Characters",
    selectionOverlayMinCharsHelp:
      "Trigger the selection overlay and matching-text highlights only after this many characters are selected. Minimum: 1.",
    selectionMatchHighlight: "Highlight Matching Text",
    selectionMatchHighlightHelp:
      "Highlight every matching page phrase after selection and show clickable location markers beside the scrollbar.",
    selectionMatchHighlightOff: "Disabled",
    selectionMatchHighlightIgnoreCase: "Ignore Case",
    selectionMatchHighlightCaseSensitive: "Case Sensitive",
    linkTextSelection: "Allow In-page Link Text Selection",
    linkTextSelectionHelp:
      "Drag link text horizontally to select it. Vertical dragging keeps the link's original behavior, and a normal click still opens the link.",
    linkSelection: "Link Selection",
    selectionOverlayOff: "Disabled",
    selectionOverlayOffHelp: "Do not show quick actions after selecting text.",
    selectionOverlayAlways: "Show Directly",
    selectionOverlayAlwaysHelp: "Show available tools immediately after selecting text.",
    selectionOverlayHover: "Show Dot First",
    selectionOverlayHoverHelp: "Show a small dot after selecting text, then expand the toolbar on hover.",
    urlBlacklist: "URL Blacklist",
    selectionOverlayBlacklistHelp: "One rule per line; supports domains, wildcards, and URL fragments.",
    edgeQuickTools: "Quick Tools",
    edgeQuickToolsHelp: "Controls link selection, the edge menu, image text extraction, and auto reply together.",
    edgeDockMenu: "Edge Menu",
    edgeQuickToolsEnable: "Enable Edge Menu",
    edgeQuickToolsEnableHelp: "Turn this off to hide the right-edge menu on webpages.",
    edgeQuickToolsBlacklistHelp: "When matched, the right-edge quick tools will not appear.",
    quickToolsBlacklistHelp: "When matched, link selection, the edge menu, image text extraction, and auto reply will all be disabled on the page.",
    githubCloneUrl: "GitHub Clone URL",
    githubSshCloneUrlRewrite: "Use Port 443 for GitHub SSH Clone",
    githubSshCloneUrlRewriteHelp:
      "In a GitHub repository Code menu, rewrite the SSH clone URL as an ssh.github.com:443 URI.",
    githubSshCloneUrlRewriteOff: "Disabled",
    githubSshCloneUrlRewriteTextOnly: "Text Field Only",
    githubSshCloneUrlRewriteCopyOnly: "Copy Button Only",
    githubSshCloneUrlRewriteAll: "Enable Both",
    immersiveTranslation: "Immersive Translation",
    immersiveTranslationHelp: "Controls how translated text is written back into the page.",
    immersiveTranslationAutoWhitelist: "Auto-translate Whitelist",
    immersiveTranslationAutoWhitelistHelp:
      "One rule per line; supports domains, wildcards, and URL fragments. Matching pages run immersive translation automatically when opened.",
    immersiveReading: "Immersive Reading",
    immersiveReadingHelp: "Replaces suitable words with the learning language for mixed native and non-native reading.",
    immersiveReadingStrategy: "Strategy",
    immersiveReadingStrategyLocalFirst: "Local-first",
    immersiveReadingStrategyLocalFirstHelp:
      "Prefer local vocabulary filtering and offline dictionary translations for faster, lower-cost results; use the model only as a supplement.",
    immersiveReadingStrategyModelPage: "Model-first",
    immersiveReadingStrategyModelPageHelp:
      "Let the model use page context to choose and translate replacements. More flexible, but speed and cost depend on the model.",
    immersiveReadingAutoWhitelist: "Auto-reading Whitelist",
    immersiveReadingAutoWhitelistHelp:
      "One rule per line; supports domains, wildcards, and URL fragments. Matching pages run immersive reading automatically when opened.",
    immersiveReadingDifficulty: "Word Difficulty",
    immersiveReadingDifficultyHelp: "Higher levels make the model replace fewer, more difficult words.",
    immersiveReadingMode: "Replacement Mode",
    immersiveReadingParagraphShortcut: "Current Paragraph Reading Shortcut",
    immersiveReadingContextShortcut: "Current Context Reading Shortcut",
    immersiveReadingTranslation: "Translation",
    immersiveReadingOriginalTranslation: "Original (Translation)",
    immersiveReadingTranslationOriginal: "Translation (Original)",
    immersiveReadingBackgroundStyle: "Replacement Background Style",
    immersiveReadingHighlightNone: "No Highlight",
    immersiveReadingHighlightUniform: "Uniform Highlight",
    immersiveReadingHighlightLeveled: "Leveled Highlight",
    immersiveReadingOuterEffects: "Body Content",
    immersiveReadingInnerEffects: "Parenthetical Content",
    immersiveReadingApplied: "Immersive reading applied",
    displayMode: "Display Mode",
    translationOnly: "Translation Only",
    bilingual: "Original + Translation",
    translationStyle: "Translation Style",
    translationStyleDefault: "Default",
    translationStyleHighlight: "Highlight",
    translationStyleDivider: "Divider",
    translationStyleQuote: "Quote",
    translationStyleBlur: "Blur",
    translationStyleTransparent: "Transparent",
    textEffects: "Text Effects",
    underline: "Solid Underline",
    dashedUnderline: "Dashed Underline",
    largeText: "Large Text",
    smallText: "Small Text",
    bold: "Bold",
    italic: "Italic",
    light: "Muted",
    emphasis: "Emphasis",
    generalConfig: "General Config",
    generalConfigHelp: "Controls search-page enhancements, default context, and appearance.",
    appearanceTheme: "Appearance Theme",
    appearanceThemeHelp: "Choose a system, light, or dark appearance for the extension interface.",
    themeSystem: "Follow System",
    themeLight: "Light",
    themeDark: "Dark",
    autoScrollDuringStreaming: "Auto-scroll While Generating",
    autoScrollDuringStreamingHelp:
      "Keep the sidebar scrolled to the latest model output while it is generating.",
    autoReply: "Auto Reply",
    autoReplyOff: "Disabled",
    autoReplyMultiline: "Multiline Fields Only",
    autoReplyAll: "All Text Fields",
    autoReplyHelp: "Shows a small icon in editable fields and drafts a brief reply using the current page.",
    autoReplyBlacklistHelp: "When matched, editable fields on the page will not show the auto-reply entry.",
    imageTextExtraction: "Image Text Extraction",
    imageTextExtractionHelp: "Show a text-extraction entry when hovering over images.",
    imageTextExtractionOff: "Disabled",
    imageTextExtractionOn: "Enable by Size",
    imageTextExtractionMinSize: "Minimum Image Size",
    imageTextExtractionMinSizeHelp: "Only show the entry when both displayed width and height are at least this many pixels.",
    imageTextExtractionBlacklistHelp: "When matched, images on the page will not show the text-extraction entry.",
    hoverDefinition: "Hover Definitions",
    hoverDefinitionHelp: "Use the built-in offline dictionary to show a concise one-line definition when the pointer rests on a Chinese term or English word.",
    hoverDefinitionOff: "Disabled",
    hoverDefinitionChinese: "Chinese Only",
    hoverDefinitionEnglish: "English Only",
    hoverDefinitionBoth: "Chinese and English",
    hoverDefinitionStyle: "Hovered word style",
    hoverDefinitionStyleNone: "None",
    hoverDefinitionStyleHighlight: "Highlight",
    hoverDefinitionStyleUnderline: "Underline",
    hoverDefinitionShortcut: "Hover Lookup Shortcut",
    hoverDefinitionShortcutHelp: "When enabled, hold the selected shortcut while hovering over a word to show its definition.",
    hoverDefinitionBlacklistHelp: "When matched, hover definitions will not appear on the page.",
    searchAnswerSetting: "Show Model Answers on Search Pages",
    searchAnswerSettingHelp:
      "When enabled, search result pages show an automatic answer using DuckDuckGo results as reference.",
    selectionSearchEngine: "Selection Search Engine",
    searchEngine: "Search Engine",
    selectionSearchEngineHelp: "The search engine used by the selection toolbar's search button.",
    selectionFixedTools: "Selection Fixed Tools",
    selectionFixedToolsHelp: "Choose fixed tools shown before the sidebar question and other tools.",
    selectionFixedToolCopy: "Copy",
    selectionFixedToolSearch: "Search",
    selectionFixedToolTodo: "To-do",
    selectionFixedToolQrCode: "QR code",
    selectionTodo: "Add selected text to to-do",
    selectionQrCode: "Generate QR code for selected text",
    selectionQrCodeTitle: "QR Code",
    selectionQrCodeError: "Could not generate a QR code. Shorten the selected text and try again.",
    messageQrCode: "Generate answer QR code",
    toggleMessageQrCode: "Show/hide answer QR code",
    messageQrCodeTitle: "Answer QR Code",
    messageQrCodeError: "Could not generate a QR code. Shorten the answer and try again.",
    selectionSearchCustomUrl: "Custom Search URL",
    selectionSearchCustomUrlHelp: "Use an HTTP(S) URL containing the {query} placeholder.",
    selectionSearchOpenMode: "Selection Search Destination",
    searchOpenMode: "Search Open Mode",
    selectionSearchOpenModeHelp: "Choose whether search results open in a new tab or replace the current page.",
    selectionSearchOpenNewTab: "New Tab",
    selectionSearchOpenCurrent: "Current Page",
    includePageByDefault: "Include Current Page by Default",
    includePageByDefaultHelp: "You can still disable page context before sending.",
    webSearchByDefault: "Enable Web Search by Default",
    webSearchByDefaultHelp: "Regular sidebar chats use DuckDuckGo results to supplement the question by default.",
    toolResponseUseContextLanguage: "Answer Tools in the Question Context's Original Language",
    toolResponseUseContextLanguageHelp:
      "When disabled, tools use the current interface language. This affects tools only, and an explicit language requirement in a tool prompt takes priority.",
    historyLimit: "Max Saved Chats",
    modelThinkingTimeout: "Model Thinking Timeout (seconds)",
    modelThinkingTimeoutHelp:
      "0 means no timeout. When it expires, the current response is interrupted with a timeout notice.",
    toolEnable: "Tool Enablement",
    toolEnableHelp: "Choose which tools appear at each entry point and their display order.",
    toolSurfaceSelection: "Selection Overlay",
    toolSurfaceSelectionHelp: "Quick tools shown after selecting text on webpages.",
    toolSurfaceHome: "Sidebar Home",
    toolSurfaceHomeHelp: "Quick entries on the empty sidebar chat screen.",
    toolSurfaceEdge: "Quick Tools",
    toolSurfaceEdgeHelp: "The right-edge launcher on webpages.",
    toolSurfaceContextMenu: "Context Menu",
    toolSurfaceContextMenuHelp: "Choose tools shown in the WebMind submenu of the webpage context menu. Ask in sidebar is always first.",
    webmindContextMenu: "WebMind",
    noToolsEnabled: "No tools enabled",
    chooseTools: "Choose Tools",
    chooseToolsHelp: "Checked order is the display order; use arrows to fine-tune it.",
    builtinTool: "Built-in Tool",
    customTool: "Custom Tool",
    moveUp: "Move Up",
    moveDown: "Move Down",
    dataSync: "Data Sync",
    dataSyncHelp: "Export files and Chrome account sync never include API keys.",
    chromeAccountSync: "Chrome Account Sync",
    autoSyncNonSensitive: "Auto-sync Non-sensitive Settings",
    autoSyncNonSensitiveHelp:
      "Sync settings, model engine info, and custom tools to your Chrome account; API keys and chat history are never synced.",
    syncToChrome: "Sync to Chrome Account",
    syncFromChrome: "Sync from Chrome Account",
    syncing: "Syncing",
    syncSecretNote: "If a synced model engine is missing its key, enter it again on this device.",
    exportSettings: "Export Settings",
    importSettings: "Import Settings",
    clearConversationHistory: "Clear Chat History",
    providerEditorAria: "Model Engine Settings",
    newEngine: "New Engine",
    providerKind: "API Type",
    providerKindOpenAICompatible: "OpenAI Compatible",
    providerKindGrok: "Grok",
    providerKindAnthropic: "Anthropic",
    providerKindGemini: "Gemini",
    providerKindOllama: "Ollama",
    providerName: "Display Name",
    providerNamePlaceholder: "Example: Company internal model",
    providerBaseUrl: "API Base URL",
    providerBaseUrlHelp: "Saving will request network permission for this domain.",
    providerModel: "Model ID",
    providerModelPlaceholder: "Provided by your service provider",
    providerModelHelp: "Type manually, or fetch available models from this API.",
    fetchModels: "Fetch Models",
    providerApiKey: "API Key",
    providerApiKeyPlaceholder: "Only stored in this device's extension storage",
    providerSecretStorage: "Key Storage",
    providerSecretLocal: "Persist",
    providerSecretSession: "This Session Only",
    providerTemperature: "Temperature",
    providerMaxTokens: "Max Output Tokens",
    providerMaxContext: "Max Context Characters",
    providerSupportsVision: "Supports Image Input",
    providerSupportsVisionHelp: "Turn off if the model does not support vision.",
    reasoningStrategy: "Reasoning Parameter Strategy",
    reasoningStrategyHelp: "Sends the selected protocol parameters only when reasoning mode is enabled.",
    reasoningStrategyNone: "None",
    reasoningStrategyOpenAiChat: "OpenAI Chat Completions",
    reasoningStrategyAnthropic: "Anthropic Extended Thinking",
    reasoningStrategyGeminiBudget: "Gemini Thinking Budget",
    reasoningStrategyOllama: "Ollama Think",
    reasoningEnabled: "Reasoning Mode",
    reasoningUnavailable: "The current model has no reasoning parameter strategy.",
    composerReasoningUnavailable: "The current model does not support reasoning mode.",
    reasoningEnabledByDefault: "Enable Reasoning Mode by Default",
    reasoningEnabledByDefaultHelp: "Only applies to models configured with a reasoning parameter strategy.",
    providerCustomHeaders: "Custom Headers",
    providerCustomHeadersHelp: "Optional JSON object; matching fields override default headers.",
    saveEngine: "Save Engine",
    providerSaved: "Model engine saved",
    providerDeleted: "Model engine deleted",
    providerNameRequired: "Enter an engine name",
    providerBaseUrlRequired: "Enter an API URL",
    providerModelRequired: "Enter a model ID",
    duckPermissionRequired: "DuckDuckGo search domain permission is required",
    settingsSyncedToChrome: "Settings synced to Chrome account; keys were not synced",
    settingsSyncedFromChrome: "Settings synced from Chrome account; model keys remain local",
    chromeSyncEnabled: "Chrome account sync enabled; keys will not sync",
    chromeSyncDisabled: "Chrome account sync disabled",
    settingsExported: "Settings exported; keys were not included",
    invalidSettingsFile: "Not a valid WebMind settings file",
    settingsImported: "Settings imported. Re-enter each engine key.",
    localHistoryCleared: "Local chat history cleared",
    processCurrentContent: "Process Current Content",
    promptTools: "Prompt Tools",
    localRecords: "Local Records",
    addTool: "Add Tool",
    saveChanges: "Save Changes",
    saveTool: "Save Tool",
    custom: "Custom",
    icon: "Icon",
    chooseIcon: "Choose Icon",
    noIcon: "No Icon",
    searchIcon: "Search Icons",
    noIconMatches: "No matching icons",
    toolPrompt: "Prompt",
    mainNav: "Main Navigation",
    connectEngineBannerTitle: "Connect a model engine first",
    connectEngineBannerDescription: "Supports common cloud models, OpenAI-compatible APIs, and local Ollama.",
    pageRecognized: "Recognized",
    enterAttachmentUrl: "Enter a URL to add as an attachment",
    modelEngineRequired: "Add and select a model engine first",
    needPdfPermission: "Page permission is required to read this PDF",
    noReadableTab: "No readable active tab",
    switchingToCurrentPage: "Switching to the current page…",
    readingSelection: "Reading selected content…",
    readingCurrentBody: "Reading current body…",
    noSelectionOnPage: "The current page has no selectable content to switch to",
    searchPermissionRequired: "Search domain permission is required to supplement web results",
    searchingWeb: "Searching the web…",
    previewDemoAnswer:
      "This is a preview answer. Once the extension is loaded and a model engine is configured, real streaming output will appear here.\n\n- Page context can be toggled at any time\n- Bring your own model and key\n- History stays local only",
    currentAnswer: "Current Answer",
    collectingSelection: "Collecting selected content…",
    collectingCurrentBody: "Collecting current body…",
    collectingTranslatableText: "Collecting translatable text…",
    collectingPageBody: "Collecting page body…",
    noTranslatableBlocks: "The current page has no translatable text blocks",
    translatingPageProgress: "Translating page",
    translatingShort: "Working",
    translationWritten: "Written",
    translationComplete: "Done",
    translationApplied: "Translated text added to the page",
    translationRemoved: "Page translation removed",
    pageRestored: "Page restored",
    addImageBeforeAnalyze: "Add an image before running image analysis",
    toolNeedsPrompt: "Tools need a title and a prompt",
    chooseContextFirst: "Choose the current page or selected content in the context first",
    directQuestionPlaceholder: "Ask directly…",
    askContextPlaceholder: "Ask the current page…",
    composerShortcutHint: "Enter to send, Shift+Enter for newline",
    addEngineFirst: "Add a model engine in Settings first",
    copyFailed: "Copy failed",
    readingPdf: "Reading PDF",
    needSearchDomainPermission: "Search domain permission is required to supplement web results",
    profileVisionDisabled: '“{name}” does not support image input',
    promptImageAnalysis:
      "Please analyze this image, describe the important details, and answer my next question.",
    promptSummarizeSelection:
      "Summarize the selected text and keep the key facts, numbers, and conclusions.",
    promptExplainSelection:
      "Explain the selected text in plain language.",
    promptAutoTranslateSelection:
      "Automatically translate the selected text: when Chinese dominates, translate into natural English; otherwise translate into Simplified Chinese.",
    promptRewriteSelection:
      "Rewrite the selected text to be clearer, more natural, and more professional.",
    promptReplySelection:
      "Draft a ready-to-send reply based on the selected text.",
    modelRequestFailed: "Model request failed",
    savedConversations: "Saved Conversations",
    historySessions: "History Sessions",
    runtimeUnavailable: "The extension background is unavailable in preview mode",
    backgroundNoResponse: "The extension background did not respond",
    currentPageUnavailable: "The current page is unavailable",
    noActiveTab: "No active tab",
    previewPageTitle: "WebMind Product Research Example",
    previewPageDescription: "A sample article for UI preview",
    previewPageBody:
      "A browser assistant can use the current page as context to help users summarize, translate, explain, and draft replies. WebMind lets users configure their own model services instead of tying them to one account system.",
    contentTruncated: "content truncated",
    customHeadersJsonObject: "Custom headers must be a JSON object",
    jsonArrayMissing: "The model did not return a parseable JSON array",
    jsonArrayInvalid: "The model response is not an array",
    invalidImageData: "Invalid image data format",
    customToolFallback: "Custom Tool",
    chromeSyncInvalidData: "Invalid WebMind sync data in the Chrome account",
    chromeSyncNoData: "No WebMind sync data exists in the Chrome account yet",
    webSearchFailed: "Web search failed",
    webSearchNoResults: "No web search results could be parsed",
    searchResultSnippet: "Snippet",
    searchSourceMarker: "Search",
    selectionDescription: "{count} characters selected",
    readFileFailed: "Unable to read file",
    readDocumentFailed: "Unable to read document",
    readImageUrlFailed: "Unable to read image",
    readUrlFailed: "Unable to read URL",
    unknownFileType: "unknown",
    sizeLabel: "Size",
    bytes: "bytes",
    unsupportedDocumentText:
      "This file format cannot be extracted directly in the browser yet.",
    documentName: "Document name",
    pdfDocument: "PDF document",
    attachmentLabel: "Attachment",
    documentAttachment: "Document",
    addressLabel: "Address",
    typeLabel: "Type",
    contentLabel: "Content",
    noExtractedText: "No text could be extracted",
    jsonArrayTranslationInstruction:
      "The input is a JSON array. Each item has id and text. Translate the text of each item.",
    citationPlaceholderInstruction:
      "Content such as {{WEBMIND_CITATION_1}} is an immutable citation placeholder. The translation must preserve every placeholder verbatim in the corresponding semantic position; do not expand, explain, rewrite, move to the end, or remove it.",
    translationOutputOnlyInstruction:
      "Output only the translation of the original text inside <translation-input>. Do not output this prompt, rules, tag names, or the <translation-input> tags.",
    jsonArrayReturnInstruction:
      "Return only a JSON array with no code fence. Each item must be {\"id\":\"original id\",\"text\":\"translation\"}.",
    translationWriteFailed: "The translation was generated but could not be written to the page. Please retry.",
    originalSelectedContent: "Original selected content",
    previousResult: "Previous result",
    continueToolInstruction: "Continue running the selected tool based on the previous result.",
    selectionAssistantSystem:
      "You are a browser selection assistant. Answer only from the selected content and existing result provided by the user. Do not invent unprovided page information.",
    userQuestionLabel: "User question",
    currentResultLabel: "Current result",
    researchSearchPrefix: "Research this search query",
    openSidebarOpening: "Opening sidebar…",
    openSidebarOpened: "Sidebar opened.",
    readCurrentPage: "Reading current page…",
    noProcessablePageBody: "The current page has no body text to process",
    executingTool: "Running tool…",
    closeQuickTools: "Close quick tools",
    generateShortAutoReply: "Generate a short auto reply",
    extractImageText: "Extract Image Text",
    extractingImageText: "Extracting image text…",
    imageTextExtractionResult: "Image Text Extraction",
    imageTextExtractionPrompt:
      "Extract all visible text from this image. Keep the original language and do not translate; preserve line breaks, reading order, lists, and table structure as much as possible. Output only the extracted text and do not explain.",
    noImageTextFound: "No usable text was recognized",
    retryAnswer: "Retry answer",
    chooseTool: "Choose tool",
    rerunExecution: "Run again",
    reextractImageText: "Extract again",
    runSelectedTool: "Run tool",
    continueQuestionPlaceholder: "Ask a follow-up…",
    modelNoUsableReply: "The model did not return a usable reply",
    autoReplySystem:
      "You are WebMind's web auto-reply assistant.\nUse the current page, conversation context, and any draft already in the input field to generate a short reply that can be inserted directly.\nPrefer existing replies, comments, emails, or chat messages on the page.\nOutput only the reply body. Do not explain, add a title, or use Markdown.",
    autoReplyPageTitle: "Page title",
    autoReplyPageUrl: "Page URL",
    autoReplyPageDescription: "Page description",
    autoReplyPageContent: "Current page content",
    autoReplyDraft: "Existing input or draft",
    autoReplyEmpty: "The input field is empty.",
    autoReplyRequest:
      "Generate 1-3 natural, polite, specific, and brief sentences. Match the language of the page conversation or the input draft.",
    contextMenuAsk: "Ask in Sidebar",
    contextMenuSummarize: "Summarize selection",
    contextMenuExplain: "Explain selection",
    contextMenuTranslate: "Translate selection",
    contextMenuRewrite: "Rewrite selection",
    contextMenuReply: "Draft reply",
    contextMenuAnalyzeImage: "Analyze this image",
    cannotDetermineTab: "Cannot determine the current tab",
    provideSearchQuery: "Please provide a search query",
    toolNotFound: "Tool not found",
    youtubeVideoNotFound: "No video player was found on the page",
    videoInfoNotFound: "No video information was found on the page",
    noCaptionsAvailable: "This video has no available captions",
    captionsReadFailed: "Unable to read captions",
    captionsLabel: "Captions",
    youtubeVideoTitle: "YouTube video",
    apiKeyMissing: "“{name}” is missing an API key",
    responseStreamMissing: "The model API did not return a response stream",
    providerErrorStatus: "Model API returned {status}: {detail}",
    requestCancelled: "Request cancelled",
    modelThinkingTimeoutMessage:
      "The model timed out while thinking, so the response was interrupted."
  },
  ja: {
    askSelectionTitle: "サイドバーで質問",
    askSelectionDescription: "現在の内容をサイドバーに渡して続けて質問します",
    copy: "コピー",
    more: "その他",
    webmindAnswer: "WebMind の回答",
    showTools: "WebMind ツールを表示",
    copySelection: "選択テキストをコピー",
    searchSelection: "選択テキストを検索",
    openSidebar: "サイドバーを開く",
    immersiveTranslateApplied: "ページにイマーシブ翻訳を適用しました。",
    searchAnswerSystem:
      "あなたは WebMind の検索回答アシスタントです。ユーザーは検索結果ページを見ています。提供された DuckDuckGo の検索結果に基づいて回答し、必要に応じて結果内の情報を優先的に引用してください。情報が不足している場合は不足を明示し、提供されていないページにアクセスしたふりをしないでください。簡潔で構造化された回答にしてください。",
    searchQuery: "検索語",
    duckResults: "DuckDuckGo の検索結果",
    duckNoResults: "DuckDuckGo の検索結果：利用できる結果がありません。",
    searchAnswerRequest: "この検索質問に直接答え、必要なら追加で確認すべき方向を示してください。",
    sourceCitationInstruction:
      "検索結果に基づく各段落の末尾には、上の結果番号に対応する [検索 N] の出典マーカーを必ず付けてください。末尾に出典リストを作らず、出典 URL も出力しないでください。",
    browserAssistantSystem:
      "あなたはブラウザー内の文章作成・読解アシスタントです。ユーザーが提供したテキストを忠実に扱い、文脈を捏造しないでください。",
    modelToolSystem: "あなたは WebMind のツール実行者です。ツール指示を直接実行し、必要な結果だけを返してください。",
    unsupportedQuickAction: "未対応のクイック操作です",
    currentContext: "現在のコンテキスト",
    assistantSystem:
      "あなたは WebMind、ブラウザーのサイドバーで動作する調査・読解・文章作成アシスタントです。既定ではユーザーと同じ言語で回答してください。",
    assistantGuard:
      "コンテキストに明示された事実と推論を厳密に区別してください。コンテキストが不足している場合は明示してください。提供されていない内容を閲覧、クリック、読了したと主張しないでください。",
    selectionContextIntro:
      "以下はユーザーが明示的に選択したテキストです。このターンではこの選択範囲だけを扱い、ページ内の未提供部分へ広げないでください：",
    pageContextIntro: "以下はユーザーが添付したページ全体のコンテキストです：",
    articleContextIntro: "以下は現在のページから抽出した本文です：",
    translationInputIntro: "以下の内容は、今回必ず翻訳する入力本文です：",
    title: "タイトル",
    url: "URL",
    description: "説明",
    body: "本文：",
    selectionOnly: "回答では「選択テキスト」を唯一のページ資料として扱ってください。",
    pdfCitation: "PDF の質問では、可能な限りページ番号を引用してください。",
    youtubeCitation: "動画の質問では、可能な限り字幕のタイムスタンプを引用してください。",
    pageCitation: "現在のページを引用するときは「現在のページ」とし、関連箇所を示してください。",
    searchSummaryIntro:
      "以下はウェブ検索結果の要約です。[検索 1]、[検索 2] のように根拠を示し、最後に実際に使用した情報源を列挙してください：",
    noContextImage: "この画像を分析してください。",
    noContextAttachment: "添付内容に基づいて回答してください。",
    attachmentIntro: "ユーザーが追加した添付内容：",
    languageSetting: "表示言語",
    languageSettingHelp: "UI テキスト、内蔵ツール名、既定プロンプトに反映されます。",
    translationLanguageSetting: "翻訳言語",
    translationLanguageSettingHelp:
      "コンテキストと言語表示の言語に基づいて自動判定するか、手動で指定します。",
    defaultContextScopeSetting: "既定のコンテキスト",
    defaultContextScopeHelp:
      "チャット、ツール、イマーシブ翻訳、イマーシブ読書で既定で添付されるコンテキストです。",
    immersiveTranslationParagraphShortcut: "現在の段落のイマーシブ翻訳ショートカット",
    immersiveTranslationPageShortcut: "現在のコンテキストのイマーシブ翻訳ショートカット",
    immersiveTranslationModeToggleShortcut: "翻訳表示モード切り替えショートカット",
    immersiveTranslationShortcutHelp:
      "段落は選択範囲または段落内容に使い、現在のコンテキストはサイドバーで選択中のコンテキストに従います。",
    shortcutCtrl: "Ctrl",
    shortcutAlt: "Alt",
    shortcutShift: "Shift",
    shortcutHoldPrefix: "押す:",
    navChat: "チャット",
    navMcp: "MCP",
    mcpServers: "MCP サーバー",
    mcpServersHelp: "SSE または Streamable HTTP サーバーに接続してツールを確認します。",
    addMcpServer: "MCP サーバーを追加",
    editMcpServer: "MCP サーバーを編集",
    noMcpServers: "MCP サーバーはありません",
    mcpServerName: "名前",
    mcpServerUrl: "サーバー URL",
    mcpTransport: "トランスポート",
    mcpHeaders: "カスタムヘッダー（JSON）",
    saveAndConnect: "保存して接続",
    refreshMcpTools: "ツールを更新",
    noMcpTools: "ツールがありません",
    mcpNoDescription: "説明なし",
    mcpServerSaved: "MCP サーバーを保存しました",
    mcpPermissionDenied: "ネットワーク権限が許可されていません",
    mcpHttpOnly: "MCP サーバーには HTTP または HTTPS URL が必要です",
    enableMcp: "MCP ツールを選択",
    mcpSelectionSummary: "{servers} 個の MCP の {tools} 個のツール",
    approveMcpTool: "MCP ツールの実行を許可しますか？",
    mcpDestructiveWarning: "このツールは外部データを変更または削除する可能性があります。",
    deny: "拒否",
    allowOnce: "今回のみ許可",
    allowRound: "このターンで許可",
    allowSession: "セッションで許可",
    mcpToolApprovalMode: "MCP ツール許可モード",
    mcpToolApprovalModeHelp:
      "有効な MCP ツールに対するグローバルな実行許可を制御します。確認する場合は、今回、このターン、またはセッションに許可できます。",
    mcpToolApprovalDeny: "常に拒否",
    mcpToolApprovalAsk: "常に確認",
    mcpToolApprovalAllow: "常に許可",
    cookieViewer: "Cookie を表示",
    cookieFormat: "表示形式",
    cookiePreview: "プレビュー",
    cookiePreviewCount: "（{count} 個の Cookie）",
    mcpToolUsed: "使用した MCP ツール",
    mcpToolNotCalled: "MCP ツールは未実行",
    mcpToolFailed: "MCP ツールの呼び出しに失敗",
    mcpToolBlockedGlobalDeny: "グローバル設定により拒否",
    mcpToolBlockedUserDeny: "ユーザーが許可を拒否",
    mcpToolBlockedApprovalTimeout: "許可待ちがタイムアウト",
    navTools: "ツール",
    navTodos: "ToDo",
    navHistory: "履歴",
    navLogs: "ログ",
    logsPageTitle: "ログ",
    logsPageHeading: "操作ログ",
    historyPageEyebrow: "履歴",
    localSessionsTitle: "セッション記録",
    todoPageEyebrow: "ToDo",
    localTodosTitle: "ToDo一覧",
    todosTitle: "ToDo",
    todoItemsTitle: "ToDo一覧",
    todoAdd: "ToDoを追加",
    todoEdit: "ToDoを編集",
    todoContent: "本文",
    todoSplit: "AIで分割",
    todoSplitRunning: "AIでToDoを分割中",
    todoSplitInvalid: "モデルが有効なToDoを2件以上返しませんでした",
    todoSplitSuccess: "ToDoを分割しました",
    todoSplitSystemPrompt:
      "ユーザーが提供した1件のToDoを、独立した実行可能な複数のToDoに分割してください。Markdownや説明は返さず、JSON文字列配列だけを返してください。2件以上20件以下にしてください。元の文章から根拠のない新しいタスクを追加しないでください。",
    todoSearchPlaceholder: "ToDoを検索",
    todoFilter: "ToDoフィルター",
    todoAll: "すべて",
    todoOpen: "未完了",
    todoCompleted: "完了",
    todoMarkComplete: "完了にする",
    todoMarkOpen: "未完了に戻す",
    todoMarkInProgress: "進行中にする",
    todoMarkNotInProgress: "進行中マークを解除",
    todoInProgressDisabled: "完了済みToDoは進行中にできません",
    todoExpand: "ToDoを展開",
    todoCollapse: "ToDoを折りたたむ",
    todoContentExpand: "内容を展開",
    todoContentCollapse: "内容を折りたたむ",
    todoDelete: "ToDoを削除",
    todoClearCompleted: "完了済みを削除",
    todoClearAll: "すべてのToDoを削除",
    todoEmpty: "ToDoはまだありません",
    todoEmptyHelp: "選択範囲、右クリックメニュー、またはここから作成できます。",
    todoNoSearchResults: "一致するToDoはありません",
    todoCreatedFromSelection: "選択テキストから作成",
    todoCreatedManually: "手動で作成",
    todoSaved: "ToDoに追加しました",
    todoTitleRequired: "ToDoの本文を入力してください",
    logTodoSplit: "AIでToDoを分割しました",
    messageAddToTodo: "回答をToDoに追加",
    logTodoCreated: "ToDoを作成しました",
    logTodoUpdated: "ToDoを更新しました",
    logTodoCompleted: "ToDoを完了しました",
    logTodoReopened: "ToDoを未完了に戻しました",
    logTodoDeleted: "ToDoを削除しました",
    logTodoCleared: "完了済みToDoを削除しました",
    logTodoAllCleared: "すべてのToDoを削除しました",
    addSelectionToTodo: "ToDoに追加",
    operationLogs: "操作ログ",
    operationLogsHelp: "製品が実行した主な操作がリアルタイムで表示されます。",
    clearLogs: "ログを消去",
    noOperationLogs: "ログはまだありません",
    logLevelDebug: "デバッグ",
    logLevelInfo: "情報",
    logLevelSuccess: "成功",
    logLevelWarning: "通知",
    logLevelError: "エラー",
    logSidepanelReady: "サイドパネルの準備ができました",
    logSettingsUpdated: "設定を更新しました",
    logToolsUpdated: "ツール設定を更新しました",
    logPendingAction: "ページ操作を受信しました",
    logChatStart: "チャットを開始しました",
    logChatDone: "回答が完了しました",
    logChatCancelled: "回答をキャンセルしました",
    logChatStop: "ユーザーが回答を停止しました",
    logChatRegenerate: "回答を再生成します",
    logToolRun: "ツールを実行",
    logToolSelected: "ツールを選択",
    logAskSelectionReady: "サイドパネルで質問する準備ができました",
    logNewChat: "新しいチャットを開始",
    logAttachmentAdded: "添付を追加",
    logConversationLoaded: "保存済みチャットを読み込み",
    logMcpApproval: "MCP ツールの許可：{decision} - {server} / {tool}",
    logMcpToolCalled: "MCP ツール呼び出し成功：{server} / {tool}",
    logMcpToolFailed: "MCP ツール呼び出し失敗：{server} / {tool}",
    logMcpToolBlocked: "MCP ツールは実行されませんでした：{server} / {tool}",
    logMcpServerSaved: "MCP サーバーを保存：{server}（{count} ツール）",
    logMcpToolsRefreshed: "MCP ツールを更新：{server}（{count} ツール）",
    logMcpServerDeleted: "MCP サーバーを削除：{server}",
    logEnabled: "有効",
    logDisabled: "無効",
    logRuntimeRequest: "実行リクエスト",
    displayLogLevel: "表示するログレベル",
    displayLogLevelHelp:
      "ログパネルには選択したレベル以上のみ表示されます。デバッグでは各モデルリクエストの詳細を表示します。",
    cancel: "キャンセル",
    close: "閉じる",
    save: "保存",
    saving: "保存中",
    add: "追加",
    edit: "編集",
    delete: "削除",
    test: "テスト",
    testing: "テスト中",
    current: "現在",
    modelRoles: "役割",
    defaultModelRole: "既定",
    translationModelRole: "翻訳",
    visionModelRole: "画像",
    setDefaultModelRole: "既定のモデルに設定",
    setTranslationModelRole: "翻訳モデルに設定",
    clearTranslationModelRole: "翻訳モデルを解除",
    setVisionModelRole: "画像モデルに設定",
    clearVisionModelRole: "画像モデルを解除",
    visionModelRoleUnavailable: "このエンジンでは画像認識が無効です",
    settings: "設定",
    loading: "読み込み中",
    newChat: "新しいチャット",
    send: "送信",
    stop: "停止",
    restorePage: "ページを復元",
    chooseModel: "モデルを選択",
    currentModelEngine: "現在のモデルエンジン",
    selectTool: "ツールを選択",
    selectMoreTools: "他のツールを選択",
    moreTools: "その他のツール",
    copyContent: "内容をコピー",
    clearComposer: "入力をクリア",
    copyCurrentBody: "本文をコピー",
    copyUrl: "ページ URL をコピー",
    copied: "コピー済み",
    regenerate: "再回答",
    continueExecution: "続けて実行",
    replace: "置換",
    closeNotice: "通知を閉じる",
    removeAttachment: "添付を削除",
    openTools: "ツールを開く",
    currentPage: "現在のページ",
    currentBody: "現在の本文",
    currentSelection: "現在の選択",
    currentBodyPreview: "コンテキストプレビュー",
    nonePreview: "プレビュー",
    pagePreview: "ページプレビュー",
    bodyPreview: "本文プレビュー",
    selectionPreview: "選択プレビュー",
    expand: "展開",
    collapse: "折りたたむ",
    currentBodyBlocks: "{count} ブロック",
    currentBodyChars: "{count} 文字",
    currentBodyScore: "本文スコア：{score}",
    articleMetricLength: "長さ",
    articleMetricStructure: "構造",
    articleMetricHeading: "見出し",
    articleMetricSemantics: "セマンティック要素",
    articleMetricDensity: "テキスト密度",
    articleMetricLinkPurity: "リンク純度",
    articleMetricFocus: "集中度",
    articleMetricCleanliness: "クリーン度",
    currentBodySourceDom: "DOM",
    currentBodySourceRule: "ルール",
    currentBodySourceManual: "手動選択",
    currentBodySourceEdited: "編集済み",
    selectCurrentBody: "本文を選択",
    restoreCurrentBody: "本文を復元",
    highlightCurrentBodyBlock: "ページ上でこの段落をハイライト",
    removeCurrentBodyBlock: "現在の本文からこのブロックを削除",
    smartPruneCurrentBody: "自動除外",
    modelPruneCurrentBody: "AI 除外",
    modelPruneCurrentBodyRunning: "モデルに本文 block を判定させています",
    modelPruneInvalidResult: "モデルが返した本文除外結果を解析できません",
    modelPruneNoChanges: "モデルは除外する本文 block がないと判断しました",
    modelPruneAllRejected: "誤削除を防ぐため、本文 block をすべて除外することはできません",
    articleRecognition: "本文認識",
    recognitionRules: "認識ルール",
    articleExtractionRules: "本文抽出ルール",
    articleExtractionRulesHelp:
      "各ルールには対象ページ URL と本文 selector を含めます。URL はドメイン、ワイルドカード、URL 断片に対応します。",
    articleExtractionUrlPattern: "対象ページ URL",
    articleExtractionSelector: "本文 selector",
    articleExtractionUrlPatternPlaceholder:
      "ページ URL。例: example.com、*.example.com、https://example.com/*",
    articleExtractionSelectorPlaceholder:
      "CSS selector。例: article、main、#content、.post-body",
    addArticleExtractionRule: "本文ルールを追加",
    deleteArticleExtractionRule: "本文ルールを削除",
    noArticleExtractionRules: "本文抽出ルールはまだありません",
    articleExtractionRuleSaved: "本文抽出ルールを保存しました",
    articleExtractionRuleInvalid: "ページ URL と selector を入力してください",
    selectingBodyRange: "本文範囲を選択中",
    manualBodySelectionHint: "ホイール/矢印キーで範囲調整、クリックで確定、Esc でキャンセル",
    manualBodySelectionCancelled: "本文範囲の選択をキャンセルしました",
    noneContext: "コンテキストなし",
    webSearch: "ウェブ検索",
    composerWebSearch: "ウェブ検索",
    addAttachment: "画像または文書を追加",
    captureScreenshot: "スクリーンショット",
    composerAddScreenshot: "スクリーンショットを追加",
    screenshotSelectArea: "ページ上でドラッグして範囲を選択します。Esc でキャンセルできます。",
    addUrl: "URL を追加",
    you: "あなた",
    ordinaryConversation: "通常のチャット",
    usedTool: "使用したツール",
    questionContext: "質問コンテキスト",
    imageChat: "画像チャット",
    openAnyPage: "任意のページを開いて、ここから始めてください。",
    noEnabledTools: "この場所ではまだツールが有効になっていません",
    toolsPageShowsAll: "ツールページには利用可能なすべてのツールが表示されます。",
    noSavedConversations: "保存されたチャットはまだありません",
    conversationsAutoSave: "1 回のやり取りが完了するとローカルに自動保存されます。",
    languageOptionAuto: "自動",
    languageOptionZhCN: "簡体字中国語",
    languageOptionZhTW: "繁体字中国語",
    languageOptionEn: "英語",
    languageOptionJa: "日本語",
    languageOptionKo: "韓国語",
    appSubtitle: "ローカル優先のブラウザーモデル作業台",
    modelEngines: "モデルエンジン",
    modelEnginesDescription:
      "アカウントやサブスクリプションは不要です。既定、翻訳、画像の各用途にはそれぞれ 1 つのモデルを指定でき、同じモデルに複数のマークを付けられます。翻訳と画像の専用指定がなければ既定モデルを使用します。エンジン設定とキーは拡張機能ストレージに保存されます。",
    addEngine: "エンジンを追加",
    noModelEngines: "モデルエンジンはまだありません",
    noModelEnginesHelp: "主要なクラウドモデル、OpenAI 互換 API、ローカル Ollama を追加できます。",
    pageFeatures: "ページ機能",
    pageFeaturesHelp: "各モジュールの設定を分けて管理し、影響範囲を確認しやすくします。",
    selectionOverlay: "選択オーバーレイ",
    selectionOverlayHelp: "テキスト選択後に表示されるフローティングツールバーを制御します。",
    selectionOverlayMode: "選択オーバーレイ",
    selectionOverlayShortcut: "選択オーバーレイショートカット",
    selectionOverlayShortcutHelp:
      "有効にすると、選択したショートカットを押しながらテキストを選択する場合、または既存の選択範囲を見る場合にだけオーバーレイを表示します。",
    selectionOverlayMinChars: "最小トリガー文字数",
    selectionOverlayMinCharsHelp:
      "この文字数以上を選択した場合にのみ、選択オーバーレイと同じテキストのハイライトを表示します。最小値は 1 です。",
    selectionMatchHighlight: "同じテキストをハイライト",
    selectionMatchHighlightHelp:
      "ページ上の文字を選択すると同じ内容をすべてハイライトし、スクロールバー横にクリック可能な位置マーカーを表示します。",
    selectionMatchHighlightOff: "無効",
    selectionMatchHighlightIgnoreCase: "大文字と小文字を区別しない",
    selectionMatchHighlightCaseSensitive: "大文字と小文字を区別",
    linkTextSelection: "ページ内リンク文字の選択を許可",
    linkTextSelectionHelp:
      "リンク文字を横方向にドラッグすると通常のテキストとして選択できます。縦方向のドラッグはリンク本来の動作を維持し、通常のクリックでは引き続きリンクを開きます。",
    linkSelection: "リンク選択",
    selectionOverlayOff: "無効",
    selectionOverlayOffHelp: "テキスト選択後にクイック入口を表示しません。",
    selectionOverlayAlways: "直接表示",
    selectionOverlayAlwaysHelp: "テキスト選択後すぐに利用可能なツールを表示します。",
    selectionOverlayHover: "先にドットを表示",
    selectionOverlayHoverHelp: "テキスト選択後に小さなドットを表示し、ホバーでツールバーを展開します。",
    urlBlacklist: "URL ブラックリスト",
    selectionOverlayBlacklistHelp: "1 行に 1 ルール。ドメイン、ワイルドカード、URL 断片に対応します。",
    edgeQuickTools: "クイックツール",
    edgeQuickToolsHelp: "リンク選択、端のメニュー、画像テキスト抽出、自動返信をまとめて制御します。",
    edgeDockMenu: "端のメニュー",
    edgeQuickToolsEnable: "端のメニューを有効化",
    edgeQuickToolsEnableHelp: "オフにするとページ右端のメニューは表示されません。",
    edgeQuickToolsBlacklistHelp: "一致したページでは右端のクイックツールを表示しません。",
    quickToolsBlacklistHelp: "一致したページでは、リンク選択、端のメニュー、画像テキスト抽出、自動返信がすべて無効になります。",
    githubCloneUrl: "GitHub クローン URL",
    githubSshCloneUrlRewrite: "GitHub SSH クローンにポート 443 を使用",
    githubSshCloneUrlRewriteHelp:
      "GitHub リポジトリの Code メニューで、SSH クローン URL を ssh.github.com:443 URI に書き換えます。",
    githubSshCloneUrlRewriteOff: "無効",
    githubSshCloneUrlRewriteTextOnly: "テキスト欄のみ有効",
    githubSshCloneUrlRewriteCopyOnly: "コピーボタンのみ有効",
    githubSshCloneUrlRewriteAll: "両方有効",
    immersiveTranslation: "イマーシブ翻訳",
    immersiveTranslationHelp: "翻訳文をページへ書き戻した後の表示方法を制御します。",
    immersiveTranslationAutoWhitelist: "自動イマーシブ翻訳ホワイトリスト",
    immersiveTranslationAutoWhitelistHelp:
      "1 行に 1 ルール。ドメイン、ワイルドカード、URL 断片に対応します。一致するページを開くと自動でイマーシブ翻訳を実行します。",
    immersiveReading: "イマーシブリーディング",
    immersiveReadingHelp: "難易度に合う語句を学習言語へ置き換え、母語と非母語が混在する読書体験を作ります。",
    immersiveReadingStrategy: "実行方式",
    immersiveReadingStrategyLocalFirst: "ローカル優先",
    immersiveReadingStrategyLocalFirstHelp:
      "ローカルの語彙選別とオフライン辞書翻訳を優先し、高速かつ低コストに処理します。モデルは補完にのみ使用します。",
    immersiveReadingStrategyModelPage: "モデル優先",
    immersiveReadingStrategyModelPageHelp:
      "ページ文脈を使った選定と翻訳をモデルに任せます。柔軟ですが、速度とコストはモデルに依存します。",
    immersiveReadingAutoWhitelist: "自動イマーシブリーディングホワイトリスト",
    immersiveReadingAutoWhitelistHelp:
      "1 行に 1 ルール。ドメイン、ワイルドカード、URL 断片に対応します。一致するページを開くと自動でイマーシブリーディングを実行します。",
    immersiveReadingDifficulty: "語句の難易度",
    immersiveReadingDifficultyHelp: "難易度が高いほど、より少数の難しい語句だけを置き換えます。",
    immersiveReadingMode: "置換モード",
    immersiveReadingParagraphShortcut: "現在の段落のイマーシブリーディングショートカット",
    immersiveReadingContextShortcut: "現在のコンテキストのイマーシブリーディングショートカット",
    immersiveReadingTranslation: "翻訳",
    immersiveReadingOriginalTranslation: "原文（翻訳）",
    immersiveReadingTranslationOriginal: "翻訳（原文）",
    immersiveReadingBackgroundStyle: "置換内容の背景スタイル",
    immersiveReadingHighlightNone: "ハイライトなし",
    immersiveReadingHighlightUniform: "一律ハイライト",
    immersiveReadingHighlightLeveled: "レベル別ハイライト",
    immersiveReadingOuterEffects: "本文",
    immersiveReadingInnerEffects: "括弧内",
    immersiveReadingApplied: "イマーシブリーディングを適用しました",
    displayMode: "表示モード",
    translationOnly: "翻訳のみ",
    bilingual: "原文 + 翻訳",
    translationStyle: "翻訳スタイル",
    translationStyleDefault: "標準",
    translationStyleHighlight: "ハイライト",
    translationStyleDivider: "区切り線",
    translationStyleQuote: "引用",
    translationStyleBlur: "ぼかし",
    translationStyleTransparent: "透明",
    textEffects: "文字効果",
    underline: "実線下線",
    dashedUnderline: "破線下線",
    largeText: "大きめ文字",
    smallText: "小さめ文字",
    bold: "太字",
    italic: "斜体",
    light: "弱め",
    emphasis: "強調",
    generalConfig: "一般設定",
    generalConfigHelp: "検索ページ拡張、既定コンテキスト、外観を制御します。",
    appearanceTheme: "外観テーマ",
    appearanceThemeHelp: "拡張機能の画面表示をシステム設定、ライト、ダークから選択します。",
    themeSystem: "システムに従う",
    themeLight: "ライト",
    themeDark: "ダーク",
    autoScrollDuringStreaming: "生成中に自動スクロール",
    autoScrollDuringStreamingHelp:
      "モデルの生成中、サイドバーを最新の出力に自動追従させます。",
    autoReply: "自動返信",
    autoReplyOff: "無効",
    autoReplyMultiline: "複数行フィールドのみ",
    autoReplyAll: "すべてのテキストフィールド",
    autoReplyHelp: "編集可能な入力欄の右上に小さなアイコンを表示し、現在のページを参考に短い返信を作成します。",
    autoReplyBlacklistHelp: "一致したページでは入力欄に自動返信の入口を表示しません。",
    imageTextExtraction: "画像テキスト抽出",
    imageTextExtractionHelp: "画像にホバーしたときにテキスト抽出の入口を表示します。",
    imageTextExtractionOff: "無効",
    imageTextExtractionOn: "サイズで有効化",
    imageTextExtractionMinSize: "最小画像サイズ",
    imageTextExtractionMinSizeHelp: "表示上の幅と高さがどちらもこのピクセル値以上の画像だけに入口を表示します。",
    imageTextExtractionBlacklistHelp: "一致したページでは画像にテキスト抽出の入口を表示しません。",
    hoverDefinition: "ホバー辞書",
    hoverDefinitionHelp: "中国語の語句または英単語にマウスを置くと、内蔵のオフライン辞書を使用して簡潔な意味を1行で表示します。",
    hoverDefinitionOff: "無効",
    hoverDefinitionChinese: "中国語のみ",
    hoverDefinitionEnglish: "英語のみ",
    hoverDefinitionBoth: "中国語と英語",
    hoverDefinitionStyle: "ホバー語句のスタイル",
    hoverDefinitionStyleNone: "なし",
    hoverDefinitionStyleHighlight: "ハイライト",
    hoverDefinitionStyleUnderline: "下線",
    hoverDefinitionShortcut: "ホバー単語検索ショートカット",
    hoverDefinitionShortcutHelp: "有効にすると、選択したショートカットを押しながら単語にホバーしたときに意味を表示します。",
    hoverDefinitionBlacklistHelp: "一致したページではホバー辞書を表示しません。",
    searchAnswerSetting: "検索ページにモデル回答を表示",
    searchAnswerSettingHelp:
      "有効にすると検索結果ページの右側に、DuckDuckGo の検索結果を参考にした自動回答を表示します。",
    selectionSearchEngine: "選択テキストの検索エンジン",
    searchEngine: "検索エンジン",
    selectionSearchEngineHelp: "選択ツールバーの検索ボタンで使用する検索エンジンです。",
    selectionFixedTools: "選択テキスト固定ツール",
    selectionFixedToolsHelp: "サイドバーへの質問とその他のツールより前に表示する固定ツールを選択します。",
    selectionFixedToolCopy: "コピー",
    selectionFixedToolSearch: "検索",
    selectionFixedToolTodo: "ToDo",
    selectionFixedToolQrCode: "QRコード",
    selectionTodo: "選択テキストをToDoに追加",
    selectionQrCode: "選択テキストのQRコードを生成",
    selectionQrCodeTitle: "QRコード",
    selectionQrCodeError: "QRコードを生成できませんでした。選択テキストを短くして再試行してください。",
    messageQrCode: "回答のQRコードを生成",
    toggleMessageQrCode: "回答のQRコードを表示/非表示",
    messageQrCodeTitle: "回答のQRコード",
    messageQrCodeError: "QRコードを生成できませんでした。回答を短くして再試行してください。",
    selectionSearchCustomUrl: "カスタム検索URL",
    selectionSearchCustomUrlHelp: "{query} プレースホルダーを含むHTTP(S) URLを指定します。",
    selectionSearchOpenMode: "選択テキスト検索の開き方",
    searchOpenMode: "検索の開き方",
    selectionSearchOpenModeHelp: "検索結果を新しいタブで開くか、現在のページを置き換えるかを選択します。",
    selectionSearchOpenNewTab: "新しいタブ",
    selectionSearchOpenCurrent: "現在のページ",
    includePageByDefault: "現在のページを既定で添付",
    includePageByDefaultHelp: "送信前に入力欄でページコンテキストをオフにできます。",
    webSearchByDefault: "チャットでウェブ検索を既定で有効化",
    webSearchByDefaultHelp: "通常のサイドバーチャットで DuckDuckGo の結果を使って質問を補足します。",
    toolResponseUseContextLanguage: "質問コンテキストの原文言語でツール回答",
    toolResponseUseContextLanguageHelp:
      "無効時は現在のインターフェース言語を使用します。ツールだけに適用され、ツールプロンプトで明示された回答言語が優先されます。",
    historyLimit: "保存するチャット数の上限",
    modelThinkingTimeout: "モデル思考タイムアウト（秒）",
    modelThinkingTimeoutHelp:
      "0 はタイムアウトなしです。時間を超えると現在の回答を中断し、タイムアウトを知らせます。",
    toolEnable: "ツール有効化",
    toolEnableHelp: "各入口に表示するツールと表示順を選択します。",
    toolSurfaceSelection: "選択オーバーレイ",
    toolSurfaceSelectionHelp: "ウェブページ上でテキストを選択した後に表示されるクイックツール。",
    toolSurfaceHome: "サイドバー面",
    toolSurfaceHomeHelp: "空のサイドバーチャット画面に表示するクイック入口。",
    toolSurfaceEdge: "クイックツール",
    toolSurfaceEdgeHelp: "ページ右端に展開される入口。",
    toolSurfaceContextMenu: "コンテキストメニュー",
    toolSurfaceContextMenuHelp: "ウェブページの WebMind サブメニューに表示するツールを選択します。サイドバーで質問は常に先頭です。",
    webmindContextMenu: "WebMind",
    noToolsEnabled: "有効なツールはありません",
    chooseTools: "ツールを選択",
    chooseToolsHelp: "チェックした順序が表示順です。矢印で微調整できます。",
    builtinTool: "内蔵ツール",
    customTool: "カスタムツール",
    moveUp: "上へ",
    moveDown: "下へ",
    dataSync: "データ同期",
    dataSyncHelp: "エクスポートファイルと Chrome アカウント同期には API キーは含まれません。",
    chromeAccountSync: "Chrome アカウント同期",
    autoSyncNonSensitive: "非機密設定を自動同期",
    autoSyncNonSensitiveHelp:
      "設定、モデルエンジン情報、カスタムツールを Chrome アカウントに同期します。API キーとチャット履歴は同期されません。",
    syncToChrome: "Chrome アカウントへ同期",
    syncFromChrome: "Chrome アカウントから同期",
    syncing: "同期中",
    syncSecretNote: "同期後のモデルエンジンにキーがない場合は、このデバイスで再入力してください。",
    exportSettings: "設定をエクスポート",
    importSettings: "設定をインポート",
    clearConversationHistory: "チャット履歴を消去",
    providerEditorAria: "モデルエンジン設定",
    newEngine: "新しいエンジン",
    providerKind: "API 種別",
    providerKindOpenAICompatible: "OpenAI 互換",
    providerKindGrok: "Grok",
    providerKindAnthropic: "Anthropic",
    providerKindGemini: "Gemini",
    providerKindOllama: "Ollama",
    providerName: "表示名",
    providerNamePlaceholder: "例: 社内モデル",
    providerBaseUrl: "API Base URL",
    providerBaseUrlHelp: "保存時にこのドメインへのネットワーク権限を要求します。",
    providerModel: "モデル ID",
    providerModelPlaceholder: "サービス提供元から指定されたもの",
    providerModelHelp: "手入力するか、右のボタンでこの API から取得できます。",
    fetchModels: "モデル一覧を取得",
    providerApiKey: "API キー",
    providerApiKeyPlaceholder: "このデバイスの拡張機能ストレージにのみ保存されます",
    providerSecretStorage: "キーの保存方法",
    providerSecretLocal: "永続保存",
    providerSecretSession: "このセッションのみ",
    providerTemperature: "温度",
    providerMaxTokens: "最大出力 Token",
    providerMaxContext: "最大コンテキスト文字数",
    providerSupportsVision: "画像入力に対応",
    providerSupportsVisionHelp: "モデルが視覚入力に対応しない場合はオフにしてください",
    reasoningStrategy: "推論パラメータ戦略",
    reasoningStrategyHelp: "推論モード有効時にのみ、選択したプロトコルのパラメータを送信します。",
    reasoningStrategyNone: "None",
    reasoningStrategyOpenAiChat: "OpenAI Chat Completions",
    reasoningStrategyAnthropic: "Anthropic Extended Thinking",
    reasoningStrategyGeminiBudget: "Gemini Thinking Budget",
    reasoningStrategyOllama: "Ollama Think",
    reasoningEnabled: "推論モード",
    reasoningUnavailable: "現在のモデルには推論パラメータ戦略が設定されていません",
    composerReasoningUnavailable: "現在のモデルは推論モードに対応していません",
    reasoningEnabledByDefault: "チャットで推論モードを既定で有効化",
    reasoningEnabledByDefaultHelp: "推論パラメータ戦略が設定されたモデルだけに適用されます。",
    providerCustomHeaders: "カスタムヘッダー",
    providerCustomHeadersHelp: "任意。JSON オブジェクト形式。同名フィールドは既定ヘッダーを上書きします。",
    saveEngine: "エンジンを保存",
    providerSaved: "モデルエンジンを保存しました",
    providerDeleted: "モデルエンジンを削除しました",
    providerNameRequired: "エンジン名を入力してください",
    providerBaseUrlRequired: "API URL を入力してください",
    providerModelRequired: "モデル ID を入力してください",
    duckPermissionRequired: "DuckDuckGo 検索ドメインの権限が必要です",
    settingsSyncedToChrome: "設定を Chrome アカウントに同期しました。キーは同期されません",
    settingsSyncedFromChrome: "Chrome アカウントから設定を同期しました。モデルキーはローカルのままです",
    chromeSyncEnabled: "Chrome アカウント同期を有効にしました。キーは同期されません",
    chromeSyncDisabled: "Chrome アカウント同期を無効にしました",
    settingsExported: "設定をエクスポートしました。キーは含まれていません",
    invalidSettingsFile: "有効な WebMind 設定ファイルではありません",
    settingsImported: "設定をインポートしました。各エンジンのキーを再入力してください",
    localHistoryCleared: "ローカルのチャット履歴を消去しました",
    processCurrentContent: "現在の内容を処理",
    promptTools: "プロンプトツール",
    localRecords: "ローカル記録",
    addTool: "ツールを追加",
    saveChanges: "変更を保存",
    saveTool: "ツールを保存",
    custom: "カスタム",
    icon: "アイコン",
    chooseIcon: "アイコンを選択",
    noIcon: "アイコンなし",
    searchIcon: "アイコンを検索",
    noIconMatches: "一致するアイコンがありません",
    toolPrompt: "プロンプト",
    mainNav: "メインナビゲーション",
    connectEngineBannerTitle: "先にモデルエンジンを接続してください",
    connectEngineBannerDescription: "主要なクラウドモデル、OpenAI 互換 API、ローカル Ollama に対応しています。",
    pageRecognized: "認識済み",
    enterAttachmentUrl: "添付として追加する URL を入力",
    modelEngineRequired: "先にモデルエンジンを追加して選択してください",
    needPdfPermission: "この PDF を読むにはページ権限が必要です",
    noReadableTab: "読み取れる現在のタブがありません",
    switchingToCurrentPage: "現在のページへ切り替えています…",
    readingSelection: "選択内容を読み取っています…",
    readingCurrentBody: "現在の本文を読み取っています…",
    noSelectionOnPage: "現在のページには切り替え可能な選択内容がありません",
    searchPermissionRequired: "ウェブ結果を補足するには検索ドメイン権限が必要です",
    searchingWeb: "ウェブを検索しています…",
    previewDemoAnswer:
      "これはプレビューモードのサンプル回答です。Chrome 拡張機能として読み込み、モデルエンジンを設定すると、ここに実際のストリーミング出力が表示されます。\n\n- ページコンテキストはいつでも切り替え可能\n- 自分のモデルとキーを使用可能\n- 履歴はローカルにのみ保存",
    currentAnswer: "現在の回答",
    collectingSelection: "選択内容を収集しています…",
    collectingCurrentBody: "現在の本文を収集しています…",
    collectingTranslatableText: "翻訳可能なテキストを収集しています…",
    collectingPageBody: "本文を収集しています…",
    noTranslatableBlocks: "現在のページには翻訳可能な本文ブロックがありません",
    translatingPageProgress: "ページを翻訳中",
    translatingShort: "翻訳中",
    translationWritten: "書き込み済み",
    translationComplete: "完了",
    translationApplied: "ページに追加しました",
    translationRemoved: "ページ翻訳を削除しました",
    pageRestored: "ページを復元しました",
    addImageBeforeAnalyze: "画像分析ツールを実行する前に画像を追加してください",
    toolNeedsPrompt: "ツールにはタイトルとプロンプトが必要です",
    chooseContextFirst: "先にコンテキストで現在のページまたは選択内容を選んでください",
    directQuestionPlaceholder: "直接質問…",
    askContextPlaceholder: "現在のページに質問…",
    composerShortcutHint: "Enter で送信、Shift+Enter で改行",
    addEngineFirst: "先に設定でモデルエンジンを追加してください",
    copyFailed: "コピーに失敗しました",
    readingPdf: "PDF を読み取り中",
    needSearchDomainPermission: "ウェブ結果を補足するには検索ドメイン権限が必要です",
    profileVisionDisabled: "「{name}」は画像入力が有効ではありません",
    promptImageAnalysis: "この画像を分析し、重要な詳細を説明して次の質問に答えてください。",
    promptSummarizeSelection: "選択テキストを要約し、重要な事実、数字、結論を保ってください。",
    promptExplainSelection: "選択テキストを平易な言葉で説明してください。",
    promptAutoTranslateSelection:
      "選択テキストを自動翻訳してください。中国語が主なら自然な英語に、それ以外が主なら日本語に翻訳してください。",
    promptRewriteSelection: "選択テキストをより明確で自然かつ専門的に書き直してください。",
    promptReplySelection: "選択テキストに基づいて、そのまま送れる返信を下書きしてください。",
    modelRequestFailed: "モデルリクエストに失敗しました",
    savedConversations: "保存済みチャット",
    historySessions: "履歴チャット",
    runtimeUnavailable: "プレビューモードでは拡張機能のバックグラウンドを呼び出せません",
    backgroundNoResponse: "拡張機能のバックグラウンドが応答しませんでした",
    currentPageUnavailable: "現在のページにアクセスできません",
    noActiveTab: "アクティブなタブがありません",
    previewPageTitle: "WebMind 製品調査サンプル",
    previewPageDescription: "UI プレビュー用のサンプル記事",
    previewPageBody:
      "ブラウザーアシスタントは現在のページをコンテキストとして使い、要約、翻訳、説明、返信作成を支援できます。WebMind は単一のアカウント体系に縛られず、ユーザーが自分のモデルサービスを設定できます。",
    contentTruncated: "内容は切り詰められました",
    customHeadersJsonObject: "カスタムヘッダーは JSON オブジェクトである必要があります",
    jsonArrayMissing: "モデルが解析可能な JSON 配列を返しませんでした",
    jsonArrayInvalid: "モデルの応答形式が配列ではありません",
    invalidImageData: "画像データ形式が無効です",
    customToolFallback: "カスタムツール",
    chromeSyncInvalidData: "Chrome アカウント内の WebMind 同期データが無効です",
    chromeSyncNoData: "Chrome アカウントにはまだ WebMind 同期データがありません",
    webSearchFailed: "ウェブ検索に失敗しました",
    webSearchNoResults: "ウェブ検索結果を解析できませんでした",
    searchResultSnippet: "概要",
    searchSourceMarker: "検索",
    selectionDescription: "{count} 文字を選択済み",
    readFileFailed: "ファイルを読み取れません",
    readDocumentFailed: "文書を読み取れません",
    readImageUrlFailed: "画像を読み取れません",
    readUrlFailed: "URL を読み取れません",
    unknownFileType: "不明",
    sizeLabel: "サイズ",
    bytes: "バイト",
    unsupportedDocumentText:
      "このファイル形式はまだブラウザー内で本文を直接抽出できません。",
    documentName: "文書名",
    pdfDocument: "PDF 文書",
    attachmentLabel: "添付",
    documentAttachment: "文書",
    addressLabel: "アドレス",
    typeLabel: "種類",
    contentLabel: "内容",
    noExtractedText: "本文を抽出できませんでした",
    jsonArrayTranslationInstruction:
      "入力は JSON 配列です。各項目には id と text があります。各項目の text を翻訳してください。",
    citationPlaceholderInstruction:
      "{{WEBMIND_CITATION_1}} のような内容は翻訳不可の引用プレースホルダーです。翻訳文では各プレースホルダーを対応する意味位置に一字一句そのまま残し、展開、説明、書き換え、末尾への移動、削除をしないでください。",
    translationOutputOnlyInstruction:
      "<translation-input> 内の原文の翻訳だけを出力してください。このプロンプト、規則、タグ名、<translation-input> タグは出力しないでください。",
    jsonArrayReturnInstruction:
      "コードフェンスなしで JSON 配列だけを返してください。配列要素の形式は {\"id\":\"元の id\",\"text\":\"翻訳文\"} です。",
    translationWriteFailed: "翻訳文は生成されましたが、元のページに書き込めませんでした。もう一度お試しください。",
    originalSelectedContent: "元の選択内容",
    previousResult: "前回の結果",
    continueToolInstruction: "前回の結果に基づいて選択したツールを続けて実行してください。",
    selectionAssistantSystem:
      "あなたはブラウザーの選択テキストアシスタントです。ユーザーが提供した選択内容と既存結果だけに基づいて回答し、提供されていないページ情報を捏造しないでください。",
    userQuestionLabel: "ユーザーの質問",
    currentResultLabel: "現在の結果",
    researchSearchPrefix: "この検索クエリを調査してください",
    openSidebarOpening: "サイドバーを開いています…",
    openSidebarOpened: "サイドバーを開きました。",
    readCurrentPage: "現在のページを読み取っています…",
    noProcessablePageBody: "現在のページには処理できる本文がありません",
    executingTool: "ツールを実行しています…",
    closeQuickTools: "クイックツールを閉じる",
    generateShortAutoReply: "短い自動返信を生成",
    extractImageText: "画像の文字を抽出",
    extractingImageText: "画像の文字を抽出中…",
    imageTextExtractionResult: "画像テキスト抽出",
    imageTextExtractionPrompt:
      "この画像に写っている可視テキストをすべて抽出してください。元の言語を維持し、翻訳しないでください。改行、読み順、箇条書き、表構造をできるだけ保ち、抽出した文字だけを出力して説明はしないでください。",
    noImageTextFound: "利用可能な文字が認識されませんでした",
    retryAnswer: "再回答",
    chooseTool: "ツールを選択",
    rerunExecution: "再実行",
    reextractImageText: "再抽出",
    runSelectedTool: "ツールを実行",
    continueQuestionPlaceholder: "続けて質問…",
    modelNoUsableReply: "モデルが利用可能な返信を返しませんでした",
    autoReplySystem:
      "あなたは WebMind のウェブ自動返信アシスタントです。\n現在のページ内容、会話コンテキスト、入力欄に既にある下書きを使って、そのまま入力欄に入れられる短い返信を生成してください。\nページ内の既存の返信、コメント、メール、チャット内容を優先的に参考にしてください。\n返信本文だけを出力してください。説明、タイトル、Markdown は不要です。",
    autoReplyPageTitle: "ページタイトル",
    autoReplyPageUrl: "ページ URL",
    autoReplyPageDescription: "ページ説明",
    autoReplyPageContent: "現在のページ内容",
    autoReplyDraft: "入力欄の既存内容または下書き",
    autoReplyEmpty: "入力欄は空です。",
    autoReplyRequest:
      "自然で丁寧、具体的かつ短い 1-3 文の返信を生成してください。ページの会話または入力欄の下書きの言語に合わせてください。",
    contextMenuAsk: "サイドバーで質問",
    contextMenuSummarize: "選択範囲を要約",
    contextMenuExplain: "選択範囲を説明",
    contextMenuTranslate: "選択範囲を翻訳",
    contextMenuRewrite: "選択範囲を書き直す",
    contextMenuReply: "返信を下書き",
    contextMenuAnalyzeImage: "この画像を分析",
    cannotDetermineTab: "現在のタブを特定できません",
    provideSearchQuery: "検索内容を入力してください",
    toolNotFound: "ツールが見つかりません",
    youtubeVideoNotFound: "ページ内に動画プレーヤーが見つかりません",
    videoInfoNotFound: "ページ内に動画情報が見つかりません",
    noCaptionsAvailable: "この動画には利用可能な字幕がありません",
    captionsReadFailed: "字幕を読み取れません",
    captionsLabel: "字幕",
    youtubeVideoTitle: "YouTube 動画",
    apiKeyMissing: "「{name}」に API キーが入力されていません",
    responseStreamMissing: "モデル API がレスポンスストリームを返しませんでした",
    providerErrorStatus: "モデル API が {status} を返しました: {detail}",
    requestCancelled: "リクエストはキャンセルされました",
    modelThinkingTimeoutMessage:
      "モデルの思考がタイムアウトしたため、回答を中断しました。"
  },
  ko: {
    askSelectionTitle: "사이드바에서 질문",
    askSelectionDescription: "현재 내용을 사이드바로 보내 이어서 질문합니다",
    copy: "복사",
    more: "더보기",
    webmindAnswer: "WebMind 답변",
    showTools: "WebMind 도구 표시",
    copySelection: "선택한 텍스트 복사",
    searchSelection: "선택한 텍스트 검색",
    openSidebar: "사이드바 열기",
    immersiveTranslateApplied: "페이지에 몰입형 번역을 적용했습니다.",
    searchAnswerSystem:
      "당신은 WebMind 검색 답변 도우미입니다. 사용자는 검색 결과 페이지에 있습니다. 제공된 DuckDuckGo 웹 검색 결과를 바탕으로 답변하고, 필요한 경우 결과의 정보를 우선 인용하세요. 정보가 부족하면 부족하다고 말하고, 제공되지 않은 페이지에 접근한 것처럼 말하지 마세요. 답변은 간결하고 구조적으로 작성하세요.",
    searchQuery: "검색어",
    duckResults: "DuckDuckGo 웹 검색 결과",
    duckNoResults: "DuckDuckGo 웹 검색 결과: 사용할 수 있는 결과가 없습니다.",
    searchAnswerRequest: "이 검색 질문에 직접 답하고, 필요하면 추가 확인 방향을 제시하세요.",
    sourceCitationInstruction:
      "검색 결과에 근거한 각 단락 끝에는 위 결과 번호에 맞는 [검색 N] 출처 표시를 반드시 붙이세요. 마지막에 출처 목록을 만들지 말고 출처 URL도 출력하지 마세요.",
    browserAssistantSystem:
      "당신은 브라우저 안의 글쓰기 및 읽기 도우미입니다. 사용자가 제공한 텍스트를 충실히 처리하고 문맥을 지어내지 마세요.",
    modelToolSystem: "당신은 WebMind 도구 실행기입니다. 도구 지시를 직접 수행하고 사용자에게 필요한 결과만 반환하세요.",
    unsupportedQuickAction: "지원하지 않는 빠른 작업입니다",
    currentContext: "현재 컨텍스트",
    assistantSystem:
      "당신은 브라우저 사이드바에서 동작하는 조사, 읽기, 쓰기 도우미 WebMind입니다. 기본적으로 사용자와 같은 언어로 답하세요.",
    assistantGuard:
      "컨텍스트에 명시된 사실과 추론을 엄격히 구분하세요. 컨텍스트가 부족하면 직접 말하세요. 제공되지 않은 내용을 탐색, 클릭, 읽었다고 주장하지 마세요.",
    selectionContextIntro:
      "다음은 사용자가 직접 선택한 텍스트입니다. 이번 작업은 이 선택 내용만 대상으로 하며 제공되지 않은 페이지 부분으로 확장하지 마세요:",
    pageContextIntro: "다음은 사용자가 첨부한 전체 페이지 컨텍스트입니다:",
    articleContextIntro: "다음은 현재 페이지에서 추출한 본문입니다:",
    translationInputIntro: "다음 내용은 이번에 반드시 번역해야 하는 입력 본문입니다:",
    title: "제목",
    url: "주소",
    description: "설명",
    body: "본문:",
    selectionOnly: "답변 시 선택한 텍스트를 유일한 페이지 자료로 다루세요.",
    pdfCitation: "PDF 질문에 답할 때는 가능하면 페이지 번호를 인용하세요.",
    youtubeCitation: "동영상 질문에 답할 때는 가능하면 자막 타임스탬프를 인용하세요.",
    pageCitation: "현재 페이지를 인용할 때는 '현재 페이지'라고 하고 관련 단락을 밝혀 주세요.",
    searchSummaryIntro:
      "다음은 웹 검색 결과 요약입니다. 답변에서 [검색 1], [검색 2] 형식으로 근거를 표시하고 마지막에 실제 사용한 출처를 나열하세요:",
    noContextImage: "이 이미지를 분석해 주세요.",
    noContextAttachment: "첨부 내용에 근거해 답변해 주세요.",
    attachmentIntro: "사용자가 추가한 첨부 내용:",
    languageSetting: "인터페이스 언어",
    languageSettingHelp: "UI 텍스트, 내장 도구 이름, 기본 프롬프트에 적용됩니다.",
    translationLanguageSetting: "번역 언어",
    translationLanguageSettingHelp:
      "문맥 언어와 인터페이스 언어를 바탕으로 자동 판단하거나 수동으로 지정합니다.",
    defaultContextScopeSetting: "기본 컨텍스트",
    defaultContextScopeHelp:
      "대화, 도구, 몰입 번역 및 몰입 읽기에 기본으로 첨부되는 컨텍스트입니다.",
    immersiveTranslationParagraphShortcut: "현재 문단 몰입 번역 단축키",
    immersiveTranslationPageShortcut: "현재 컨텍스트 몰입 번역 단축키",
    immersiveTranslationModeToggleShortcut: "번역 표시 모드 전환 단축키",
    immersiveTranslationShortcutHelp:
      "문단은 선택 영역 또는 문단 내용에 사용하고, 현재 컨텍스트는 사이드바에서 선택한 컨텍스트를 따릅니다.",
    shortcutCtrl: "Ctrl",
    shortcutAlt: "Alt",
    shortcutShift: "Shift",
    shortcutHoldPrefix: "누르기:",
    navChat: "대화",
    navMcp: "MCP",
    mcpServers: "MCP 서버",
    mcpServersHelp: "SSE 또는 Streamable HTTP 서버를 연결하고 도구를 확인합니다.",
    addMcpServer: "MCP 서버 추가",
    editMcpServer: "MCP 서버 편집",
    noMcpServers: "MCP 서버가 없습니다",
    mcpServerName: "이름",
    mcpServerUrl: "서버 URL",
    mcpTransport: "전송 방식",
    mcpHeaders: "사용자 지정 헤더(JSON)",
    saveAndConnect: "저장 및 연결",
    refreshMcpTools: "도구 새로고침",
    noMcpTools: "도구가 없습니다",
    mcpNoDescription: "설명 없음",
    mcpServerSaved: "MCP 서버가 저장되었습니다",
    mcpPermissionDenied: "네트워크 권한이 허용되지 않았습니다",
    mcpHttpOnly: "MCP 서버에는 HTTP 또는 HTTPS URL이 필요합니다",
    enableMcp: "MCP 도구 선택",
    mcpSelectionSummary: "{servers}개 MCP의 {tools}개 도구",
    approveMcpTool: "MCP 도구 실행을 허용하시겠습니까?",
    mcpDestructiveWarning: "이 도구는 외부 데이터를 변경하거나 삭제할 수 있습니다.",
    deny: "거부",
    allowOnce: "이번 호출 허용",
    allowRound: "이번 턴 허용",
    allowSession: "세션 허용",
    mcpToolApprovalMode: "MCP 도구 권한 모드",
    mcpToolApprovalModeHelp:
      "활성화된 MCP 도구의 전역 실행 권한을 제어합니다. 항상 묻기에서는 이번 호출, 이번 턴 또는 세션 단위로 허용할 수 있습니다.",
    mcpToolApprovalDeny: "항상 거부",
    mcpToolApprovalAsk: "항상 묻기",
    mcpToolApprovalAllow: "항상 허용",
    cookieViewer: "Cookie 보기",
    cookieFormat: "표시 형식",
    cookiePreview: "미리보기",
    cookiePreviewCount: "({count}개 Cookie)",
    mcpToolUsed: "사용한 MCP 도구",
    mcpToolNotCalled: "MCP 도구를 호출하지 않음",
    mcpToolFailed: "MCP 도구 호출 실패",
    mcpToolBlockedGlobalDeny: "전역 설정에 따라 거부됨",
    mcpToolBlockedUserDeny: "사용자가 권한을 거부함",
    mcpToolBlockedApprovalTimeout: "권한 대기 시간이 초과됨",
    navTools: "도구",
    navTodos: "할 일",
    navHistory: "기록",
    navLogs: "로그",
    logsPageTitle: "로그",
    logsPageHeading: "작업 로그",
    historyPageEyebrow: "기록",
    localSessionsTitle: "세션 기록",
    todoPageEyebrow: "할 일",
    localTodosTitle: "할 일",
    todosTitle: "할 일",
    todoItemsTitle: "할 일 목록",
    todoAdd: "할 일 추가",
    todoEdit: "할 일 편집",
    todoContent: "본문",
    todoSplit: "AI로 분할",
    todoSplitRunning: "AI로 할 일을 분할하는 중",
    todoSplitInvalid: "모델이 유효한 할 일을 두 개 이상 반환하지 않았습니다",
    todoSplitSuccess: "할 일을 분할했습니다",
    todoSplitSystemPrompt:
      "사용자가 제공한 하나의 할 일을 서로 독립적이고 실행 가능한 여러 할 일로 나누세요. Markdown이나 설명 없이 JSON 문자열 배열만 반환하세요. 2개 이상 20개 이하로 반환하고, 원문에 근거하지 않은 새 작업은 만들지 마세요.",
    todoSearchPlaceholder: "할 일 검색",
    todoFilter: "할 일 필터",
    todoAll: "전체",
    todoOpen: "미완료",
    todoCompleted: "완료",
    todoMarkComplete: "완료로 표시",
    todoMarkOpen: "미완료로 되돌리기",
    todoMarkInProgress: "진행 중으로 표시",
    todoMarkNotInProgress: "진행 중 표시 해제",
    todoInProgressDisabled: "완료한 할 일은 진행 중으로 표시할 수 없습니다",
    todoExpand: "할 일 펼치기",
    todoCollapse: "할 일 접기",
    todoContentExpand: "내용 펼치기",
    todoContentCollapse: "내용 접기",
    todoDelete: "할 일 삭제",
    todoClearCompleted: "완료한 항목 지우기",
    todoClearAll: "모든 할 일 지우기",
    todoEmpty: "아직 할 일이 없습니다",
    todoEmptyHelp: "선택 오버레이, 컨텍스트 메뉴 또는 여기에서 만들 수 있습니다.",
    todoNoSearchResults: "일치하는 할 일이 없습니다",
    todoCreatedFromSelection: "선택한 텍스트에서 생성",
    todoCreatedManually: "수동 생성",
    todoSaved: "할 일에 추가했습니다",
    todoTitleRequired: "할 일 본문을 입력하세요",
    logTodoSplit: "AI로 할 일을 분할했습니다",
    messageAddToTodo: "답변을 할 일에 추가",
    logTodoCreated: "할 일을 만들었습니다",
    logTodoUpdated: "할 일을 업데이트했습니다",
    logTodoCompleted: "할 일을 완료했습니다",
    logTodoReopened: "할 일을 다시 열었습니다",
    logTodoDeleted: "할 일을 삭제했습니다",
    logTodoCleared: "완료한 할 일을 지웠습니다",
    logTodoAllCleared: "모든 할 일을 지웠습니다",
    addSelectionToTodo: "할 일에 추가",
    operationLogs: "작업 로그",
    operationLogsHelp: "제품이 실행한 주요 작업이 실시간으로 표시됩니다.",
    clearLogs: "로그 비우기",
    noOperationLogs: "아직 로그가 없습니다",
    logLevelDebug: "디버그",
    logLevelInfo: "정보",
    logLevelSuccess: "성공",
    logLevelWarning: "알림",
    logLevelError: "오류",
    logSidepanelReady: "사이드패널 준비 완료",
    logSettingsUpdated: "설정 업데이트됨",
    logToolsUpdated: "도구 설정 업데이트됨",
    logPendingAction: "페이지 작업 수신",
    logChatStart: "대화 시작",
    logChatDone: "답변 완료",
    logChatCancelled: "답변 취소됨",
    logChatStop: "사용자가 답변을 중지함",
    logChatRegenerate: "답변 다시 생성",
    logToolRun: "도구 실행",
    logToolSelected: "도구 선택",
    logAskSelectionReady: "사이드패널 질문 준비 완료",
    logNewChat: "새 대화 시작",
    logAttachmentAdded: "첨부 추가",
    logConversationLoaded: "저장된 대화 불러오기",
    logMcpApproval: "MCP 도구 권한: {decision} - {server} / {tool}",
    logMcpToolCalled: "MCP 도구 호출 성공: {server} / {tool}",
    logMcpToolFailed: "MCP 도구 호출 실패: {server} / {tool}",
    logMcpToolBlocked: "MCP 도구가 실행되지 않음: {server} / {tool}",
    logMcpServerSaved: "MCP 서버 저장됨: {server} ({count}개 도구)",
    logMcpToolsRefreshed: "MCP 도구 새로고침: {server} ({count}개 도구)",
    logMcpServerDeleted: "MCP 서버 삭제됨: {server}",
    logEnabled: "켜짐",
    logDisabled: "꺼짐",
    logRuntimeRequest: "실행 요청",
    displayLogLevel: "표시할 로그 수준",
    displayLogLevelHelp:
      "로그 패널은 선택한 수준 이상만 표시합니다. 디버그에는 모든 모델 요청의 상세 기록이 포함됩니다.",
    cancel: "취소",
    close: "닫기",
    save: "저장",
    saving: "저장 중",
    add: "추가",
    edit: "편집",
    delete: "삭제",
    test: "테스트",
    testing: "테스트 중",
    current: "현재",
    modelRoles: "역할",
    defaultModelRole: "기본",
    translationModelRole: "번역",
    visionModelRole: "비전",
    setDefaultModelRole: "기본 모델로 설정",
    setTranslationModelRole: "번역 모델로 설정",
    clearTranslationModelRole: "번역 모델 해제",
    setVisionModelRole: "비전 모델로 설정",
    clearVisionModelRole: "비전 모델 해제",
    visionModelRoleUnavailable: "이 엔진은 이미지 인식이 비활성화되어 있습니다",
    settings: "설정",
    loading: "불러오는 중",
    newChat: "새 대화",
    send: "보내기",
    stop: "중지",
    restorePage: "페이지 복원",
    chooseModel: "모델 선택",
    currentModelEngine: "현재 모델 엔진",
    selectTool: "도구 선택",
    selectMoreTools: "더 많은 도구 선택",
    moreTools: "더 많은 도구",
    copyContent: "내용 복사",
    clearComposer: "입력 지우기",
    copyCurrentBody: "본문 복사",
    copyUrl: "페이지 URL 복사",
    copied: "복사됨",
    regenerate: "다시 답변",
    continueExecution: "계속 실행",
    replace: "바꾸기",
    closeNotice: "알림 닫기",
    removeAttachment: "첨부 삭제",
    openTools: "도구 열기",
    currentPage: "현재 페이지",
    currentBody: "현재 본문",
    currentSelection: "현재 선택",
    currentBodyPreview: "컨텍스트 미리보기",
    nonePreview: "미리보기",
    pagePreview: "페이지 미리보기",
    bodyPreview: "본문 미리보기",
    selectionPreview: "선택 미리보기",
    expand: "펼치기",
    collapse: "접기",
    currentBodyBlocks: "{count}개 블록",
    currentBodyChars: "{count}자",
    currentBodyScore: "본문 점수: {score}",
    articleMetricLength: "길이",
    articleMetricStructure: "구조",
    articleMetricHeading: "제목",
    articleMetricSemantics: "시맨틱 컨테이너",
    articleMetricDensity: "텍스트 밀도",
    articleMetricLinkPurity: "링크 순도",
    articleMetricFocus: "집중도",
    articleMetricCleanliness: "정돈도",
    currentBodySourceDom: "DOM",
    currentBodySourceRule: "규칙",
    currentBodySourceManual: "수동 선택",
    currentBodySourceEdited: "편집됨",
    selectCurrentBody: "본문 선택",
    restoreCurrentBody: "본문 복원",
    highlightCurrentBodyBlock: "페이지에서 이 블록 강조",
    removeCurrentBodyBlock: "현재 본문에서 이 블록 삭제",
    smartPruneCurrentBody: "자동 제거",
    modelPruneCurrentBody: "AI 제거",
    modelPruneCurrentBodyRunning: "모델에 본문 block 판정을 요청하는 중",
    modelPruneInvalidResult: "모델이 반환한 본문 제거 결과를 해석할 수 없습니다",
    modelPruneNoChanges: "모델이 제거할 본문 block이 없다고 판단했습니다",
    modelPruneAllRejected: "데이터 손실을 막기 위해 모든 본문 block을 제거할 수 없습니다",
    articleRecognition: "본문 인식",
    recognitionRules: "인식 규칙",
    articleExtractionRules: "본문 추출 규칙",
    articleExtractionRulesHelp:
      "각 규칙에는 적용할 페이지 URL과 본문 selector가 포함됩니다. URL은 도메인, 와일드카드, URL 조각을 지원합니다.",
    articleExtractionUrlPattern: "적용 페이지 URL",
    articleExtractionSelector: "본문 selector",
    articleExtractionUrlPatternPlaceholder:
      "페이지 URL 예: example.com, *.example.com, https://example.com/*",
    articleExtractionSelectorPlaceholder:
      "CSS selector 예: article, main, #content, .post-body",
    addArticleExtractionRule: "본문 규칙 추가",
    deleteArticleExtractionRule: "본문 규칙 삭제",
    noArticleExtractionRules: "본문 추출 규칙이 아직 없습니다",
    articleExtractionRuleSaved: "본문 추출 규칙을 저장했습니다",
    articleExtractionRuleInvalid: "페이지 URL과 selector를 입력하세요",
    selectingBodyRange: "본문 범위 선택 중",
    manualBodySelectionHint: "휠/화살표 키로 범위 조정, 클릭으로 확정, Esc로 취소",
    manualBodySelectionCancelled: "본문 범위 선택을 취소했습니다",
    noneContext: "컨텍스트 없음",
    webSearch: "웹 검색",
    composerWebSearch: "인터넷 검색",
    addAttachment: "이미지 또는 문서 추가",
    captureScreenshot: "스크린샷",
    composerAddScreenshot: "스크린샷 추가",
    screenshotSelectArea: "페이지에서 드래그해 영역을 선택하세요. Esc 키로 취소할 수 있습니다.",
    addUrl: "URL 추가",
    you: "나",
    ordinaryConversation: "일반 대화",
    usedTool: "사용한 도구",
    questionContext: "질문 컨텍스트",
    imageChat: "이미지 대화",
    openAnyPage: "아무 웹페이지나 연 뒤 여기에서 시작하세요.",
    noEnabledTools: "이 위치에는 아직 활성화된 도구가 없습니다",
    toolsPageShowsAll: "도구 페이지에는 사용 가능한 모든 도구가 표시됩니다.",
    noSavedConversations: "저장된 대화가 아직 없습니다",
    conversationsAutoSave: "질문과 답변이 한 번 완료되면 로컬에 자동 저장됩니다.",
    languageOptionAuto: "자동",
    languageOptionZhCN: "중국어 간체",
    languageOptionZhTW: "중국어 번체",
    languageOptionEn: "영어",
    languageOptionJa: "일본어",
    languageOptionKo: "한국어",
    appSubtitle: "로컬 우선 브라우저 모델 작업 공간",
    modelEngines: "모델 엔진",
    modelEnginesDescription:
      "계정이나 구독이 필요 없습니다. 기본, 번역, 비전 역할은 각각 하나의 모델에 지정할 수 있으며 같은 모델에 여러 표시를 동시에 붙일 수 있습니다. 번역과 비전 전용 모델이 없으면 기본 모델을 사용합니다. 엔진 설정과 키는 확장 프로그램 저장소에 보관됩니다.",
    addEngine: "엔진 추가",
    noModelEngines: "모델 엔진이 아직 없습니다",
    noModelEnginesHelp: "주요 클라우드 모델, OpenAI 호환 API 또는 로컬 Ollama를 추가하세요.",
    pageFeatures: "페이지 기능",
    pageFeaturesHelp: "각 모듈 설정을 분리해 영향 범위를 쉽게 확인할 수 있습니다.",
    selectionOverlay: "선택 텍스트 플로팅",
    selectionOverlayHelp: "텍스트 선택 후 표시되는 플로팅 도구 모음을 제어합니다.",
    selectionOverlayMode: "선택 플로팅",
    selectionOverlayShortcut: "선택 플로팅 단축키",
    selectionOverlayShortcutHelp:
      "켜면 선택한 단축키를 누른 채 텍스트를 선택하거나 기존 선택 영역을 확인할 때만 플로팅 도구를 표시합니다.",
    selectionOverlayMinChars: "최소 트리거 글자 수",
    selectionOverlayMinCharsHelp:
      "선택한 글자 수가 이 값 이상일 때 선택 플로팅과 같은 텍스트 강조를 표시합니다. 최솟값은 1입니다.",
    selectionMatchHighlight: "같은 텍스트 강조",
    selectionMatchHighlightHelp:
      "페이지 텍스트를 선택하면 같은 내용을 모두 강조하고 스크롤바 옆에 클릭 가능한 위치 표시를 보여 줍니다.",
    selectionMatchHighlightOff: "사용 안 함",
    selectionMatchHighlightIgnoreCase: "대소문자 무시",
    selectionMatchHighlightCaseSensitive: "대소문자 구분",
    linkTextSelection: "페이지 내 링크 텍스트 선택 허용",
    linkTextSelectionHelp:
      "링크 텍스트를 가로로 드래그하면 일반 텍스트처럼 선택합니다. 세로 드래그는 링크의 원래 동작을 유지하며, 일반 클릭은 계속 링크를 엽니다.",
    linkSelection: "링크 선택",
    selectionOverlayOff: "사용 안 함",
    selectionOverlayOffHelp: "텍스트 선택 후 빠른 진입점을 표시하지 않습니다.",
    selectionOverlayAlways: "바로 표시",
    selectionOverlayAlwaysHelp: "텍스트 선택 후 사용 가능한 도구를 즉시 표시합니다.",
    selectionOverlayHover: "점 먼저 표시",
    selectionOverlayHoverHelp: "텍스트 선택 후 작은 점을 표시하고, 마우스를 올리면 도구 모음을 펼칩니다.",
    urlBlacklist: "URL 블랙리스트",
    selectionOverlayBlacklistHelp: "한 줄에 규칙 하나. 도메인, 와일드카드, URL 조각을 지원합니다.",
    edgeQuickTools: "빠른 도구",
    edgeQuickToolsHelp: "링크 선택, 가장자리 메뉴, 이미지 텍스트 추출, 자동 답장을 함께 제어합니다.",
    edgeDockMenu: "가장자리 메뉴",
    edgeQuickToolsEnable: "가장자리 메뉴 사용",
    edgeQuickToolsEnableHelp: "끄면 웹페이지 오른쪽 가장자리 메뉴가 표시되지 않습니다.",
    edgeQuickToolsBlacklistHelp: "일치하는 페이지에서는 오른쪽 빠른 도구가 표시되지 않습니다.",
    quickToolsBlacklistHelp: "일치하는 페이지에서는 링크 선택, 가장자리 메뉴, 이미지 텍스트 추출, 자동 답장이 모두 비활성화됩니다.",
    githubCloneUrl: "GitHub 클론 URL",
    githubSshCloneUrlRewrite: "GitHub SSH 클론에 포트 443 사용",
    githubSshCloneUrlRewriteHelp:
      "GitHub 저장소의 Code 메뉴에서 SSH 클론 URL을 ssh.github.com:443 URI로 바꿉니다.",
    githubSshCloneUrlRewriteOff: "사용 안 함",
    githubSshCloneUrlRewriteTextOnly: "텍스트 상자만 사용",
    githubSshCloneUrlRewriteCopyOnly: "복사 버튼만 사용",
    githubSshCloneUrlRewriteAll: "모두 사용",
    immersiveTranslation: "몰입 번역",
    immersiveTranslationHelp: "번역문을 페이지에 다시 쓴 뒤의 표시 방식을 제어합니다.",
    immersiveTranslationAutoWhitelist: "자동 몰입 번역 화이트리스트",
    immersiveTranslationAutoWhitelistHelp:
      "한 줄에 규칙 하나. 도메인, 와일드카드, URL 조각을 지원합니다. 일치하는 페이지를 열면 몰입 번역이 자동으로 실행됩니다.",
    immersiveReading: "몰입 읽기",
    immersiveReadingHelp: "난이도에 맞는 단어를 학습 언어로 바꾸어 모국어와 외국어가 섞인 읽기를 제공합니다.",
    immersiveReadingStrategy: "실행 방식",
    immersiveReadingStrategyLocalFirst: "로컬 우선",
    immersiveReadingStrategyLocalFirstHelp:
      "로컬 단어 선별과 오프라인 사전 번역을 우선 사용해 더 빠르고 저렴하게 처리하며, 모델은 보완에만 사용합니다.",
    immersiveReadingStrategyModelPage: "모델 우선",
    immersiveReadingStrategyModelPageHelp:
      "페이지 문맥을 바탕으로 모델이 교체 대상을 고르고 번역합니다. 더 유연하지만 속도와 비용은 모델에 따라 달라집니다.",
    immersiveReadingAutoWhitelist: "자동 몰입 읽기 화이트리스트",
    immersiveReadingAutoWhitelistHelp:
      "한 줄에 규칙 하나. 도메인, 와일드카드, URL 조각을 지원합니다. 일치하는 페이지를 열면 몰입 읽기가 자동으로 실행됩니다.",
    immersiveReadingDifficulty: "단어 난이도",
    immersiveReadingDifficultyHelp: "난이도가 높을수록 더 적고 어려운 단어만 바꿉니다.",
    immersiveReadingMode: "교체 모드",
    immersiveReadingParagraphShortcut: "현재 문단 몰입 읽기 단축키",
    immersiveReadingContextShortcut: "현재 컨텍스트 몰입 읽기 단축키",
    immersiveReadingTranslation: "번역문",
    immersiveReadingOriginalTranslation: "원문(번역문)",
    immersiveReadingTranslationOriginal: "번역문(원문)",
    immersiveReadingBackgroundStyle: "교체 내용 배경 스타일",
    immersiveReadingHighlightNone: "강조 없음",
    immersiveReadingHighlightUniform: "통일 강조",
    immersiveReadingHighlightLeveled: "단계별 강조",
    immersiveReadingOuterEffects: "본문 내용",
    immersiveReadingInnerEffects: "괄호 내용",
    immersiveReadingApplied: "몰입 읽기가 적용되었습니다",
    displayMode: "표시 모드",
    translationOnly: "번역문만",
    bilingual: "원문 + 번역문",
    translationStyle: "번역 스타일",
    translationStyleDefault: "기본",
    translationStyleHighlight: "강조",
    translationStyleDivider: "구분선",
    translationStyleQuote: "인용",
    translationStyleBlur: "흐림",
    translationStyleTransparent: "투명",
    textEffects: "문자 효과",
    underline: "실선 밑줄",
    dashedUnderline: "점선 밑줄",
    largeText: "큰 글자",
    smallText: "작은 글자",
    bold: "굵게",
    italic: "기울임",
    light: "약화",
    emphasis: "강조",
    generalConfig: "일반 설정",
    generalConfigHelp: "검색 페이지 강화, 기본 컨텍스트, 외관을 제어합니다.",
    appearanceTheme: "외관 테마",
    appearanceThemeHelp: "확장 프로그램 인터페이스에 시스템 설정, 밝게 또는 어둡게 표시를 선택합니다.",
    themeSystem: "시스템 따름",
    themeLight: "밝게",
    themeDark: "어둡게",
    autoScrollDuringStreaming: "생성 중 자동 스크롤",
    autoScrollDuringStreamingHelp:
      "모델이 생성하는 동안 사이드바가 최신 출력으로 자동 스크롤됩니다.",
    autoReply: "자동 답장",
    autoReplyOff: "사용 안 함",
    autoReplyMultiline: "여러 줄 입력란만",
    autoReplyAll: "모든 텍스트 입력란",
    autoReplyHelp: "편집 가능한 입력란 오른쪽 위에 작은 아이콘을 표시하고, 현재 페이지를 참고해 짧은 답장을 생성합니다.",
    autoReplyBlacklistHelp: "일치하는 페이지에서는 입력란에 자동 답장 진입점이 표시되지 않습니다.",
    imageTextExtraction: "이미지 텍스트 추출",
    imageTextExtractionHelp: "이미지에 마우스를 올리면 텍스트 추출 진입점을 표시합니다.",
    imageTextExtractionOff: "사용 안 함",
    imageTextExtractionOn: "크기로 사용",
    imageTextExtractionMinSize: "최소 이미지 크기",
    imageTextExtractionMinSizeHelp: "표시된 너비와 높이가 모두 이 픽셀 값 이상인 이미지에만 진입점을 표시합니다.",
    imageTextExtractionBlacklistHelp: "일치하는 페이지에서는 이미지에 텍스트 추출 진입점이 표시되지 않습니다.",
    hoverDefinition: "마우스 오버 뜻풀이",
    hoverDefinitionHelp: "중국어 어구나 영어 단어 위에 마우스를 잠시 두면 내장 오프라인 사전을 사용해 한 줄의 간단한 뜻을 표시합니다.",
    hoverDefinitionOff: "사용 안 함",
    hoverDefinitionChinese: "중국어만",
    hoverDefinitionEnglish: "영어만",
    hoverDefinitionBoth: "중국어와 영어",
    hoverDefinitionStyle: "호버 단어 스타일",
    hoverDefinitionStyleNone: "없음",
    hoverDefinitionStyleHighlight: "강조",
    hoverDefinitionStyleUnderline: "밑줄",
    hoverDefinitionShortcut: "호버 단어 찾기 단축키",
    hoverDefinitionShortcutHelp: "켜면 선택한 단축키를 누른 채 단어에 마우스를 올릴 때 뜻을 표시합니다.",
    hoverDefinitionBlacklistHelp: "일치하는 페이지에서는 마우스 오버 뜻풀이가 표시되지 않습니다.",
    searchAnswerSetting: "검색 페이지에 모델 답변 표시",
    searchAnswerSettingHelp:
      "켜면 검색 결과 페이지 오른쪽에 DuckDuckGo 검색 결과를 참고한 자동 답변을 표시합니다.",
    selectionSearchEngine: "선택 텍스트 검색 엔진",
    searchEngine: "검색 엔진",
    selectionSearchEngineHelp: "선택 도구 모음의 검색 버튼에서 사용할 검색 엔진입니다.",
    selectionFixedTools: "선택 텍스트 고정 도구",
    selectionFixedToolsHelp: "사이드바 질문 및 다른 도구보다 먼저 표시할 고정 도구를 선택합니다.",
    selectionFixedToolCopy: "복사",
    selectionFixedToolSearch: "검색",
    selectionFixedToolTodo: "할 일",
    selectionFixedToolQrCode: "QR 코드",
    selectionTodo: "선택 텍스트를 할 일에 추가",
    selectionQrCode: "선택 텍스트 QR 코드 생성",
    selectionQrCodeTitle: "QR 코드",
    selectionQrCodeError: "QR 코드를 만들 수 없습니다. 선택 텍스트를 줄인 후 다시 시도하세요.",
    messageQrCode: "답변 QR 코드 생성",
    toggleMessageQrCode: "답변 QR 코드 표시/숨기기",
    messageQrCodeTitle: "답변 QR 코드",
    messageQrCodeError: "QR 코드를 만들 수 없습니다. 답변을 줄인 후 다시 시도하세요.",
    selectionSearchCustomUrl: "사용자 지정 검색 URL",
    selectionSearchCustomUrlHelp: "{query} 자리 표시자를 포함한 HTTP(S) URL을 입력하세요.",
    selectionSearchOpenMode: "선택 텍스트 검색 열기 방식",
    searchOpenMode: "검색 열기 방식",
    selectionSearchOpenModeHelp: "검색 결과를 새 탭에서 열지, 현재 페이지에서 이동할지 선택합니다.",
    selectionSearchOpenNewTab: "새 탭",
    selectionSearchOpenCurrent: "현재 페이지",
    includePageByDefault: "현재 페이지 기본 첨부",
    includePageByDefaultHelp: "보내기 전에 입력 영역에서 페이지 컨텍스트를 끌 수 있습니다.",
    webSearchByDefault: "대화에서 웹 검색 기본 사용",
    webSearchByDefaultHelp: "일반 사이드바 대화에서 DuckDuckGo 결과로 질문을 보완합니다.",
    toolResponseUseContextLanguage: "질문 컨텍스트의 원문 언어로 도구 답변",
    toolResponseUseContextLanguageHelp:
      "끄면 현재 인터페이스 언어를 사용합니다. 도구에만 적용되며 도구 프롬프트에 명시된 답변 언어가 우선합니다.",
    historyLimit: "최대 저장 대화 수",
    modelThinkingTimeout: "모델 사고 시간 초과 (초)",
    modelThinkingTimeoutHelp:
      "0은 시간 제한이 없다는 뜻입니다. 시간이 초과되면 현재 답변을 중단하고 시간 초과를 알립니다.",
    toolEnable: "도구 활성화",
    toolEnableHelp: "각 진입점에 표시할 도구와 표시 순서를 선택합니다.",
    toolSurfaceSelection: "선택 텍스트 플로팅",
    toolSurfaceSelectionHelp: "웹페이지에서 텍스트를 선택한 뒤 표시되는 빠른 도구입니다.",
    toolSurfaceHome: "사이드바 위치",
    toolSurfaceHomeHelp: "빈 사이드바 대화 홈에 표시되는 빠른 진입점입니다.",
    toolSurfaceEdge: "빠른 도구",
    toolSurfaceEdgeHelp: "페이지 오른쪽 가장자리에서 펼쳐지는 진입점입니다.",
    toolSurfaceContextMenu: "컨텍스트 메뉴",
    toolSurfaceContextMenuHelp: "웹페이지 오른쪽 클릭 메뉴의 WebMind 하위 메뉴에 표시할 도구를 선택합니다. 사이드바 질문은 항상 첫 번째입니다.",
    webmindContextMenu: "WebMind",
    noToolsEnabled: "활성화된 도구 없음",
    chooseTools: "도구 선택",
    chooseToolsHelp: "선택한 순서가 표시 순서입니다. 화살표로 조정할 수 있습니다.",
    builtinTool: "내장 도구",
    customTool: "사용자 지정 도구",
    moveUp: "위로 이동",
    moveDown: "아래로 이동",
    dataSync: "데이터 동기화",
    dataSyncHelp: "내보낸 파일과 Chrome 계정 동기화에는 API 키가 포함되지 않습니다.",
    chromeAccountSync: "Chrome 계정 동기화",
    autoSyncNonSensitive: "민감하지 않은 설정 자동 동기화",
    autoSyncNonSensitiveHelp:
      "설정, 모델 엔진 정보, 사용자 지정 도구를 Chrome 계정에 동기화합니다. API 키와 대화 기록은 동기화되지 않습니다.",
    syncToChrome: "Chrome 계정으로 동기화",
    syncFromChrome: "Chrome 계정에서 동기화",
    syncing: "동기화 중",
    syncSecretNote: "동기화된 모델 엔진에 키가 없으면 이 기기에서 다시 입력하세요.",
    exportSettings: "설정 내보내기",
    importSettings: "설정 가져오기",
    clearConversationHistory: "대화 기록 비우기",
    providerEditorAria: "모델 엔진 설정",
    newEngine: "새 엔진",
    providerKind: "API 유형",
    providerKindOpenAICompatible: "OpenAI 호환",
    providerKindGrok: "Grok",
    providerKindAnthropic: "Anthropic",
    providerKindGemini: "Gemini",
    providerKindOllama: "Ollama",
    providerName: "표시 이름",
    providerNamePlaceholder: "예: 사내 모델",
    providerBaseUrl: "API Base URL",
    providerBaseUrlHelp: "저장 시 이 도메인에 대한 네트워크 권한을 요청합니다.",
    providerModel: "모델 ID",
    providerModelPlaceholder: "서비스 제공자가 제공한 값",
    providerModelHelp: "직접 입력하거나 오른쪽 버튼으로 이 API에서 가져올 수 있습니다.",
    fetchModels: "모델 목록 가져오기",
    providerApiKey: "API 키",
    providerApiKeyPlaceholder: "이 기기의 확장 프로그램 저장소에만 저장됩니다",
    providerSecretStorage: "키 저장 방식",
    providerSecretLocal: "계속 저장",
    providerSecretSession: "이번 세션만",
    providerTemperature: "온도",
    providerMaxTokens: "최대 출력 Token",
    providerMaxContext: "최대 컨텍스트 문자 수",
    providerSupportsVision: "이미지 입력 지원",
    providerSupportsVisionHelp: "모델이 비전 입력을 지원하지 않으면 끄세요",
    reasoningStrategy: "추론 매개변수 전략",
    reasoningStrategyHelp: "추론 모드가 켜진 경우에만 선택한 프로토콜 매개변수를 전송합니다.",
    reasoningStrategyNone: "None",
    reasoningStrategyOpenAiChat: "OpenAI Chat Completions",
    reasoningStrategyAnthropic: "Anthropic Extended Thinking",
    reasoningStrategyGeminiBudget: "Gemini Thinking Budget",
    reasoningStrategyOllama: "Ollama Think",
    reasoningEnabled: "추론 모드",
    reasoningUnavailable: "현재 모델에 추론 매개변수 전략이 설정되지 않았습니다",
    composerReasoningUnavailable: "현재 모델은 추론 모드를 지원하지 않습니다",
    reasoningEnabledByDefault: "대화에서 추론 모드 기본 사용",
    reasoningEnabledByDefaultHelp: "추론 매개변수 전략이 설정된 모델에만 적용됩니다.",
    providerCustomHeaders: "사용자 지정 요청 헤더",
    providerCustomHeadersHelp: "선택 사항. JSON 객체 형식이며 같은 이름의 필드는 기본 헤더를 덮어씁니다.",
    saveEngine: "엔진 저장",
    providerSaved: "모델 엔진을 저장했습니다",
    providerDeleted: "모델 엔진을 삭제했습니다",
    providerNameRequired: "엔진 이름을 입력하세요",
    providerBaseUrlRequired: "API URL을 입력하세요",
    providerModelRequired: "모델 ID를 입력하세요",
    duckPermissionRequired: "DuckDuckGo 검색 도메인 권한이 필요합니다",
    settingsSyncedToChrome: "설정이 Chrome 계정에 동기화되었습니다. 키는 동기화되지 않았습니다",
    settingsSyncedFromChrome: "Chrome 계정에서 설정을 동기화했습니다. 모델 키는 로컬 기준입니다",
    chromeSyncEnabled: "Chrome 계정 동기화를 켰습니다. 키는 동기화되지 않습니다",
    chromeSyncDisabled: "Chrome 계정 동기화를 껐습니다",
    settingsExported: "설정을 내보냈습니다. 키는 포함되지 않았습니다",
    invalidSettingsFile: "유효한 WebMind 설정 파일이 아닙니다",
    settingsImported: "설정을 가져왔습니다. 각 엔진 키를 다시 입력하세요",
    localHistoryCleared: "로컬 대화 기록을 비웠습니다",
    processCurrentContent: "현재 내용 처리",
    promptTools: "프롬프트 도구",
    localRecords: "로컬 기록",
    addTool: "도구 추가",
    saveChanges: "변경 저장",
    saveTool: "도구 저장",
    custom: "사용자 지정",
    icon: "아이콘",
    chooseIcon: "아이콘 선택",
    noIcon: "아이콘 없음",
    searchIcon: "아이콘 검색",
    noIconMatches: "일치하는 아이콘 없음",
    toolPrompt: "프롬프트",
    mainNav: "기본 탐색",
    connectEngineBannerTitle: "먼저 모델 엔진을 연결하세요",
    connectEngineBannerDescription: "주요 클라우드 모델, OpenAI 호환 API와 로컬 Ollama를 지원합니다.",
    pageRecognized: "인식됨",
    enterAttachmentUrl: "첨부로 추가할 URL 입력",
    modelEngineRequired: "먼저 모델 엔진을 추가하고 선택하세요",
    needPdfPermission: "이 PDF를 읽으려면 페이지 권한이 필요합니다",
    noReadableTab: "읽을 수 있는 현재 탭이 없습니다",
    switchingToCurrentPage: "현재 페이지로 전환 중…",
    readingSelection: "선택 내용을 읽는 중…",
    readingCurrentBody: "현재 본문을 읽는 중…",
    noSelectionOnPage: "현재 페이지에는 전환할 수 있는 선택 내용이 없습니다",
    searchPermissionRequired: "웹 결과를 보완하려면 검색 도메인 권한이 필요합니다",
    searchingWeb: "웹 검색 중…",
    previewDemoAnswer:
      "미리보기 모드의 예시 답변입니다. Chrome 확장 프로그램으로 불러오고 모델 엔진을 설정하면 실제 스트리밍 출력이 여기에 표시됩니다.\n\n- 페이지 컨텍스트는 언제든 켜고 끌 수 있습니다\n- 직접 가져온 모델과 키를 지원합니다\n- 기록은 로컬에만 저장됩니다",
    currentAnswer: "현재 답변",
    collectingSelection: "선택 내용 수집 중…",
    collectingCurrentBody: "현재 본문 수집 중…",
    collectingTranslatableText: "번역 가능한 텍스트 수집 중…",
    collectingPageBody: "본문 수집 중…",
    noTranslatableBlocks: "현재 페이지에 번역 가능한 본문 블록이 없습니다",
    translatingPageProgress: "페이지 번역 중",
    translatingShort: "번역 중",
    translationWritten: "작성됨",
    translationComplete: "완료",
    translationApplied: "페이지에 추가됨",
    translationRemoved: "페이지 번역을 제거했습니다",
    pageRestored: "페이지를 복원했습니다",
    addImageBeforeAnalyze: "이미지 분석 도구를 실행하기 전에 이미지를 추가하세요",
    toolNeedsPrompt: "도구에는 제목과 프롬프트가 필요합니다",
    chooseContextFirst: "먼저 컨텍스트에서 현재 페이지 또는 선택 내용을 선택하세요",
    directQuestionPlaceholder: "직접 질문…",
    askContextPlaceholder: "현재 페이지에 질문…",
    composerShortcutHint: "Enter로 보내기, Shift+Enter로 줄바꿈",
    addEngineFirst: "먼저 설정에서 모델 엔진을 추가하세요",
    copyFailed: "복사 실패",
    readingPdf: "PDF 읽는 중",
    needSearchDomainPermission: "웹 결과를 보완하려면 검색 도메인 권한이 필요합니다",
    profileVisionDisabled: "“{name}”에서 이미지 입력이 활성화되어 있지 않습니다",
    promptImageAnalysis: "이 이미지를 분석하고 중요한 세부사항을 설명한 뒤 다음 질문에 답하세요.",
    promptSummarizeSelection: "선택한 텍스트를 요약하고 핵심 사실, 숫자, 결론을 유지하세요.",
    promptExplainSelection: "선택한 텍스트를 쉬운 말로 설명하세요.",
    promptAutoTranslateSelection:
      "선택한 텍스트를 자동 번역하세요. 중국어가 주된 언어이면 자연스러운 영어로, 그 외에는 한국어로 번역하세요.",
    promptRewriteSelection: "선택한 텍스트를 더 명확하고 자연스럽고 전문적으로 다시 쓰세요.",
    promptReplySelection: "선택한 텍스트를 바탕으로 바로 보낼 수 있는 답장을 작성하세요.",
    modelRequestFailed: "모델 요청 실패",
    savedConversations: "저장된 대화",
    historySessions: "대화 기록",
    runtimeUnavailable: "미리보기 모드에서는 확장 프로그램 백그라운드를 호출할 수 없습니다",
    backgroundNoResponse: "확장 프로그램 백그라운드가 응답하지 않았습니다",
    currentPageUnavailable: "현재 페이지에 접근할 수 없습니다",
    noActiveTab: "활성 탭이 없습니다",
    previewPageTitle: "WebMind 제품 조사 예시",
    previewPageDescription: "UI 미리보기를 위한 예시 글",
    previewPageBody:
      "브라우저 도우미는 현재 페이지를 컨텍스트로 사용해 요약, 번역, 설명, 답장 작성을 도울 수 있습니다. WebMind는 사용자가 단일 계정 체계에 묶이지 않고 자신의 모델 서비스를 설정할 수 있게 합니다.",
    contentTruncated: "내용이 잘렸습니다",
    customHeadersJsonObject: "사용자 지정 요청 헤더는 JSON 객체여야 합니다",
    jsonArrayMissing: "모델이 파싱 가능한 JSON 배열을 반환하지 않았습니다",
    jsonArrayInvalid: "모델 응답 형식이 배열이 아닙니다",
    invalidImageData: "이미지 데이터 형식이 올바르지 않습니다",
    customToolFallback: "사용자 지정 도구",
    chromeSyncInvalidData: "Chrome 계정의 WebMind 동기화 데이터가 올바르지 않습니다",
    chromeSyncNoData: "Chrome 계정에 아직 WebMind 동기화 데이터가 없습니다",
    webSearchFailed: "웹 검색 실패",
    webSearchNoResults: "웹 검색 결과를 파싱하지 못했습니다",
    searchResultSnippet: "요약",
    searchSourceMarker: "검색",
    selectionDescription: "{count}자 선택됨",
    readFileFailed: "파일을 읽을 수 없습니다",
    readDocumentFailed: "문서를 읽을 수 없습니다",
    readImageUrlFailed: "이미지를 읽을 수 없습니다",
    readUrlFailed: "URL을 읽을 수 없습니다",
    unknownFileType: "알 수 없음",
    sizeLabel: "크기",
    bytes: "바이트",
    unsupportedDocumentText:
      "이 파일 형식은 아직 브라우저에서 본문을 직접 추출할 수 없습니다.",
    documentName: "문서 이름",
    pdfDocument: "PDF 문서",
    attachmentLabel: "첨부",
    documentAttachment: "문서",
    addressLabel: "주소",
    typeLabel: "유형",
    contentLabel: "내용",
    noExtractedText: "본문을 추출하지 못했습니다",
    jsonArrayTranslationInstruction:
      "입력은 JSON 배열입니다. 각 항목에는 id와 text가 있습니다. 각 항목의 text를 번역하세요.",
    citationPlaceholderInstruction:
      "{{WEBMIND_CITATION_1}} 같은 내용은 번역하면 안 되는 인용 자리표시자입니다. 번역문에서도 모든 자리표시자를 해당 의미 위치에 그대로 유지하고 펼치거나 설명, 수정, 끝으로 이동, 삭제하지 마세요.",
    translationOutputOnlyInstruction:
      "<translation-input> 안의 원문 번역만 출력하세요. 이 프롬프트, 규칙, 태그 이름 또는 <translation-input> 태그를 출력하지 마세요.",
    jsonArrayReturnInstruction:
      "코드 블록 없이 JSON 배열만 반환하세요. 배열 요소 형식은 {\"id\":\"원래 id\",\"text\":\"번역문\"}입니다.",
    translationWriteFailed: "번역문은 생성됐지만 원래 페이지에 표시하지 못했습니다. 다시 시도하세요.",
    originalSelectedContent: "원래 선택 내용",
    previousResult: "이전 결과",
    continueToolInstruction: "이전 결과를 바탕으로 선택한 도구를 계속 실행하세요.",
    selectionAssistantSystem:
      "당신은 브라우저 선택 텍스트 도우미입니다. 사용자가 제공한 선택 내용과 기존 결과만 근거로 답하고, 제공되지 않은 페이지 정보를 지어내지 마세요.",
    userQuestionLabel: "사용자 질문",
    currentResultLabel: "현재 결과",
    researchSearchPrefix: "이 검색 질문을 조사하세요",
    openSidebarOpening: "사이드바 여는 중…",
    openSidebarOpened: "사이드바가 열렸습니다.",
    readCurrentPage: "현재 페이지 읽는 중…",
    noProcessablePageBody: "현재 페이지에 처리할 수 있는 본문이 없습니다",
    executingTool: "도구 실행 중…",
    closeQuickTools: "빠른 도구 닫기",
    generateShortAutoReply: "짧은 자동 답장 생성",
    extractImageText: "이미지 텍스트 추출",
    extractingImageText: "이미지 텍스트를 추출하는 중…",
    imageTextExtractionResult: "이미지 텍스트 추출",
    imageTextExtractionPrompt:
      "이 이미지에 보이는 모든 텍스트를 추출하세요. 원문 언어를 유지하고 번역하지 마세요. 줄바꿈, 읽기 순서, 목록, 표 구조를 최대한 보존하고 추출한 텍스트만 출력하며 설명은 하지 마세요.",
    noImageTextFound: "사용할 수 있는 텍스트를 인식하지 못했습니다",
    retryAnswer: "다시 답변",
    chooseTool: "도구 선택",
    rerunExecution: "다시 실행",
    reextractImageText: "다시 추출",
    runSelectedTool: "도구 실행",
    continueQuestionPlaceholder: "이어서 질문…",
    modelNoUsableReply: "모델이 사용할 수 있는 답장을 반환하지 않았습니다",
    autoReplySystem:
      "당신은 WebMind의 웹 자동 답장 도우미입니다.\n현재 페이지 내용, 대화 컨텍스트, 입력란의 기존 초안을 사용해 입력란에 바로 넣을 수 있는 짧은 답장을 생성하세요.\n페이지에 있는 기존 답장, 댓글, 이메일, 채팅 내용을 우선 참고하세요.\n답장 본문만 출력하세요. 설명, 제목, Markdown은 사용하지 마세요.",
    autoReplyPageTitle: "페이지 제목",
    autoReplyPageUrl: "페이지 주소",
    autoReplyPageDescription: "페이지 설명",
    autoReplyPageContent: "현재 페이지 내용",
    autoReplyDraft: "입력란의 기존 내용 또는 초안",
    autoReplyEmpty: "입력란이 비어 있습니다.",
    autoReplyRequest:
      "자연스럽고 정중하며 구체적이지만 짧은 1-3문장 답장을 생성하세요. 페이지 대화 또는 입력란 초안의 언어에 맞추세요.",
    contextMenuAsk: "사이드바에서 질문",
    contextMenuSummarize: "선택 내용 요약",
    contextMenuExplain: "선택 내용 설명",
    contextMenuTranslate: "선택 내용 번역",
    contextMenuRewrite: "선택 내용 다시 쓰기",
    contextMenuReply: "답장 초안 작성",
    contextMenuAnalyzeImage: "이 이미지 분석",
    cannotDetermineTab: "현재 탭을 확인할 수 없습니다",
    provideSearchQuery: "검색 내용을 입력하세요",
    toolNotFound: "도구를 찾을 수 없습니다",
    youtubeVideoNotFound: "페이지에서 동영상 플레이어를 찾을 수 없습니다",
    videoInfoNotFound: "페이지에서 동영상 정보를 찾을 수 없습니다",
    noCaptionsAvailable: "이 동영상에는 사용할 수 있는 자막이 없습니다",
    captionsReadFailed: "자막을 읽을 수 없습니다",
    captionsLabel: "자막",
    youtubeVideoTitle: "YouTube 동영상",
    apiKeyMissing: "“{name}”의 API 키가 입력되지 않았습니다",
    responseStreamMissing: "모델 API가 응답 스트림을 반환하지 않았습니다",
    providerErrorStatus: "모델 API가 {status}를 반환했습니다: {detail}",
    requestCancelled: "요청이 취소되었습니다",
    modelThinkingTimeoutMessage:
      "모델의 사고 시간이 초과되어 답변을 중단했습니다."
  }
} satisfies Record<PromptLanguage, Record<string, string>>;

export type UiTextKey = keyof (typeof CORE_UI_TEXT)["zh-CN"];
export type UiTextDictionary = Record<UiTextKey, string>;

const WESTERN_UI_TEXT = {
  es: esUiText,
  fr: frUiText,
  de: deUiText,
  it: itUiText
} satisfies Record<Exclude<ResolvedLanguage, PromptLanguage>, UiTextDictionary>;

export const UI_TEXT = {
  ...CORE_UI_TEXT,
  ...WESTERN_UI_TEXT
} satisfies Record<ResolvedLanguage, UiTextDictionary>;

export function uiText(
  language: AppLanguage | undefined,
  key: UiTextKey
): string {
  return UI_TEXT[resolveLanguage(language)][key] ?? UI_TEXT["zh-CN"][key];
}
